...
5. 授权通过后由主线程实施；重新评估点限于阶段切换、首次不通过、实施转验收或用户改变约束。重新评估不等于重新索取批准；范围未变时沿用已有授权，明确要求当次批准的门禁除外。

## 六门禁主线

所有主线按 `G0 → G1 → G2 → G3 → G4 → G5` 顺序推进，保留全部六门禁记录。G0 先把下表要求映射到实际任务范围与当前代码影响面，逐项记录 `适用` 或 `不适用（N/A）` 及证据；适用项未通过，禁止进入下一门禁或声明完成。整个门禁确无适用项时，以附理由的 N/A 记录结清，不伪称测试通过。

只有范围和代码证据证明要求不涉及本任务时才可 N/A，例如纯文档主线的数据迁移检查；不得因失败、缺少权限、缺少环境或尚未验证而把适用项改为 N/A。不得通过缩小记录范围豁免用户验收要求或本轮引入的风险。范围变化重新核定适用性；门禁本身不授予数据写入、Git 或发布权限。仅本地环境的项目不要求非本地验收，也不得输出生产通过结论。
...
- 每个发现项按根规则规定的任务类型和授权范围归类；正式承接及写回只适用于已授权的治理实施与正式收口。只读评审交付发现项即可完成评审任务，不自动授权整改或写回，也不代表被评对象验收通过。
- 输出按任务类型最小包含：
  - 计划：计划书、范围、验收、实施前置。
  - 实施：修改、验证、Git 状态、剩余风险。
  - 验收：通过/不通过、阻塞/非阻塞、依据、剩余风险。
  - 上线裁决：在项目允许且用户请求该裁决时，给出是否可上线、目标环境证据、回滚与数据风险；仅本地项目写“生产上线裁决不适用”，只裁决本地范围，不列非本地验收为阻塞。
- 已授权的正式收口必须给出新增后续项、关闭后续项、净变化；主线或 Ready 完成后按权威产品/AutoPilot 入口回写项目地图及相关状态。只读评审不因未获写回授权而被要求修改载体。
