...
3. 仅在实时查询不可得时参考历史样本：平均344.8秒、中位数5分52秒、范围4分19秒～6分15秒；原采样日期未记录，2026-09-05规则维护仅保留该旧值，不代表当天重新测量。新采样必须同时记录日期、run/step来源和有效样本数。
...
6. 动态异常阈值为 `max(平均时长 × 1.5, 最近最大值 + 120秒)`；上述旧样本计算值约 `8分37秒`，不得当作当前固定阈值。超过阈值进入疑似卡住检查，先分类为 `unknown`，取得线程、进程或测试报告证据后再归因，不因单纯变慢判定失败。
