...
$postMergeGate = Read-RepoText 'scripts\ci\verify-post-merge-ci.ps1'
...
Assert-TextContains $ciWorkflow @('branches-ignore: [master, main]','workflow_dispatch:','Verify MySQL migration user scope','bash ./scripts/ci/verify-mysql-grants.sh','Run MySQL least-privilege Flyway migration smoke test','Verify backend test order independence','frontend-v2-gate','supply-chain-security','e2e') 'CI workflow pre-PR equivalence'
Assert-TextContains $postMergeWorkflow @('Post-merge verification','checks: read','pull-requests: read','verify-post-merge-ci.ps1','test-workflow-contract.ps1','test-codex-task-execution-policy.ps1') 'post-merge verification workflow'
Assert-TextContains $postMergeGate @('PrePrRequiredJobs','POST_MERGE_PR_EVIDENCE_MISSING','POST_MERGE_CI_EVIDENCE_MISSING','merge_commit_sha','check-runs?filter=latest') 'post-merge evidence gate'
