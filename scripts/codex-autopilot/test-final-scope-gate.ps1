...
"@|Set-Content (Join-Path $backlog 'ready-issues.md') -Encoding UTF8
  ...
  $old=$ErrorActionPreference;$ErrorActionPreference='Continue';$output=& pwsh -NoProfile -ExecutionPolicy Bypass -File $runner -RepoRoot $root -ConfigPath $config -MaxIterations 1 -MaxLoops 1 -ApplyBacklogSplit -AuthorizeBranch -AuthorizeCommit -AuthorizeMerge 2>&1|Out-String;$code=$LASTEXITCODE;$ErrorActionPreference=$old
