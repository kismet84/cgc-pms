...
function Get-AutopilotRoute {
  ...
  $roots = @($paths | ForEach-Object { (($_ -replace '\\','/') -split '/')[0] } | Where-Object { $_ -in @('backend','frontend-admin-v2','mobile','deploy','docs') } | Select-Object -Unique)
  ...
  $role = if ($Issue.taskNature -eq '运维治理') { 'ops' } elseif ($text -match 'frontend-admin-v2|mobile') { if ($text -match 'backend') { 'BC' } else { 'B' } } elseif ($text -match 'backend') { 'C' } else { 'F' }
