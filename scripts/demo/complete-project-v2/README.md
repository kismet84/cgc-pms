...
- 同一 package 通过 `sys_bootstrap_state` 的 23 个阶段键续载；已完成且版本相同的阶段跳过。角色账号、工作流状态与结算来源阶段升级版本时会各重放一次，用于把旧夹具收敛到 V293 的九类角色和审批矩阵；重放只能清理旧授权，不能恢复迁移前的宽权限。含 `NOW()` 相对期限的角色驾驶舱阶段每次加载都会重放，避免自然时间推进改变逾期计数。相关 SQL 必须保持幂等。
