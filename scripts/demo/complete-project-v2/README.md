...
- 同一 package 通过 `sys_bootstrap_state` 的 23 个阶段键续载；已完成且版本相同的阶段跳过。角色账号、工作流状态与结算来源阶段升级版本时会各重放一次，用于收敛后续权限/模板迁移造成的本地夹具漂移；含 `NOW()` 相对期限的角色驾驶舱阶段每次加载都会重放，避免自然时间推进改变逾期计数。相关 SQL 必须保持幂等。
...
- 业务门：23 个阶段完成；内部业务编号统一为 `{PREFIX}-{yyyyMMdd}-{3位序号}`，历史演示数据同步规范化，银行流水号、凭证号和发票号不改；8 个角色测试账号，项目经理夹具显式具备 9 项变更权限与 2 项成本汇总权限，10 个固定项目覆盖全部项目类型、项目状态、审批状态及互斥收尾动作阶段，计划只读和缺陷登记/复验均复用既有项目；后续普通模块样本禁止新增项目，只有新的互斥项目生命周期约束且无法复用时才允许调整固定集合；M3 另含计划只读、质量安全、技术管理与竣工收尾分权账号、责任人有效项目成员约束、缺陷登记/复验/项目关闭可写阶段、合法待关闭/已关闭链路和 CLEAN 证据数据；每个角色具备审批中、已通过、已驳回、已撤回、已作废 5 种工作流实例及配套待办、已办、抄送数据，并具备 6 项审批动作权限，业务类型按现有模板轮转；在建项目具备八角色统一“高/中/低/其他”风险筛选数据，以及项目经理待办/临期合同、商务变更/计量/结算、采购/验收/低库存、生产领用、总工技术事项和成本经理两级科目分解样本；成本分解父级金额与四个子级合计一致，其他金额、数量、库存、计量、发票分配和会计平衡差额均为 0。
