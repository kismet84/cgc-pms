...
- 同一 package 通过 `sys_bootstrap_state` 的 23 个阶段键续载；已完成且版本相同的阶段跳过。`ROLE_TEST_ACCOUNTS` 当前为 v6；角色账号、工作流状态与结算来源阶段升级版本时会各重放一次，用于把旧夹具收敛到 V293 的九类角色和审批矩阵；重放只能清理旧授权，不能恢复迁移前的宽权限。含 `NOW()` 相对期限的角色驾驶舱阶段每次加载都会重放，避免自然时间推进改变逾期计数。相关 SQL 必须保持幂等。
...
- 业务门：23 个阶段完成；内部业务编号统一为 `{PREFIX}-{yyyyMMdd}-{3位序号}`，历史演示数据同步规范化，银行流水号、凭证号和发票号不改；8 个角色测试账号与 11 个本地切换账号绑定 V293 规范角色，权限由 Mainline 89 最小权限矩阵提供；公司财务具备 6 项工作流动作，其他专业角色具备 4 项，不由演示包补权。员工账号 `ui26.staff01` 以 ACTIVE 项目成员身份在项目 `520000000000009002` 验证 `site:daily:self`，施工负责人无 `site:daily:edit/self`，演示包不得为通过页面测试扩大角色权限。`demo.schedule.query` 与 `demo.member-readonly` 是包自有的精确分权账号；每次 M3 阶段重放都清除 V293 历史降级残留后仅绑定各自专用角色。10 个固定项目覆盖全部项目类型、项目状态、审批状态及互斥收尾动作阶段，计划只读和缺陷登记/复验均复用既有项目；后续普通模块样本禁止新增项目，只有新的互斥项目生命周期约束且无法复用时才允许调整固定集合；M3 另含计划只读、质量安全、技术管理与竣工收尾分权账号、责任人有效项目成员约束、缺陷登记/复验/项目关闭可写阶段、合法待关闭/已关闭链路和 CLEAN 证据数据；每个角色具备审批中、已通过、已驳回、已撤回、已作废 5 种工作流实例及配套待办、已办、抄送数据，业务类型按现有模板轮转；在建项目具备八角色统一“高/中/低/其他”风险筛选数据，以及项目经理待办/临期合同、项目会计变更/计量/结算、采购/验收/低库存、施工负责人领用、技术负责人技术事项和项目会计两级科目分解样本；成本分解父级金额与四个子级合计一致，其他金额、数量、库存、计量、发票分配和会计平衡差额均为 0。
- 数据质量门：合作方完整性、项目/合同/成本权威字典、成本科目、审批场景与质量处置合同追溯的错误/缺失指标均为 0。
- 幂等门：数据质量阶段重复执行两次，全部数量、金额、业务恒等式和权威校验保持不变。
- 正式验收依据：`docs/archive/v1.5/quality/第52条主线-Phase4.1-全页面演示数据包V2验收报告-2026-07-18.md`。
- 应用规则与三工作树整合验收：`docs/archive/v1.5/quality/第52-53条主线-DQ-005-DQ-009应用闭环与三工作树整合验收报告-2026-07-19.md`。
