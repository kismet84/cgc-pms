...
$frontendTests = (Get-ChildItem "$root\frontend-admin-v2\tests\unit" -Recurse -File |
  Where-Object { $_.Name -match '\.(test|spec)\.ts$' }).Count
$e2eCases = (Select-String -Path "$root\frontend-admin-v2\e2e\*.spec.ts" -Pattern '\btest\(' -AllMatches |
  ForEach-Object { $_.Matches.Count } | Measure-Object -Sum).Sum
$largeVue = Get-ChildItem "$root\frontend-admin-v2\src" -Recurse -Filter *.vue |
