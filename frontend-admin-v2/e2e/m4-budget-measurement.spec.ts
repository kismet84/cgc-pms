...
test.describe('M4 budget and measurement routes', () => {
  ...
  test('renders measurement in three viewports and uploads controlled evidence once', async ({
    ...
    await contract.locator('..').getByRole('option', { name: '业主合同' }).click()
    const period = dialog.getByRole('button', { name: /^计量期间：/ })
    await period.click()
    await period.locator('..').getByRole('option', { name: '2026-07' }).click()
