...
test('master-data deep links render server facts and preserve material redirect state', async ({
  ...
  await page.goto('/v2/partner?source=e2e#list')
  await expect(page).toHaveURL(/\/v2\/partner\?source=e2e#list$/)
  ...
  await page.goto('/v2/org?view=tree')
  ...
  await page.goto('/v2/material?source=legacy#dictionary')
  await expect(page).toHaveURL(/\/v2\/material\/dictionary\?source=legacy#dictionary$/)
