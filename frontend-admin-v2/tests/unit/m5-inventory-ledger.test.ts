...
describe('M5 inventory workspace contract', () => {
  ...
  it('uses server facts, pagination, abort and no manual stock movement', () => {
    ...
    expect(source).not.toContain('filter.materialId')
    expect(source).not.toContain('materialOptions')
    expect(source).not.toContain('loadMaterials')
    expect(source).toContain('placeholder="物料编码、名称或规格"')
    ...
    expect(source).toContain('{{ dateTimeLabel(baseline.cutoffAt) }}')
    expect(source).toContain('{{ dateTimeLabel(item.createdTime) }}')
