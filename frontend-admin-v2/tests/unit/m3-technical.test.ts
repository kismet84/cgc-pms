...
describe('M3 technical management closed loop', () => {
  ...
  it('keeps all technical permissions, authoritative reread and responsive semantics explicit', () => {
    ...
    expect(pageSource).toContain('inspection.wbsTaskId === reference.wbsTaskId')
    expect(pageSource).toContain('const archiveableReferences = computed(() =>')
    expect(pageSource).toContain('selectedArchiveInspections.map((item) => ({')
    expect(pageSource).toContain("'待归档施工依据暂无同 WBS 通过的质量检查'")
