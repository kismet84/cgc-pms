...
describe('M5 purchase request, order and receipt contract', () => {
  ...
  it('uses route-layered procurement execution and re-reads every successful write', () => {
    ...
    expect(orderSource).toContain("session.hasAdminOrPermission('purchase:order:delete')")
    expect(orderSource).toContain("selected.value?.approvalStatus === 'DRAFT'")
    expect(orderSource).toContain("selected.value?.orderStatus === 'DRAFT'")
    expect(orderSource).toContain('await deletePurchaseOrder(id)')
    expect(orderSource).toContain('pendingDeleteCode.value = orderCode(selected.value)')
    expect(orderSource).toContain('clearDetail()')
    expect(orderSource).toContain('title="删除草稿采购订单"')
    expect(sharedSource).toContain('canDelete?: boolean')
    expect(sharedSource).toContain('删除草稿')
