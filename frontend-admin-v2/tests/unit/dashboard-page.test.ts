...
describe('M2 dashboard page', () => {
  ...
  it('opens and disposes an authoritative alert from the dashboard list', async () => {
    ...
    expect(wrapper.get('.v2-dialog-standard').classes()).not.toContain('v2-detail-dialog')
    expect(wrapper.get('.v2-dialog__title > span').text()).toBe('预警处置')
    expect(wrapper.get('.v2-dialog__title .v2-badge').text()).toBe('高')
    ...
    expect(wrapper.get('.v2-dialog__footer').findAll('button')).toHaveLength(3)
