...
describe('M91 F03 closeout server pagination', () => {
  ...
  it('keeps abort, generation and selected-project detail compatibility', () => {
    ...
    expect(page).toContain(':page-size="detailPageSize"')
    expect(page).toContain(':total="detailTotal"')
    expect(page).toContain('projectId.value || !pageChanged')
    expect(page).not.toContain('detailPageChanged')
    expect(service).toContain("params.set('detailPageSize'")
  })
})
