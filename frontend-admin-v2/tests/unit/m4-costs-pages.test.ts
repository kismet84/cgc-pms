...
describe('M4 costs pages', () => {
  it('keeps cost ledger on the standard table and 10-row pagination contract', async () => {
    ...
    expect(commercial.loadCostLedgerSummary).not.toHaveBeenCalled()
  ...
  it('loads accessible all-project facts when the public shell selects all projects', async () => {
    ...
    expect(summaryView.wrapper.text()).not.toContain('全部项目成本汇总（1）')
    ...
    expect(controlView.wrapper.text()).not.toContain('全部项目动态利润（1）')
    expect(controlView.wrapper.find('.v2-ledger-kpis').exists()).toBe(false)
    expect(controlView.wrapper.text()).toContain('项目一')
    expect(controlView.wrapper.text()).not.toContain('整体利润率')
  })
