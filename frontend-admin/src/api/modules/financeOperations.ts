...
export function getFinanceAlerts(status = 'OPEN') {
  return request<FinanceRow[]>({
    url: '/finance-operations/alerts',
    method: 'get',
    params: { status },
  })
}
...
export function handleFinanceAlert(id: string, status: 'RESOLVED' | 'IGNORED', note: string) {
  return request<void>({
    url: `/finance-operations/alerts/${id}/handle`,
    method: 'post',
    data: { status, note },
  })
}
export function runFinanceReconciliation(businessDate?: string) {
  return request<FinanceRow>({
    url: '/finance-operations/reconciliations/run',
    method: 'post',
    params: { businessDate },
  })
}
export function getPaymentSchedules(status?: string) {
  return request<FinanceRow[]>({
    url: '/finance-operations/schedules',
    method: 'get',
    params: { status },
  })
}
...
export function rebuildFinanceSnapshot(projectId: string, date?: string) {
  return request<FinanceRow>({
    url: `/finance-operations/snapshots/${projectId}/rebuild`,
    method: 'post',
    params: { date, mode: 'INCREMENTAL' },
  })
}
...
export function createIntegrationEndpoint(data: FinanceRow) {
  return request<FinanceRow>({
    url: '/finance-operations/integrations/endpoints',
    method: 'post',
    data,
  })
}
