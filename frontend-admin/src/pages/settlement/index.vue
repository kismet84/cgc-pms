  ...
  <div class="project-target-redesign app-page">
    <div class="pt-page-head">
      <a-breadcrumb class="pt-breadcrumb"
        ...
        ><a-breadcrumb-item>结算列表</a-breadcrumb-item></a-breadcrumb
      >
    ...
    <div class="pt-kpi-strip" style="grid-template-columns: repeat(4, 1fr)">
      <div class="pt-kpi">
        <div class="pt-kpi-label">累计结算金额</div>
        <div class="pt-kpi-value">{{ fmtWan(kpi.totalFinalAmount) }}<small>万元</small></div>
      </div>
      <div class="pt-kpi">
        <div class="pt-kpi-label">待审核金额</div>
        <div class="pt-kpi-value">{{ fmtWan(kpi.totalContractAmount) }}<small>万元</small></div>
      </div>
      <div class="pt-kpi">
        <div class="pt-kpi-label">已确认金额</div>
        <div class="pt-kpi-value">{{ fmtWan(kpi.totalPaidAmount) }}<small>万元</small></div>
      </div>
      <div class="pt-kpi">
        <div class="pt-kpi-label">结算进度</div>
        <div class="pt-kpi-value">{{ progressPct }}<small>已定案</small></div>
      </div>
    </div>
    ...
    <div class="pt-ledger-layout">
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'settlementStatus'"
              ><a-tag
                :color="
                  SETTLEMENT_STATUS_COLOR[record.settlementStatus as SettlementStatus] || 'default'
                "
                size="small"
                >{{
                  SETTLEMENT_STATUS_LABEL[record.settlementStatus as SettlementStatus] ??
                  record.settlementStatus
                }}</a-tag
              ></template
            >
            <template v-else-if="column.key === 'ops'"
              ><a class="pt-link" @click="handleView(record)">查看</a
              ><a
      ...
      <aside class="pt-analysis-rail">
        <section class="pt-panel">
          <div class="pt-panel-header">结算状态分布</div>
        ...
        <section class="pt-panel">
          <div class="pt-panel-header">月度结算趋势</div>
          <div class="pt-panel-body">
            <ul class="pt-compact-list">
              <li class="pt-compact-row">
                <span>草稿</span><b>{{ kpi.draftCount }} 条</b>
              </li>
              <li class="pt-compact-row">
                <span>已定案</span><b>{{ kpi.finalizedCount }} 条</b>
              </li>
            </ul>
          </div>
        </section>
        <section class="pt-panel">
          <div class="pt-panel-header">异常结算提醒</div>
          <div class="pt-panel-body">
            <ul class="pt-compact-list">
              <li class="pt-compact-row">
                <span>未付金额</span
                ><b style="color: #ef4444">{{ fmtWan(kpi.totalUnpaidAmount) }} 万</b>
              </li>
            </ul>
          </div>
        </section>
      ...
      <a-form layout="vertical">
        ...
        <a-form-item label="结算类型"
          ><a-select
            ...
            ><a-select-option value="INTERIM">期中结算</a-select-option></a-select
          ></a-form-item
        >
