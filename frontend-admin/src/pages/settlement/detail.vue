...
const attachmentColumns = [
  { title: '文件名', dataIndex: 'originalName', key: 'originalName', width: 240, ellipsis: true },
  { title: '大小', dataIndex: 'fileSize', key: 'fileSize', width: 90 },
  { title: '上传人', dataIndex: 'createdBy', key: 'createdBy', width: 110 },
...
function jumpToSource(sourceType: string, sourceId: string) {
  ...
  if (base) router.push(base + '/' + sourceId)
}
function goBack() {
  router.back()
}
  ...
  <div class="settlement-detail-page lg-page app-page project-target-redesign">
    <div class="pt-page-head">
      <a-breadcrumb class="pt-breadcrumb"
        ...
        ><a-breadcrumb-item>结算详情</a-breadcrumb-item></a-breadcrumb
      >
          ...
          >
            ...
            <div>
              ...
              ><a-tag
                ...
                size="small"
                >{{
                  SETTLEMENT_STATUS_LABEL[detail.settlementStatus as SettlementStatus] ||
            ...
            <div>
              ...
              ><a-tag
                ...
                size="small"
                >{{ APPROVAL_STATUS_LABEL[detail.approvalStatus] || detail.approvalStatus }}</a-tag
              >
            </div>
          </div>
        </section>
                  ...
                  <div>
                    ...
                    ><span>{{
                      detail.settlementType === 'PROGRESS'
                        ? '进度结算'
                        : detail.settlementType === 'FINAL'
                          ? '竣工结算'
                          : detail.settlementType === 'INTERIM'
                            ? '期中结算'
              ...
              >
                <template #bodyCell="{ column, record }"
                  ><template v-if="column.key === 'direction'"
                    ><a-tag :color="DIRECTION_COLOR[record.direction] || 'default'" size="small">{{
                      DIRECTION_LABEL[record.direction] || record.direction
                    }}</a-tag></template
                  ><template
                    v-else-if="
                      ['reportedAmount', 'approvedAmount', 'confirmedAmount'].includes(column.key)
                    "
                    ><span>{{ formatAmount(record[column.key]) }}</span></template
                  ></template
                >
              </a-table>
              ...
              >
                <template #bodyCell="{ column, record }"
                  ...
                  ><template v-else-if="column.key === 'costStatus'"
                    ><a-tag
                      :color="COST_STATUS_COLOR[record.costStatus] || 'default'"
                      size="small"
                      >{{ COST_STATUS_LABEL[record.costStatus] || record.costStatus }}</a-tag
                    ></template
                  ><template
              ...
              <a-timeline v-if="approvalRecords.length > 0" style="padding: 16px 0 20px">
                <a-timeline-item v-for="r in approvalRecords" :key="r.id">
                  <div>
                    ...
                    ><a-tag style="margin-left: 8px" size="small">{{
                      actionNameMap[r.actionType] || r.actionName
                    }}</a-tag
                    ><span
                      v-if="r.nodeName"
