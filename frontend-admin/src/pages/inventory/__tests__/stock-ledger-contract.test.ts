...
describe('stock ledger project contract', () => {
  it('passes projectId to ledger and kpi requests', () => {
    expect(source).toMatch(/const filter = reactive\(\{[\s\S]*?projectId: undefined as string \| undefined/)
  ...
  it('refreshes kpi on search and project change', () => {
    ...
    expect(source).toMatch(/function onProjectChange\(projectId: string \| undefined\) \{[\s\S]*?fetchKpi\(\)/)
  })
})
