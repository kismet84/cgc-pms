...
function save() {
  if (!form.accountCode.trim() || !form.accountName.trim()) return message.warning('请填写账户编码和名称')
  if (!form.openingDate || Number(form.openingBalance) < 0) return message.warning('请填写合法期初信息')
  ...
  <a-modal :open="open" title="资金账户管理" width="920px" :footer="null" @cancel="emit('close')">
    <div class="fund-account-layout">
      <section class="account-list">
        <div class="section-heading"><strong>账户列表</strong><a-button size="small" @click="reset">新增</a-button></div>
        <div class="account-table-wrap">
          <table>
            <thead><tr><th>编码</th><th>名称</th><th>类型</th><th>账号</th><th>状态</th><th>操作</th></tr></thead>
            <tbody>
      ...
      <section class="account-form">
        ...
        <label><span>账户类型 *</span><select v-model="form.accountType"><option value="CASH">现金</option><option value="BANK">银行</option></select></label>
        <label v-if="form.accountType === 'BANK'"><span>开户行 *</span><input v-model="form.bankName" maxlength="128" /></label>
        <label v-if="form.accountType === 'BANK'"><span>完整账号 *</span><input v-model="form.bankAccountNo" maxlength="128" autocomplete="off" /></label>
        <label><span>期初日期 *</span><input v-model="form.openingDate" type="date" /></label>
        <label><span>期初余额 *</span><input v-model="form.openingBalance" type="number" min="0" step="0.01" /></label>
...
<style scoped>
.fund-account-layout { display: grid; grid-template-columns: minmax(0, 1.7fr) minmax(280px, 1fr); gap: 20px; }
...
.account-form { display: grid; align-content: start; gap: 10px; padding: 14px; background: #f8fafc; border-radius: 8px; }
label { display: grid; gap: 4px; }
label span { font-size: 12px; color: #667085; }
input, select, textarea { width: 100%; min-height: 34px; padding: 6px 8px; border: 1px solid #d9d9d9; border-radius: 6px; background: #fff; }
textarea { min-height: 62px; }
@media (max-width: 760px) { .fund-account-layout { grid-template-columns: 1fr; } }
</style>
