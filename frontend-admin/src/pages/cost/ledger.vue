...
const columns = [
  ...
  { title: "成本科目", dataIndex: "costSubjectName", width: 140, ellipsis: true },
  { title: "来源类型", dataIndex: "sourceType", width: 130 },
  { title: "金额(万元)", dataIndex: "amount", width: 130, align: "right" as const },
  { title: "成本日期", dataIndex: "costDate", width: 120 },
  { title: "状态", dataIndex: "costStatus", width: 90 },
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.dataIndex === 'costStatus'">
              <a-tag
                :color="record.costStatus === '已确认' ? 'success' : record.costStatus === '待确认' ? 'processing' : 'default'"
                size="small"
              >
                {{ record.costStatus || "-" }}
              </a-tag>
        ...
        <section class="pt-panel">
          ...
          <div class="pt-panel-body">
            <ul class="pt-compact-list">
              ...
              <li v-if="sourceBreakdown.every((i) => parseFloat(i.amount) === 0)" class="pt-compact-row">
                <span>暂无超预算项</span>
              </li>
            </ul>
          </div>
    ...
    >
      <template v-if="detailItem">
        <a-descriptions :column="2" size="small" bordered>
          ...
          <a-descriptions-item label="生成标识">{{ detailItem.generatedFlag === "1" ? "自动生成" : "手动录入" }}</a-descriptions-item>
