...
test.describe('Dashboard: Charts → Data Cards → Role Tabs', () => {
  ...
  test('should display KPI data cards with meaningful numbers', async ({ page }) => {
    ...
    if (cardCount === 0) {
      ...
      const hasEmptyPage = await page.locator('.empty-page').isVisible().catch(() => false);
      if (hasEmptyPage) {
    ...
    if (!hasNonZeroValue) {
      console.log('All KPI values are zero — this is expected if no project is selected or no data exists');
    } else {
  ...
  test('should switch between role tabs and verify content changes', async ({ page }) => {
    ...
    const initialKpiText = await page.locator('.kpi-card .kpi-title').first().textContent().catch(() => '');
    ...
    if (bmTabVisible) {
      ...
      const bmKpiText = await page.locator('.kpi-card .kpi-title').first().textContent().catch(() => '');
      ...
      const hasBMContent = await page.locator('.kpi-card .kpi-title').first().isVisible().catch(() => false);
    ...
    if (mgmtTabVisible) {
      ...
      const projectBarVisible = await page.locator('.project-bar').isVisible().catch(() => false);
      expect(projectBarVisible).toBeFalsy();

      // Management view shows project ranking table
      const mgmtContentVisible = await page.locator('.kpi-card').first().isVisible().catch(() => false);
