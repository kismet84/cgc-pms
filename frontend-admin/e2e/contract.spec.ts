...
test.describe('Contract Creation & Submission', () => {
  ...
  test('should create a contract through 4-step wizard and submit for approval', async ({
    page,
  }) => {
    ...
    await page.fill(
      '.ant-form-item:has(.ant-form-item-label:has-text("签订日期")) input',
      '2026-06-12',
    )

    // Fill start date and end date
    await page.fill(
      '.ant-form-item:has(.ant-form-item-label:has-text("开始日期")) input',
      '2026-06-12',
    )
    await page.fill(
      '.ant-form-item:has(.ant-form-item-label:has-text("结束日期")) input',
      '2027-06-12',
    )
    ...
    await expect(itemRow.locator('.ant-input-number-input').nth(3)).toHaveValue('300000.00', {
      timeout: 3000,
    })
    ...
    await expect(itemRow2.locator('.ant-input-number-input').nth(3)).toHaveValue('200000.00', {
      timeout: 3000,
    })
