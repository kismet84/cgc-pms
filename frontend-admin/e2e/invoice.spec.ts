...
test.describe('Invoice: Create → Register → Verify', () => {
  ...
  test('should create a new invoice through modal form', async ({ page }) => {
    ...
    const typeSelect = page.locator('.ant-modal .ant-form-item:has(label:has-text("发票类型")) .ant-select');
    ...
    // Fill invoice amount
    await page.locator('.ant-modal .ant-form-item:has(label:has-text("发票金额")) .ant-input-number-input').fill('150000');

    // Fill tax rate
    await page.locator('.ant-modal .ant-form-item:has(label:has-text("税率")) .ant-input-number-input').fill('13');

    // Fill tax amount
    await page.locator('.ant-modal .ant-form-item:has(label:has-text("税额")) .ant-input-number-input').fill('19500');

    // Optionally link a pay record if available — try selecting the first one
    const payRecordSelect = page.locator('.ant-modal .ant-form-item:has(label:has-text("付款记录")) .ant-select');
    await payRecordSelect.click();
    await page.waitForSelector('.ant-select-dropdown:not(.ant-select-dropdown-hidden)', { timeout: 5000 });

    const hasPayRecords = await page.locator('.ant-select-item-option').first().isVisible({ timeout: 3000 }).catch(() => false);
    if (hasPayRecords) {
    ...
    if (hasInvoice) {
      // Verify the invoice number matches
      await expect(invoiceRow.locator('td').first()).toContainText(invoiceNo);

      // Verify verify status shows "待核验" (PENDING)
      const verifyTag = invoiceRow.locator('.ant-tag');
      // The verify status tag should be visible
      await expect(verifyTag.last()).toBeVisible();
    }
  ...
  test('should verify an invoice and check status change', async ({ page }) => {
    ...
    const hasInvoices = await page.locator('.ant-table-tbody tr.ant-table-row').first().isVisible({ timeout: 5000 }).catch(() => false);
    ...
    const verifyBtn = page.locator('.ant-table-tbody tr.ant-table-row').first().locator('button:has-text("核验")');
    ...
    await expect(page.locator('.ant-modal-confirm .ant-modal-confirm-title:has-text("发票核验")')).toBeVisible();
  ...
  test('should mark an invoice as abnormal during verification', async ({ page }) => {
    ...
    const invoiceNoCell = await page.locator('.ant-table-tbody tr.ant-table-row').first().locator('td').first().textContent();
    ...
    await page.waitForSelector('.ant-message-warning, .ant-message-notice', { timeout: 10000 }).catch(() => {});
