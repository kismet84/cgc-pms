...
test.describe('Notification: Bell Popover → Read → SSE', () => {
  ...
  test('should open notification popover on bell click', async ({ page }) => {
    ...
    await expect(
      page.locator('.nb-popover .nb-header button:has-text("全部标为已读")'),
    ).toBeVisible()
  ...
  test('should mark a notification as read by clicking it', async ({ page }) => {
    ...
    if (hasUnread) {
      ...
      const dotVisible = await firstItem
        .locator('.nb-item-dot')
        .isVisible()
        .catch(() => false)
      if (dotVisible) {
  ...
  test('should simulate SSE real-time push via EventSource mock', async ({ page }) => {
    ...
    if (mockVisible) {
      ...
      console.log(
        'SSE mock notification not visible — EventSource mock may not have intercepted correctly',
      )
    }
