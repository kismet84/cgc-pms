...
async function selectFirstAvailableOption(page: import('@playwright/test').Page, selectIndex: number) {
...
test.describe('C3 failure path smoke', () => {
  test('inventory outbound overdraw gives business-level result instead of blank system crash', async ({ page }) => {
    ...
    await page.locator('.ant-form-item:has(label:has-text("出库数量")) .ant-input-number-input').fill('999999')
    ...
    console.log(`failure-path inventory overdraw: businessVisible=${businessVisible} crashVisible=${crashVisible}`)
