...
test.describe('Cost regression smoke', () => {
  ...
  test('dashboard cost view supports month switching without breaking KPI/chart structure', async () => {
    ...
    const currentLabel = (await monthSelect.locator('.ant-select-selection-item').textContent())?.trim()
    ...
    await expect(monthSelect.locator('.ant-select-selection-item')).toHaveText(singleMonthLabel, { timeout: 5000 })
    await expect(sharedPage.locator('.cost-reference-chart canvas').first()).toBeVisible({ timeout: 8000 })

    await monthSelect.click()
    const resetDropdown = sharedPage.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden)').last()
    await expect(resetDropdown).toBeVisible({ timeout: 5000 })
    await resetDropdown.locator('.ant-select-item-option').filter({ hasText: '全部' }).first().click()
    await expect(monthSelect.locator('.ant-select-selection-item')).toHaveText('全部', { timeout: 5000 })
    await expect(costKpis.first()).toBeVisible({ timeout: 5000 })

    await sharedPage.screenshot({ path: 'e2e/screenshots/cost-regression-dashboard-month-switch.png', fullPage: true })
  })
  ...
  test('contract ledger and payment list expose amount-bearing rows for cost comparison context', async () => {
    ...
    await expect(sharedPage.locator('.vxe-header--row .vxe-cell').first()).toBeVisible({ timeout: 5000 })
    ...
    await expect(sharedPage.getByText('申请金额', { exact: true }).first()).toBeVisible({ timeout: 5000 })
    await expect(sharedPage.locator('.vxe-header--row .vxe-cell').first()).toBeVisible({ timeout: 5000 })
    ...
    await sharedPage.screenshot({ path: 'e2e/screenshots/cost-regression-payment-context.png', fullPage: true })
  })
})
