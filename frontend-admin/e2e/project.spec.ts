...
test.describe('Project overview navigation', () => {
  ...
  test('navigates from project list to the first project overview', async () => {
    ...
    await expect(sharedPage.locator('.project-table-wrap .vxe-table').first()).toBeVisible({
      timeout: 10000,
    })
    ...
    if (!hasProjectLink) {
      const totalText =
        (await sharedPage.locator('.project-table-count').first().textContent())?.trim() ?? '未知'
      throw new Error(
        `项目列表为空或当前页无可点击项目编号，无法继续验证 /project/:id/overview 链路。当前计数：${totalText}`,
      )
    }
    ...
    await expect(
      sharedPage.locator('.pt-panel-header').filter({ hasText: '项目经营概览' }),
    ).toBeVisible({
      timeout: 10000,
    })
