...
test.describe('Inventory: Stock In → Stock Out → Balance', () => {
  ...
  test('should perform stock in operation', async ({ page }) => {
    ...
    if (!(await inTab.locator('..').hasClass('ant-tabs-tab-active').catch(() => false))) {
      await inTab.click();
    }
    ...
    const hasWarehouses = await page.locator('.ant-select-item-option').first().isVisible({ timeout: 3000 }).catch(() => false);
    if (!hasWarehouses) {
    ...
    const qtyInput = page.locator('.ant-form-item:has(label:has-text("入库数量")) .ant-input-number-input');
    ...
    await page.screenshot({ path: 'e2e/screenshots/inventory-stock-in-success.png', fullPage: true });
  });
  ...
  test('should verify stock balance updated after stock in', async ({ page }) => {
    ...
    const hasStock = await page.locator('text=当前库存').first().isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (hasStock) {
      ...
      const emptyText = await page.locator('text=该仓库暂无选中物料库存记录').isVisible({ timeout: 3000 }).catch(() => false);
      console.log(emptyText ? 'No stock records found (expected if no stock transactions exist)' : 'Stock page loaded');
  ...
  test('should perform stock out operation', async ({ page }) => {
    ...
    const qtyInput = page.locator('.ant-form-item:has(label:has-text("出库数量")) .ant-input-number-input');
    ...
    await page.waitForSelector('.ant-message-success, .ant-message-error', { timeout: 10000 }).catch(() => {});

    await page.screenshot({ path: 'e2e/screenshots/inventory-stock-out-result.png', fullPage: true });
  });
  ...
  test('should verify transaction log after stock operations', async ({ page }) => {
    ...
    const hasTxns = await page.locator('.ant-table-tbody tr.ant-table-row').first().isVisible({ timeout: 5000 }).catch(() => false);
