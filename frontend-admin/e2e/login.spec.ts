...
test.describe('Login Smoke Test', () => {
  test('should login with valid credentials and redirect to dashboard', async ({ page }) => {
    ...
    await expect(page.locator('text=建筑工程总包项目管理系统').or(page.locator('.ant-layout'))).toBeVisible({ timeout: 10000 });
  ...
  test('should show error for invalid credentials', async ({ page }) => {
    ...
    await expect(page.locator('.ant-message-error').or(page.locator('.ant-message-notice'))).toBeVisible({ timeout: 10000 });
  });
});
