CREATE TABLE sys_dict_group (
    ...
    PRIMARY KEY (id),
    CONSTRAINT uk_sys_dict_group_code UNIQUE (tenant_id, group_code),
    CONSTRAINT uk_sys_dict_group_tenant_id UNIQUE (tenant_id, id),
    CONSTRAINT ck_sys_dict_group_status CHECK (status IN ('ENABLE', 'DISABLE'))
);

CREATE INDEX idx_sys_dict_group_order ON sys_dict_group (tenant_id, status, order_num);

ALTER TABLE sys_dict_type ADD COLUMN group_id BIGINT;
ALTER TABLE sys_dict_type ADD COLUMN dict_class VARCHAR(30) DEFAULT 'BUSINESS' NOT NULL;
...
SET status = 'DISABLE'
WHERE dict_type_id IN (SELECT id FROM sys_dict_type WHERE dict_code = 'pay_type')
  AND dict_value IN ('ADVANCE', 'SETTLEMENT', 'WARRANTY');
...
ALTER TABLE sys_dict_type ALTER COLUMN group_id SET NOT NULL;
CREATE INDEX idx_sys_dict_type_group ON sys_dict_type (tenant_id, group_id, status);
ALTER TABLE sys_dict_type ADD CONSTRAINT fk_sys_dict_type_group
    FOREIGN KEY (tenant_id, group_id) REFERENCES sys_dict_group (tenant_id, id) ON DELETE RESTRICT;
ALTER TABLE sys_dict_type ADD CONSTRAINT ck_sys_dict_type_class
    CHECK (dict_class IN ('BUSINESS', 'SYSTEM', 'STATE_MACHINE'));
