...
SELECT 99100001, 0, 10001, 'BUD-CONTRACT-TEST', 'V1', '合同闭环测试预算',
       22000000.00, 'APPROVED', 'ACTIVE', 1, 10001, CURRENT_TIMESTAMP, 0
WHERE NOT EXISTS (
...
INSERT INTO project_budget_line
    (id, tenant_id, budget_id, project_id, cost_subject_id, budget_amount,
     reserved_amount, consumed_amount, version)
SELECT 99100003, 0, 99100001, 10001, 901002, 17000000.00, 0.00, 0.00, 0
...
FROM (VALUES
    (99100101, 30001, CAST(5000000.00 AS DECIMAL(18,2))),
    (99100102, 30002, CAST(5000000.00 AS DECIMAL(18,2))),
    (99100103, 30003, CAST(5000000.00 AS DECIMAL(18,2)))
