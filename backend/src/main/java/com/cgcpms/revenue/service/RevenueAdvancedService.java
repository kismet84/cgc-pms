...
public class RevenueAdvancedService {
    ...
    public Map<String,Object> reverseCollection(Long collectionId, CollectionReverseRequest request) {
        ...
        if (done != null) {
            Map<String,Object> result = requireSameReversal(done, collectionId, reason);
            mandatoryAuditService.verifyRevenue("COLLECTION_REVERSED", "COLLECTION_RECORD", collectionId,
        ...
        mandatoryAuditService.revenue("COLLECTION_REVERSED", "COLLECTION_RECORD", collectionId,
                longValue(collection.get("project_id")), idempotencyKey, Map.of("reason", reason));
    ...
    public Map<String,Object> createSchedule(CollectionScheduleRequest request) {
        projectAccessChecker.checkAccess(request.projectId(), "创建回款计划");
    ...
    public List<Map<String,Object>> schedules(String status){
        ...
        List<Object> args=new ArrayList<>();args.add(tenant());args.addAll(projectIds);args.add(status);args.add(status);
        return jdbc.queryForList("SELECT * FROM collection_schedule WHERE tenant_id=? AND project_id IN("
    ...
    public Map<String,Object> reconcile(LocalDate date){
        projectAccessChecker.requireAllScope("执行收入全租户对账");
    ...
    public Map<String,Object> rebuildSnapshot(Long projectId,LocalDate date,String mode){
        projectAccessChecker.checkAccess(projectId, "重建收入看板快照");
    ...
    public Map<String,Object> createInvoiceReview(SalesInvoiceReviewRequest request){
        Map<String,Object> invoice=one("SELECT id,project_id,status FROM sales_invoice WHERE id=? AND tenant_id=? AND deleted_flag=0",request.invoiceId(),tenant());
        if(invoice==null)throw error("SALES_INVOICE_NOT_FOUND","销项发票不存在");
        projectAccessChecker.checkAccess(longValue(invoice.get("project_id")),"创建销项发票复核");
        if(!Set.of("FULLY_ALLOCATED","ISSUED").contains(String.valueOf(invoice.get("status"))))throw error("SALES_INVOICE_EVIDENCE_PENDING","销项发票证据确认后才能复核");
    ...
    public Map<String,Object> decideReview(Long id,ReviewDecisionRequest request){
        ...
        Map<String,Object> review=one("SELECT r.*,i.project_id,i.status invoice_status FROM sales_invoice_review r JOIN sales_invoice i ON i.id=r.invoice_id AND i.tenant_id=r.tenant_id WHERE r.id=? AND r.tenant_id=? AND i.deleted_flag=0 FOR UPDATE",id,tenant());
        if(review==null||!"PENDING".equals(review.get("review_status")))throw error("SALES_INVOICE_REVIEW_NOT_PENDING","复核记录不存在或已处理");
        projectAccessChecker.checkAccess(longValue(review.get("project_id")),"处理销项发票复核");
        if(!Set.of("FULLY_ALLOCATED","ISSUED").contains(String.valueOf(review.get("invoice_status"))))throw error("SALES_INVOICE_REVIEW_STATE_INVALID","销项发票状态不允许复核");
        ...
        Long invoiceId=longValue(review.get("invoice_id"));
        if("APPROVED".equals(decision)){
            jdbc.update("UPDATE sales_invoice SET status='ISSUED',verification_status='VERIFIED',version=version+1,updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?",user(),invoiceId,tenant());
        }else{
            jdbc.update("DELETE FROM sales_invoice_allocation WHERE tenant_id=? AND invoice_id=?",tenant(),invoiceId);
            jdbc.update("UPDATE sales_invoice SET allocated_amount=0,status='REJECTED',verification_status='REJECTED',version=version+1,updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?",user(),invoiceId,tenant());
        }
        return one("SELECT * FROM sales_invoice_review WHERE id=?",id);
    }
    ...
    public Map<String,Object> previewImport(RevenueImportRequest request){
        projectAccessChecker.checkAccess(request.projectId(),"预览收入导入");
    ...
    public Map<String,Object> createForecast(Long projectId,Long contractId,LocalDate date,String scenario,BigDecimal amount,BigDecimal confidence,String sourceType,Long sourceId){
        projectAccessChecker.checkAccess(projectId,"创建回款预测");
    ...
    public Map<String,Object> refreshCustomerCredit(Long customerId){
        projectAccessChecker.requireAllScope("刷新客户信用画像");
    ...
    public Map<String,Object> enqueueIntegration(RevenueIntegrationRequest request){
        projectAccessChecker.requireAllScope("创建收入外部同步任务");
    ...
    public List<Map<String,Object>> auditEvents(String type,Long businessId){
        List<Long> projectIds=projectAccessChecker.accessibleProjectIds();
        if(projectIds.isEmpty())return List.of();
        List<Object> args=new ArrayList<>();args.add(tenant());args.add(type);args.add(type);args.add(businessId);args.add(businessId);args.addAll(projectIds);
        return jdbc.queryForList("SELECT * FROM revenue_audit_event WHERE tenant_id=? AND (? IS NULL OR business_type=?) AND (? IS NULL OR business_id=?) AND project_id IN("+placeholders(projectIds.size())+") ORDER BY event_at DESC,id DESC",args.toArray());
    }
