...
public class ProjectCashForecastService {
    ...
    public Map<String,Object> createCycle(CycleRequest request) {
        ...
        requireProjectForecastable(request.projectId(), request.horizonEnd());
    ...
    public List<Map<String,Object>> cycles(Long projectId){
        List<Long> projectIds;
        if(projectId!=null){projectAccessChecker.checkAccess(projectId,"查看资金预测");projectIds=List.of(projectId);}
        else projectIds=projectAccessChecker.accessibleProjectIds();
        if(projectIds.isEmpty())return List.of();
        String placeholders=String.join(",",Collections.nCopies(projectIds.size(),"?"));
        List<Object>args=new ArrayList<>();args.add(tenant());args.addAll(projectIds);
        return jdbc.queryForList("SELECT c.*,p.project_name FROM cash_forecast_cycle c JOIN pm_project p ON p.id=c.project_id AND p.tenant_id=c.tenant_id AND p.deleted_flag=0 WHERE c.tenant_id=? AND c.project_id IN ("+placeholders+") ORDER BY p.project_name,c.scenario,c.version_no DESC",args.toArray());
    }
    ...
    private void requireProjectForecastable(Long projectId,LocalDate horizonEnd){Map<String,Object> p=one("SELECT status,actual_end_date FROM pm_project WHERE id=? AND tenant_id=? AND deleted_flag=0 FOR UPDATE",projectId,tenant());if(p==null)throw error("PROJECT_NOT_FOUND","项目不存在");String status=String.valueOf(p.get("status"));if("SUSPENDED".equals(status))throw error("CASH_FORECAST_PROJECT_NOT_OPERATIONAL","暂停项目不得新建资金预测");if("CLOSED".equals(status)&&(p.get("actual_end_date")==null||horizonEnd.isAfter(localDate(p.get("actual_end_date")))))throw error("CASH_FORECAST_CLOSED_PROJECT_HORIZON_INVALID","关闭项目仅允许补录截至实际完工日的历史预测");}
