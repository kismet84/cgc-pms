...
public interface MatSupplierReturnMapper extends BaseMapper<MatSupplierReturn> {
    @Select("""
            SELECT id,tenant_id,project_id,contract_id,partner_id,purchase_order_id,receipt_id,warehouse_id,
                   return_code,return_date,return_quantity,status,idempotency_key,return_amount AS total_amount,reason,
