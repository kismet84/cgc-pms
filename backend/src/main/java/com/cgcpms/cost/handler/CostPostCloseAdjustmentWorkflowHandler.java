...
@Component
public class CostPostCloseAdjustmentWorkflowHandler implements WorkflowBusinessHandler {
    private final ObjectProvider<CostSubjectV2Service> serviceProvider;

    public CostPostCloseAdjustmentWorkflowHandler(ObjectProvider<CostSubjectV2Service> serviceProvider) {
        this.serviceProvider = serviceProvider;
    }

    @Override public String supportBusinessType() { return WorkflowBusinessTypes.COST_POST_CLOSE_ADJUSTMENT; }
    ...
    private Long id(WorkflowContext context) {
        ...
        if (id == null) throw new IllegalStateException("关闭后成本调整审批实例缺少业务ID");
        return id;
    }
}
