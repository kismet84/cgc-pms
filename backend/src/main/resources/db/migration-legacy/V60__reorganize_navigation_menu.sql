...
-- Keep legacy directories for permission history, but remove them from navigation.
UPDATE sys_menu
SET visible = 0
WHERE id IN (2, 3, 4, 5, 700, 710, 720, 730, 765);

-- Super administrators receive every new domain and page.
INSERT IGNORE INTO sys_role_menu (id, role_id, menu_id)
SELECT 600000 + id, 1, id
FROM sys_menu
WHERE id BETWEEN 900 AND 950 AND deleted_flag = 0;

-- Preserve existing role visibility by granting the new parent of any owned child menu.
INSERT IGNORE INTO sys_role_menu (id, role_id, menu_id)
