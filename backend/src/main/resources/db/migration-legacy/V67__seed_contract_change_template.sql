-- V67__seed_contract_change_template.sql
...
UPDATE wf_template SET template_code='TPL-CONTRACT-CHANGE-001', template_name='合同变更审批流程', enabled=1 WHERE id=50007 AND business_type='CT_CHANGE' AND deleted_flag=0;
...
UPDATE wf_template_node SET node_name='项目经理审批', node_order=1, timeout_hours=48 WHERE id=50701 AND deleted_flag=0;
UPDATE wf_template_node SET node_name='部门经理审批', node_order=2, timeout_hours=48 WHERE id=50702 AND deleted_flag=0;
UPDATE wf_template_node SET node_name='总经理审批',   node_order=3, timeout_hours=72 WHERE id=50703 AND deleted_flag=0;
