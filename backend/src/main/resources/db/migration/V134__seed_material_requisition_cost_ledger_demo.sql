-- V134: ensure project 2071032241708793858 cost ledger can enumerate MAT_REQUISITION.
-- Minimal idempotent demo chain: project -> requisition -> requisition item -> cost_item.
...
SELECT
    ...
    1, 1, NOW(), NOW(), 0, 'V134确保目标项目成本台账可直接枚举MAT_REQUISITION'
FROM dual
WHERE NOT EXISTS (
...
SELECT
    ...
    1, 1, NOW(), NOW(), 0, 'V134第30主线目标项目领料成本台账演示仓库'
FROM dual
WHERE EXISTS (
    SELECT 1 FROM pm_project
    WHERE tenant_id = 0 AND id = 2071032241708793858 AND deleted_flag = 0
...
SELECT
    ...
    CURDATE(), 970000000000006800, 1, 'APPROVED', 180000.0000,
    1, 1, 1, NOW(), NOW(), 0, 'V134第30主线目标项目领料成本台账演示单'
FROM dual
WHERE EXISTS (
    SELECT 1 FROM pm_project
    WHERE tenant_id = 0 AND id = 2071032241708793858 AND deleted_flag = 0
...
SELECT
    ...
    1, 1, NOW(), NOW(), 0, 'V134第30主线目标项目领料成本台账演示明细'
FROM dual
WHERE EXISTS (
    SELECT 1 FROM mat_requisition
...
SELECT
    ...
    1, 1, NOW(), NOW(), 0, 'V134第30主线目标项目成本台账直接枚举MAT_REQUISITION'
