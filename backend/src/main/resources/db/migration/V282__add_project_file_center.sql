CREATE TABLE project_file_catalog (
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_project_file_catalog_tenant_id (tenant_id, id),
    UNIQUE KEY uk_project_file_catalog_code (tenant_id, project_id, file_code),
    UNIQUE KEY uk_project_file_catalog_source
        (tenant_id, source_kind, source_business_type, source_business_id, file_code),
    KEY idx_project_file_catalog_page
        (tenant_id, project_id, category_code, updated_at, id),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目文件中心逻辑目录';
...
CREATE TABLE project_file_version_link (
    ...
    preview_storage_path VARCHAR(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NULL,
    preview_error_code VARCHAR(100) NULL,
    preview_updated_at DATETIME(3) NULL,
    created_by BIGINT NULL,
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    updated_by BIGINT NULL,
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
    ...
    remark VARCHAR(500) NULL,
    PRIMARY KEY (id),
    UNIQUE KEY uk_project_file_version_tenant_id (tenant_id, id),
    UNIQUE KEY uk_project_file_version_no (tenant_id, catalog_id, version_no),
    UNIQUE KEY uk_project_file_version_sys_file (tenant_id, sys_file_id),
    KEY idx_project_file_version_latest (tenant_id, catalog_id, version_no),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目文件中心版本与sys_file关系';

ALTER TABLE sys_file_object_task
    DROP CHECK ck_sys_file_object_task_operation,
    ADD COLUMN reference_id BIGINT NULL AFTER source_path,
    ADD CONSTRAINT ck_sys_file_object_task_operation
...
SELECT 282000300000000 + role_row.id * 10 + menu_row.id % 10,
