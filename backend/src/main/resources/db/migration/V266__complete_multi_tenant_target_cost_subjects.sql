-- Complete the V264 target-cost restructure for every tenant that owns a
-- 5401.03 tree. V264 is already applied locally and therefore stays immutable.
UPDATE cost_subject
...
FROM cost_subject p
JOIN (
    SELECT '5401.03.01' AS subject_code, '人工成本' AS subject_name, 'LABOR' AS subject_type, 1 AS sort_order, 25.0000 AS default_target_ratio
    UNION ALL SELECT '5401.03.02', '材料及工程设备成本', 'MATERIAL', 2, 40.0000
    UNION ALL SELECT '5401.03.03', '施工机械成本', 'MACHINERY', 3, 5.0000
    UNION ALL SELECT '5401.03.04', '分包成本', 'SUBCONTRACT', 4, 5.0000
    UNION ALL SELECT '5401.03.05', '施工措施成本', 'MEASURES', 5, 5.0000
    UNION ALL SELECT '5401.03.06', '项目现场管理成本', 'SITE_MANAGEMENT', 6, 3.0000
    UNION ALL SELECT '5401.03.07', '公司管理费分摊', 'OVERHEAD', 7, 5.0000
    UNION ALL SELECT '5401.03.08', '其他专项成本', 'SPECIAL', 8, 1.0000
    UNION ALL SELECT '5401.03.09', '财务及税费成本', 'FINANCE_TAX', 9, 8.0000
    UNION ALL SELECT '5401.03.10', '风险准备成本', 'RISK_RESERVE', 10, 3.0000
) d ON 1 = 1
WHERE p.subject_code = '5401.03'
...
-- V263 removed these standard 5401.04 children globally; preserve the original
-- 5401.04 catalog for non-default tenants as required by the restructure.
INSERT INTO cost_subject
    (id, tenant_id, parent_id, subject_code, subject_name, subject_type, account_category,
     level, sort_order, status, default_target_ratio, created_at, updated_at, deleted_flag)
