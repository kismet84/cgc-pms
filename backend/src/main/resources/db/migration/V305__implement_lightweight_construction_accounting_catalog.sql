ALTER TABLE cost_subject
    ADD COLUMN ledger_flag TINYINT NOT NULL DEFAULT 0 COMMENT '1=固定总账会计科目，0=项目成本/业务分类';
ALTER TABLE cost_subject
...
ALTER TABLE accounting_entry
    ADD COLUMN partner_id BIGINT DEFAULT NULL COMMENT '往来单位辅助核算',
    ADD COLUMN department_id BIGINT DEFAULT NULL COMMENT '部门辅助核算',
    ADD COLUMN employee_id BIGINT DEFAULT NULL COMMENT '员工辅助核算';
...
ALTER TABLE accounting_entry_line
    ADD COLUMN accounting_subject_id BIGINT DEFAULT NULL COMMENT '固定总账科目ID，cost_subject_id继续保存业务成本分类';
...
CREATE TABLE accounting_subject_dimension_rule (
  ...
  PRIMARY KEY (id),
  UNIQUE KEY uk_accounting_subject_dimension_rule (tenant_id,accounting_subject_id),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='固定会计科目辅助核算政策';
...
CREATE TABLE accounting_cost_carryover_mapping (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uk_accounting_cost_carryover_category (tenant_id,category_code),
  UNIQUE KEY uk_accounting_cost_carryover_source (tenant_id,fulfillment_subject_id),
  UNIQUE KEY uk_accounting_cost_carryover_target (tenant_id,expense_subject_id),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='八类合同履约成本到主营业务成本固定映射';
...
CREATE TABLE accounting_cost_carryover (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uk_accounting_cost_carryover_snapshot (tenant_id,project_id,contract_id,carryover_date,balance_hash),
  KEY idx_accounting_cost_carryover_entry (tenant_id,entry_id),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目合同成本结转批次';
...
CREATE TABLE accounting_subject_legacy_review (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uk_accounting_subject_legacy_review (tenant_id,source_subject_id),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='历史总账别名待复核清单';
...
JOIN cost_subject subject
  ON subject.tenant_id=line.tenant_id AND subject.ledger_flag=1 AND subject.deleted_flag=0
...
SET line.accounting_subject_id=subject.id
WHERE line.accounting_subject_id IS NULL AND line.deleted_flag=0;
