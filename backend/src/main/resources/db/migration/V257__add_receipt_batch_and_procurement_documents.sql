ALTER TABLE mat_receipt
    ADD COLUMN system_batch_no varchar(64) NULL,
    ADD COLUMN delivery_note_no varchar(100) NULL,
    ADD COLUMN system_batch_active_token bigint GENERATED ALWAYS AS
      (CASE WHEN deleted_flag = 0 AND system_batch_no IS NOT NULL THEN 0 ELSE id END) STORED;
ALTER TABLE mat_receipt
    ADD UNIQUE KEY uk_mat_receipt_system_batch (tenant_id, system_batch_no, system_batch_active_token),
    ADD KEY idx_mat_receipt_delivery_note (tenant_id, delivery_note_no);
UPDATE mat_receipt SET system_batch_no = CONCAT('MB-LEGACY-', id) WHERE system_batch_no IS NULL;

ALTER TABLE biz_document_template DROP CHECK ck_document_template_business;
ALTER TABLE biz_document_template
    ADD CONSTRAINT ck_document_template_business CHECK (business_type IN ('PAYMENT','SETTLEMENT','PURCHASE_REQUEST','PURCHASE_ORDER','MATERIAL_RECEIPT'));
ALTER TABLE biz_document_default_binding DROP CHECK ck_document_default_business;
ALTER TABLE biz_document_default_binding
    ADD CONSTRAINT ck_document_default_business CHECK (business_type IN ('PAYMENT','SETTLEMENT','PURCHASE_REQUEST','PURCHASE_ORDER','MATERIAL_RECEIPT'));
ALTER TABLE biz_document_generation DROP CHECK ck_document_generation_business;
ALTER TABLE biz_document_generation
    ADD CONSTRAINT ck_document_generation_business CHECK (business_type IN ('PAYMENT','SETTLEMENT','PURCHASE_REQUEST','PURCHASE_ORDER','MATERIAL_RECEIPT'));
...
FROM (SELECT DISTINCT tenant_id FROM sys_dict_type) t
JOIN sys_dict_group g ON g.tenant_id = t.tenant_id AND g.group_code = 'CONTRACT'
...
SELECT 257000200000000 + ROW_NUMBER() OVER (ORDER BY t.tenant_id), t.tenant_id, t.id, '固定单价', 'FIXED', 'primary', 1, 'ENABLE'
FROM sys_dict_type t WHERE t.dict_code='contract_pricing_mode'
  AND NOT EXISTS (SELECT 1 FROM sys_dict_data d WHERE d.dict_type_id=t.id AND d.dict_value='FIXED');
INSERT INTO sys_dict_data (id, tenant_id, dict_type_id, dict_label, dict_value, list_class, order_num, status)
SELECT 257000300000000 + ROW_NUMBER() OVER (ORDER BY t.tenant_id), t.tenant_id, t.id, '据实单价', 'ACTUAL', 'success', 2, 'ENABLE'
FROM sys_dict_type t WHERE t.dict_code='contract_pricing_mode'
