...
ALTER TABLE mat_purchase_request_item
    ...
    ADD COLUMN budget_line_id BIGINT NULL COMMENT '项目预算行ID',
    ADD KEY idx_mat_pri_wbs (tenant_id, wbs_task_id),
    ADD KEY idx_mat_pri_budget (tenant_id, budget_line_id),
...
ALTER TABLE mat_purchase_order_item
    ...
    ADD KEY idx_mat_poi_budget (tenant_id, budget_line_id),
...
CREATE TABLE mat_supplier_return (
    id BIGINT NOT NULL COMMENT '供应商退货ID',
    tenant_id BIGINT NOT NULL DEFAULT 0 COMMENT '租户ID',
    project_id BIGINT NOT NULL COMMENT '项目ID',
    contract_id BIGINT NULL COMMENT '采购合同ID',
    partner_id BIGINT NOT NULL COMMENT '供应商ID',
    receipt_id BIGINT NOT NULL COMMENT '原验收单ID',
    warehouse_id BIGINT NULL COMMENT '退货出库仓库ID',
    return_code VARCHAR(64) NOT NULL COMMENT '退货单号',
    ...
    total_amount DECIMAL(18,2) NOT NULL DEFAULT 0 COMMENT '退货总金额',
    reason VARCHAR(500) NOT NULL COMMENT '退货原因',
    confirmed_by BIGINT NULL COMMENT '确认人',
    confirmed_at DATETIME NULL COMMENT '确认时间',
    reversed_by BIGINT NULL COMMENT '冲销人',
    reversed_at DATETIME NULL COMMENT '冲销时间',
    reversal_reason VARCHAR(500) NULL COMMENT '冲销原因',
    version INT NOT NULL DEFAULT 0 COMMENT '乐观锁版本',
    created_by BIGINT NULL COMMENT '创建人',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_by BIGINT NULL COMMENT '更新人',
    ...
    UNIQUE KEY uk_supplier_return_idem (tenant_id, idempotency_key),
    UNIQUE KEY uk_supplier_return_tenant_id (tenant_id, id),
    KEY idx_supplier_return_receipt (tenant_id, receipt_id, status),
    CONSTRAINT fk_supplier_return_receipt FOREIGN KEY (tenant_id, receipt_id)
        REFERENCES mat_receipt (tenant_id, id) ON DELETE RESTRICT,
    CONSTRAINT ck_supplier_return_status CHECK (status IN ('CONFIRMED','REVERSED')),
    CONSTRAINT ck_supplier_return_amount CHECK (total_amount >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='供应商退货单';
...
CREATE TABLE mat_supplier_return_item (
    ...
    CONSTRAINT fk_supplier_return_item_head FOREIGN KEY (tenant_id, return_id)
        REFERENCES mat_supplier_return (tenant_id, id) ON DELETE RESTRICT,
