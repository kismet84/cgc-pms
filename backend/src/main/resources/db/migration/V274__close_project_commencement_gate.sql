-- Mainline 69: authoritative owner contract, target-cost provenance and commencement gate.

ALTER TABLE pm_project
    ADD COLUMN owner_contract_id BIGINT NULL COMMENT '项目指定且已批准的业主主合同';

ALTER TABLE pm_project
    ADD KEY idx_pm_project_owner_contract (tenant_id, owner_contract_id),
    ADD CONSTRAINT fk_pm_project_owner_contract FOREIGN KEY (tenant_id, owner_contract_id)
        REFERENCES ct_contract (tenant_id, id) ON DELETE RESTRICT;

ALTER TABLE pm_project
    DROP CHECK ck_pm_project_initiation_basis;

UPDATE pm_project
SET initiation_basis = 'LEGACY_UNCLASSIFIED'
WHERE initiation_basis IS NULL;

ALTER TABLE pm_project
    MODIFY COLUMN initiation_basis VARCHAR(32) NULL,
    ADD CONSTRAINT ck_pm_project_initiation_basis CHECK (
        initiation_basis IN ('BID_AWARD', 'DIRECT_APPROVAL', 'LEGACY_UNCLASSIFIED')
    );

ALTER TABLE cost_target
    ADD COLUMN source_contract_id BIGINT NULL COMMENT '目标成本版本来源主合同';

ALTER TABLE cost_target
    ADD KEY idx_cost_target_source_contract (tenant_id, source_contract_id),
    ADD CONSTRAINT fk_cost_target_source_contract FOREIGN KEY (tenant_id, source_contract_id)
        REFERENCES ct_contract (tenant_id, id) ON DELETE RESTRICT;

CREATE TABLE project_commencement (
    id BIGINT NOT NULL,
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_project_commencement_project (tenant_id, project_id),
    UNIQUE KEY uk_project_commencement_tenant_id (tenant_id, id),
    KEY idx_project_commencement_status (tenant_id, approval_status, deleted_flag),
    KEY idx_project_commencement_instance (approval_instance_id),
    CONSTRAINT fk_project_commencement_project FOREIGN KEY (tenant_id, project_id)
        REFERENCES pm_project (tenant_id, id) ON DELETE RESTRICT,
    CONSTRAINT fk_project_commencement_instance FOREIGN KEY (approval_instance_id)
        REFERENCES wf_instance (id) ON DELETE RESTRICT,
    CONSTRAINT ck_project_commencement_status CHECK (
        approval_status IN ('DRAFT', 'APPROVING', 'APPROVED', 'REJECTED')
    )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目开工准入单';
