...
SELECT s.id, 0, s.code, s.name, s.specification, s.unit, 'ENABLE',
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105驾驶舱真实分布演示物料'
FROM (
...
SELECT s.id, 0, 10001, s.code, 30001,
       s.approval_status, s.status, 1, 1, DATEADD('DAY', -s.age, CURRENT_TIMESTAMP), CURRENT_TIMESTAMP, 0, 'V105采购经理真实分布采购申请'
FROM (
...
SELECT s.id, 0, s.request_id, s.material_id, s.qty, s.unit,
       DATEADD('DAY', s.plan_days, CURRENT_DATE), 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105采购申请摘要来源明细'
FROM (
...
SELECT s.id, 0, 10001, s.request_id, 30001,
       CASE WHEN s.blank_partner = 1 THEN NULL ELSE 20002 END,
       s.code, 'MATERIAL', DATEADD('DAY', -30, CURRENT_DATE),
       DATEADD('DAY', -s.overdue_days, CURRENT_DATE), s.amount,
       'APPROVED', s.order_status, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105采购经理真实分布逾期交货'
FROM (
...
SELECT s.id, 0, s.order_id, 10001, s.material_id, s.material_name,
       'V105逾期交货演示规格', s.unit, s.qty, s.price, s.amount, 0.0000,
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105逾期交货摘要来源明细'
FROM (
...
SELECT s.id, 0, 10001, s.order_id, 30001,
       CASE WHEN s.blank_partner = 1 THEN NULL ELSE 20002 END,
       s.code, DATEADD('DAY', s.day_offset, CURRENT_DATE), 1, 1, s.quality_status, s.amount, s.approval_status,
       0, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105采购/生产经理真实分布收货验收'
FROM (
...
SELECT s.id, 0, s.receipt_id, s.order_item_id, s.material_id, s.qty, s.qualified_qty,
       ...
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105收货验收摘要来源明细'
FROM (
...
SELECT s.id, 0, 10001, 30001,
       CASE WHEN s.blank_partner = 1 THEN NULL ELSE 20002 END,
       s.code, DATEADD('DAY', -s.age, CURRENT_DATE), 1, 1,
       s.approval_status, s.amount, s.stock_out_flag, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105生产经理真实分布领料/待出库'
FROM (
...
SELECT s.id, 0, s.requisition_id, s.material_id, s.qty, s.price, s.amount, s.use_location, s.batch_no,
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105领料用途明细'
FROM (
...
SELECT s.id, 0, 10001, 30001,
       CASE WHEN s.blank_partner = 1 THEN NULL ELSE 20002 END,
       s.code, s.period, DATEADD('DAY', -s.age, CURRENT_DATE), s.reported_amount,
       s.approved_amount, s.deduction_amount, s.net_amount, s.approval_status,
       0, s.status, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105生产经理真实分布分包计量'
FROM (
    SELECT 970000000000005901 AS id, 'SM-DEMO-REAL-001' AS code, FORMATDATETIME(CURRENT_DATE, 'yyyy-MM') AS period, 0 AS age, 90000.00 AS reported_amount, 80000.00 AS approved_amount, 10000.00 AS deduction_amount, 80000.00 AS net_amount, 'APPROVING' AS approval_status, 'APPROVING' AS status, 0 AS blank_partner
    UNION ALL SELECT 970000000000005902, 'SM-DEMO-REAL-002', FORMATDATETIME(CURRENT_DATE, 'yyyy-MM'), 3, 210000.00, 200000.00, 10000.00, 200000.00, 'APPROVED', 'CONFIRMED', 0
    UNION ALL SELECT 970000000000005903, 'SM-DEMO-REAL-003', FORMATDATETIME(CURRENT_DATE, 'yyyy-MM'), 7, 420000.00, 390000.00, 30000.00, 390000.00, 'APPROVED', 'CONFIRMED', 0
    UNION ALL SELECT 970000000000005904, 'SM-DEMO-REAL-004', FORMATDATETIME(DATEADD('MONTH', -1, CURRENT_DATE), 'yyyy-MM'), 15, 680000.00, 620000.00, 60000.00, 620000.00, 'APPROVED', 'CONFIRMED', 1
    UNION ALL SELECT 970000000000005905, 'SM-DEMO-REAL-005', FORMATDATETIME(DATEADD('MONTH', -1, CURRENT_DATE), 'yyyy-MM'), 29, 320000.00, 300000.00, 20000.00, 300000.00, 'DRAFT', 'DRAFT', 0
) s
WHERE EXISTS (SELECT 1 FROM pm_project WHERE id = 10001 AND tenant_id = 0 AND deleted_flag = 0)
  AND NOT EXISTS (
...
SELECT s.id, 0, s.measure_id, s.item_name, s.unit, s.contract_qty, s.current_qty, s.cumulative_qty,
       s.price, s.amount, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105分包计量清单明细'
FROM (
...
SELECT s.id, 0, 1, s.material_id, s.available_qty, 0,
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V105生产经理低库存演示'
FROM (
...
WHERE EXISTS (SELECT 1 FROM mat_warehouse WHERE id = 1 AND tenant_id = 0 AND deleted_flag = 0)
  AND EXISTS (SELECT 1 FROM md_material m WHERE m.id = s.material_id AND m.deleted_flag = 0)
  AND NOT EXISTS (
      SELECT 1 FROM mat_stock st
      WHERE st.warehouse_id = 1 AND st.material_id = s.material_id AND st.deleted_flag = 0
  );
