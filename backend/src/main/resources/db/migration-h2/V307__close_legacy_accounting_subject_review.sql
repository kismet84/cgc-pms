ALTER TABLE fund_account ADD COLUMN accounting_subject_code VARCHAR(64) NULL;
ALTER TABLE fund_account ADD CONSTRAINT ck_fund_account_accounting_subject CHECK (
...
UPDATE fund_account SET accounting_subject_code='1001'
...
SET accounting_subject_id=NULL
WHERE line.deleted_flag=0
  ...
  AND EXISTS (
    ...
    WHERE subject.tenant_id=line.tenant_id AND subject.id=line.accounting_subject_id
      AND subject.subject_code='1002.02'
  );

CREATE LOCAL TEMPORARY TABLE m307_tenant(tenant_id BIGINT PRIMARY KEY);
INSERT INTO m307_tenant VALUES(0);
INSERT INTO m307_tenant SELECT DISTINCT tenant_id FROM sys_role WHERE tenant_id<>0;
...
SELECT 307000000000000000+ROW_NUMBER() OVER(ORDER BY t.tenant_id),t.tenant_id,
 COALESCE((SELECT MIN(m.id) FROM sys_menu m WHERE m.tenant_id=t.tenant_id AND m.deleted_flag=0
  AND m.path='/cost/subject/taxonomy'),0),'复核历史会计科目','BUTTON',NULL,NULL,
 'accounting:subject-review',NULL,96,'ENABLE',0,NULL,NULL,'HISTORICAL-ACCOUNTING-SUBJECT-REVIEW',
 CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,0
FROM m307_tenant t
WHERE NOT EXISTS(SELECT 1 FROM sys_menu m
 WHERE m.tenant_id=t.tenant_id AND m.perms='accounting:subject-review' AND m.deleted_flag=0);

INSERT INTO sys_role_menu(id,tenant_id,role_id,menu_id)
SELECT 307100000000000000+ROW_NUMBER() OVER(ORDER BY r.tenant_id,r.id,m.id),r.tenant_id,r.id,m.id
