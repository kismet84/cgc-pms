ALTER TABLE payment_application_source ADD COLUMN receipt_item_id BIGINT;
CREATE INDEX idx_payment_source_receipt_item ON payment_application_source (tenant_id, receipt_item_id, deleted_flag);
ALTER TABLE payment_application_source ADD CONSTRAINT fk_payment_source_receipt_item FOREIGN KEY (tenant_id, receipt_item_id) REFERENCES mat_receipt_item (tenant_id, id) ON DELETE RESTRICT;
ALTER TABLE payment_application_source DROP CONSTRAINT ck_payment_source_reference;
ALTER TABLE payment_application_source ADD CONSTRAINT ck_payment_source_reference CHECK (
...
SELECT 258000100000000 + ROW_NUMBER() OVER (ORDER BY tenant_id), tenant_id, id, '部分到货', 'PARTIAL_RECEIVED', 'warning', 6, 'ENABLE' FROM sys_dict_type t WHERE dict_code='purchase_order_status'
