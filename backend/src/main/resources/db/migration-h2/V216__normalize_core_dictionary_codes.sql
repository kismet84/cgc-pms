-- H2 counterpart of V216__normalize_core_dictionary_codes.sql.
...
UPDATE sys_dict_data SET dict_label='前期', dict_value='DRAFT', list_class='info', order_num=1 WHERE tenant_id=0 AND id=100101;
UPDATE sys_dict_data SET dict_label='在建', dict_value='ACTIVE', list_class='primary', order_num=2 WHERE tenant_id=0 AND id=100102;
UPDATE sys_dict_data SET dict_label='已归档', dict_value='ARCHIVED', list_class='default', order_num=5 WHERE tenant_id=0 AND id=100103;
UPDATE sys_dict_data SET dict_label='已暂停', dict_value='SUSPENDED', list_class='warning', order_num=3 WHERE tenant_id=0 AND id=100104;
UPDATE sys_dict_data SET dict_label='已关闭', dict_value='CLOSED', list_class='danger', order_num=4 WHERE tenant_id=0 AND id=100105;

UPDATE pm_project SET status=CASE status WHEN 'ONGOING' THEN 'ACTIVE' WHEN 'COMPLETED' THEN 'CLOSED' ELSE status END
WHERE status IN ('ONGOING','COMPLETED');

UPDATE sys_dict_data SET dict_value='CONSTRUCTION', dict_label='施工总承包', order_num=1 WHERE tenant_id=0 AND id=132001;
UPDATE sys_dict_data SET dict_value='PROFESSIONAL_SUBCONTRACT', dict_label='专业分包', order_num=2 WHERE tenant_id=0 AND id=132002;
UPDATE sys_dict_data SET dict_value='LABOR_SUBCONTRACT', dict_label='劳务分包', order_num=3 WHERE tenant_id=0 AND id=132003;
UPDATE sys_dict_data SET dict_value='MATERIAL_PROCUREMENT', dict_label='材料采购', order_num=4 WHERE tenant_id=0 AND id=132004;

UPDATE pm_project SET project_type=CASE project_type
    WHEN '施工总承包' THEN 'CONSTRUCTION' WHEN '房建工程' THEN 'CONSTRUCTION'
    WHEN 'BUILDING' THEN 'CONSTRUCTION' WHEN 'MAIN' THEN 'CONSTRUCTION'
    WHEN 'GENERAL' THEN 'CONSTRUCTION' WHEN 'GENERAL_CONTRACT' THEN 'CONSTRUCTION'
    WHEN '专业分包' THEN 'PROFESSIONAL_SUBCONTRACT' WHEN 'SUB' THEN 'PROFESSIONAL_SUBCONTRACT'
    WHEN 'PROFESSIONAL_SUB' THEN 'PROFESSIONAL_SUBCONTRACT'
    WHEN '劳务分包' THEN 'LABOR_SUBCONTRACT' WHEN 'LABOR' THEN 'LABOR_SUBCONTRACT'
    WHEN 'LABOR_SUB' THEN 'LABOR_SUBCONTRACT'
    WHEN '材料采购' THEN 'MATERIAL_PROCUREMENT' WHEN 'PURCHASE' THEN 'MATERIAL_PROCUREMENT'
    WHEN 'MATERIAL' THEN 'MATERIAL_PROCUREMENT' WHEN 'MATERIAL_PURCHASE' THEN 'MATERIAL_PROCUREMENT'
    ELSE project_type END
...
MERGE INTO sys_dict_type (id,tenant_id,dict_code,dict_name,status) KEY(tenant_id,dict_code)
