-- Generated from the MySQL B215 reference schema for H2 MySQL mode.
...
CREATE TABLE `accounting_entry_line` (
  ...
  CONSTRAINT `chk_entry_line_direction` CHECK ((`direction` in ('DEBIT','CREDIT')))
);
...
CREATE TABLE `bank_receipt` (
  ...
  CONSTRAINT `ck_bank_receipt_allocation_json_194` CHECK (((`allocation_json` is null) or (`allocation_json` IS JSON))),
  CONSTRAINT `ck_bank_receipt_allocation_size` CHECK (((`allocation_json` is null) or (length(`allocation_json`) <= 1048576))),
  CONSTRAINT `ck_bank_receipt_context_194` CHECK (((`direction` = 'OUT') or ((`project_id` is null) and (`contract_id` is null) and (`customer_id` is null) and (`fund_account_id` is null)) or ((`project_id` is not null) and (`contract_id` is not null) and (`customer_id` is not null) and (`fund_account_id` is not null)))),
  CONSTRAINT `ck_bank_receipt_direction_194` CHECK ((`direction` in ('IN','OUT'))),
  CONSTRAINT `ck_bank_receipt_link_194` CHECK ((((`collection_record_id` is null) or (`direction` = 'IN')) and ((`pay_record_id` is null) or (`direction` = 'OUT')))),
  CONSTRAINT `ck_bank_receipt_raw_json` CHECK (((`raw_payload_json` IS JSON) and (length(`raw_payload_json`) <= 1048576))),
  CONSTRAINT `ck_bank_receipt_status_194` CHECK ((`match_status` in ('UNMATCHED','MATCHED','MANUAL_REVIEW','IGNORED')))
);
...
CREATE TABLE `biz_document_default_binding` (
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `business_type` varchar(50) NOT NULL COMMENT 'PAYMENT或SETTLEMENT',
...
CREATE TABLE `biz_document_generation` (
  ...
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `generation_no` varchar(50) NOT NULL COMMENT '人类可读生成编号',
  `business_type` varchar(50) NOT NULL COMMENT 'PAYMENT或SETTLEMENT',
  ...
  `schema_version` varchar(30) NOT NULL COMMENT '数据契约版本快照',
  `source_digest` char(64) NOT NULL COMMENT '规范化源数据SHA-256',
  `output_sha256` char(64) DEFAULT NULL COMMENT '归档PDF SHA-256',
  `renderer_id` varchar(50) NOT NULL COMMENT '渲染器标识',
  `renderer_version` varchar(50) NOT NULL COMMENT '渲染器版本',
  `status` varchar(20) NOT NULL DEFAULT 'PENDING' COMMENT 'PENDING/RENDERING/SUCCEEDED/FAILED',
  `file_id` bigint DEFAULT NULL COMMENT '成功归档的sys_file.id',
  `idempotency_key` varchar(120) NOT NULL COMMENT '租户内生成幂等键',
  `retry_of_generation_id` bigint DEFAULT NULL COMMENT '失败重试来源生成事实ID',
  `failure_code` varchar(80) DEFAULT NULL COMMENT '稳定失败码，不存堆栈',
  ...
  CONSTRAINT `ck_document_generation_completion` CHECK ((((`status` = 'SUCCEEDED') and (`file_id` is not null) and (`output_sha256` is not null) and (`completed_at` is not null) and (`failure_code` is null)) or ((`status` = 'FAILED') and (`file_id` is null) and (`output_sha256` is null) and (`completed_at` is not null) and (`failure_code` is not null)) or ((`status` in ('PENDING','RENDERING')) and (`file_id` is null) and (`output_sha256` is null) and (`completed_at` is null) and (`failure_code` is null)))),
...
CREATE TABLE `biz_document_template` (
  ...
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `template_code` varchar(80) NOT NULL COMMENT '租户内稳定模板编码',
  `template_name` varchar(200) NOT NULL COMMENT '模板名称',
  `business_type` varchar(50) NOT NULL COMMENT 'PAYMENT或SETTLEMENT',
  `engine_type` varchar(20) NOT NULL DEFAULT 'HTML_PDF' COMMENT '受限HTML/CSS转PDF',
  ...
  `deleted_flag` tinyint NOT NULL DEFAULT '0' COMMENT '逻辑删除：0否，1是',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `active_unique_token` bigint GENERATED ALWAYS AS (case when (`deleted_flag` = 0) then 0 else `id` end) COMMENT '活动模板唯一键辅助列',
  PRIMARY KEY (`id`),
...
CREATE TABLE `biz_document_template_version` (
  ...
  `schema_version` varchar(30) NOT NULL COMMENT '数据契约版本',
  `template_content` mediumtext NOT NULL COMMENT '受限HTML/CSS模板正文',
  `content_hash` char(64) NOT NULL COMMENT '模板正文SHA-256',
  ...
  CONSTRAINT `ck_document_template_publish_metadata` CHECK ((((`status` = 'DRAFT') and (`published_by` is null) and (`published_at` is null)) or ((`status` in ('PUBLISHED','DISABLED')) and (`published_by` is not null) and (`published_at` is not null)))),
...
CREATE TABLE `cash_journal_entry` (
  ...
  CONSTRAINT `ck_cash_journal_source_type` CHECK ((`source_type` in ('MANUAL','PAY_RECORD','COLLECTION_RECORD','REVERSAL'))),
...
CREATE TABLE `cost_item` (
  ...
  `deleted_flag` tinyint NOT NULL DEFAULT '0' COMMENT '逻辑删除：0否，1是',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `active_unique_token` bigint GENERATED ALWAYS AS (case when (`deleted_flag` = 0) then 0 else `id` end) COMMENT '活动成本事实唯一键辅助列：活动行=0，删除行=id',
  PRIMARY KEY (`id`),
...
CREATE TABLE `cost_subject` (
  ...
  `active_unique_token` bigint GENERATED ALWAYS AS (case when (`deleted_flag` = 0) then 0 else `id` end) COMMENT '活动行唯一键辅助列：活动行=0，删除行=id',
  PRIMARY KEY (`id`),
...
CREATE TABLE `cost_subject_assignment_rule` (
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'æ›´æ–°æ—¶é—´',
  `remark` varchar(500) DEFAULT NULL COMMENT 'å¤‡æ³¨',
  PRIMARY KEY (`id`),
...
CREATE TABLE `finance_audit_event` (
  ...
  CONSTRAINT `ck_fin_audit_payload_json` CHECK (((`payload_json` IS JSON) and (length(`payload_json`) <= 1048576)))
);
...
CREATE TABLE `finance_import_batch` (
  ...
  CONSTRAINT `ck_fin_import_summary_json` CHECK (((`diff_summary_json` is null) or ((`diff_summary_json` IS JSON) and (length(`diff_summary_json`) <= 1048576))))
);
...
CREATE TABLE `finance_import_row` (
  ...
  CONSTRAINT `ck_fin_import_diff_json` CHECK (((`diff_json` is null) or ((`diff_json` IS JSON) and (length(`diff_json`) <= 1048576)))),
  CONSTRAINT `ck_fin_import_input_json` CHECK (((`input_json` IS JSON) and (length(`input_json`) <= 1048576)))
);
...
CREATE TABLE `finance_integration_endpoint` (
  ...
  CONSTRAINT `ck_fin_endpoint_config_json` CHECK (((`config_json` is null) or ((`config_json` IS JSON) and (length(`config_json`) <= 1048576))))
);
...
CREATE TABLE `finance_integration_message` (
  ...
  CONSTRAINT `ck_fin_message_payload_json` CHECK (((`payload_json` IS JSON) and (length(`payload_json`) <= 1048576))),
  CONSTRAINT `ck_fin_message_response_json` CHECK (((`response_json` is null) or ((`response_json` IS JSON) and (length(`response_json`) <= 1048576))))
);
...
CREATE TABLE `finance_reconciliation_run` (
  ...
  CONSTRAINT `ck_fin_recon_summary_json` CHECK (((`summary_json` is null) or ((`summary_json` IS JSON) and (length(`summary_json`) <= 1048576))))
);
...
CREATE TABLE `invoice_ocr_review` (
  ...
  CONSTRAINT `ck_invoice_ocr_comparison_json` CHECK (((`comparison_json` is null) or ((`comparison_json` IS JSON) and (length(`comparison_json`) <= 1048576)))),
  CONSTRAINT `ck_invoice_ocr_raw_json` CHECK (((`raw_result_json` IS JSON) and (length(`raw_result_json`) <= 1048576)))
);
...
CREATE TABLE `mat_material_return` (
  ...
  CONSTRAINT `ck_material_return_status` CHECK ((`status` in ('CONFIRMED','REVERSED')))
);
...
CREATE TABLE `mat_quality_disposition` (
  ...
  CONSTRAINT `ck_quality_disposition_action` CHECK ((`disposition_action` in ('RETURN_TO_SUPPLIER','REWORK','CONCESSION','SCRAP'))),
...
CREATE TABLE `mat_stock` (
  ...
  `deleted_flag` tinyint NOT NULL DEFAULT '0' COMMENT '逻辑删除：0否，1是',
  `remark` text COMMENT '备注',
  `active_unique_token` bigint GENERATED ALWAYS AS (case when (`deleted_flag` = 0) then 0 else `id` end) COMMENT '库存活动行唯一键辅助列：活动行=0，删除行=id',
  PRIMARY KEY (`id`),
...
CREATE TABLE `mat_stock_transfer` (
  ...
  `idempotency_key` varchar(100) NOT NULL COMMENT '租户内幂等键',
...
CREATE TABLE `payment_application_source` (
  ...
  CONSTRAINT `ck_payment_source_reference` CHECK ((((`source_type` = 'EXPENSE') and (`expense_id` is not null) and (`settlement_id` is null) and (`sub_measure_id` is null) and (`source_ref_id` = `expense_id`)) or ((`source_type` = 'SETTLEMENT') and (`settlement_id` is not null) and (`expense_id` is null) and (`sub_measure_id` is null) and (`source_ref_id` = `settlement_id`)) or ((`source_type` = 'SUB_MEASURE') and (`sub_measure_id` is not null) and (`expense_id` is null) and (`settlement_id` is null) and (`source_ref_id` = `sub_measure_id`)) or ((`source_type` = 'DIRECT') and (`expense_id` is null) and (`settlement_id` is null) and (`sub_measure_id` is null) and (`source_ref_id` = `pay_application_id`))))
);
...
CREATE TABLE `revenue_audit_event` (
  ...
  CONSTRAINT `ck_revenue_audit_payload_json` CHECK (((`payload_json` IS JSON) and (length(`payload_json`) <= 1048576)))
);
...
CREATE TABLE `revenue_import_batch` (
  ...
  CONSTRAINT `ck_revenue_import_summary_json` CHECK (((`diff_summary_json` is null) or ((`diff_summary_json` IS JSON) and (length(`diff_summary_json`) <= 1048576))))
);
...
CREATE TABLE `revenue_import_row` (
  ...
  CONSTRAINT `ck_revenue_import_diff_json` CHECK (((`diff_json` is null) or ((`diff_json` IS JSON) and (length(`diff_json`) <= 1048576)))),
  CONSTRAINT `ck_revenue_import_input_json` CHECK (((`input_json` IS JSON) and (length(`input_json`) <= 1048576)))
);
...
CREATE TABLE `sales_invoice_review` (
  ...
  CONSTRAINT `ck_sales_invoice_comparison_json` CHECK (((`comparison_json` is null) or ((`comparison_json` IS JSON) and (length(`comparison_json`) <= 1048576)))),
  CONSTRAINT `ck_sales_invoice_raw_json` CHECK (((`raw_result_json` IS JSON) and (length(`raw_result_json`) <= 1048576)))
);
...
CREATE TABLE `sp_blacklist_record` (
  ...
  CONSTRAINT `ck_sp_blacklist_action` CHECK ((`action_type` = 'ADD')),
...
CREATE TABLE `sp_supplier_return` (
  ...
  CONSTRAINT `ck_sp_return_status` CHECK ((`status` in ('DRAFT','CONFIRMED','REVERSED')))
);
...
CREATE TABLE `sp_supplier_return_item` (
  ...
  CONSTRAINT `ck_supplier_return_item_source_ref` CHECK ((((`return_source` = 'QUALIFIED') and (`quality_disposition_id` is null) and ((`original_stock_txn_id` is not null) or (`original_cost_item_id` is not null))) or ((`return_source` = 'REJECTED') and (`quality_disposition_id` is not null) and (`original_stock_txn_id` is null) and (`original_cost_item_id` is null))))
);
...
CREATE TABLE `sys_user_preference` (
  ...
  `tenant_id` bigint NOT NULL COMMENT '租户ID',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `preferences` text COMMENT '偏好设置，JSON 格式',
  `created_by` bigint DEFAULT NULL COMMENT '创建人',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_by` bigint DEFAULT NULL COMMENT '更新人',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
...
-- Foreign keys are added after every table and deterministic seed row exist.
ALTER TABLE `account_receivable` ADD CONSTRAINT `fk_receivable_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `account_receivable` ADD CONSTRAINT `fk_receivable_customer` FOREIGN KEY (`customer_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `account_receivable` ADD CONSTRAINT `fk_receivable_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `account_receivable` ADD CONSTRAINT `fk_receivable_settlement` FOREIGN KEY (`settlement_id`) REFERENCES `owner_settlement` (`id`) ON DELETE RESTRICT;
ALTER TABLE `accounting_entry` ADD CONSTRAINT `fk_accounting_entry_original` FOREIGN KEY (`original_entry_id`) REFERENCES `accounting_entry` (`id`) ON DELETE RESTRICT;
ALTER TABLE `accounting_entry` ADD CONSTRAINT `fk_accounting_entry_period` FOREIGN KEY (`period_id`) REFERENCES `finance_period` (`id`) ON DELETE RESTRICT;
...
ALTER TABLE `alert_lifecycle_event` ADD CONSTRAINT `fk_alert_lifecycle_alert` FOREIGN KEY (`alert_id`) REFERENCES `alert_log` (`id`) ON DELETE CASCADE;
ALTER TABLE `approval_routing_rule` ADD CONSTRAINT `fk_approval_routing_template_195` FOREIGN KEY (`tenant_id`, `workflow_template_id`) REFERENCES `wf_template` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_collection_194` FOREIGN KEY (`tenant_id`, `collection_record_id`) REFERENCES `collection_record` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_customer` FOREIGN KEY (`customer_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_endpoint` FOREIGN KEY (`endpoint_id`) REFERENCES `finance_integration_endpoint` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_fund_account` FOREIGN KEY (`fund_account_id`) REFERENCES `fund_account` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_journal` FOREIGN KEY (`cash_journal_id`) REFERENCES `cash_journal_entry` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_pay` FOREIGN KEY (`pay_record_id`) REFERENCES `pay_record` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bank_receipt` ADD CONSTRAINT `fk_bank_receipt_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer` ADD CONSTRAINT `fk_bid_target_transfer_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer` ADD CONSTRAINT `fk_bid_target_transfer_bid` FOREIGN KEY (`bid_cost_id`) REFERENCES `bid_cost` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer` ADD CONSTRAINT `fk_bid_target_transfer_mapping` FOREIGN KEY (`mapping_version_id`) REFERENCES `cost_subject_mapping_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer` ADD CONSTRAINT `fk_bid_target_transfer_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer` ADD CONSTRAINT `fk_bid_target_transfer_reversal` FOREIGN KEY (`reversal_of_id`) REFERENCES `bid_cost_target_transfer` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer` ADD CONSTRAINT `fk_bid_target_transfer_target` FOREIGN KEY (`target_id`) REFERENCES `cost_target` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer_line` ADD CONSTRAINT `fk_bid_target_transfer_line_cost` FOREIGN KEY (`source_cost_item_id`) REFERENCES `cost_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer_line` ADD CONSTRAINT `fk_bid_target_transfer_line_header` FOREIGN KEY (`transfer_id`) REFERENCES `bid_cost_target_transfer` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer_line` ADD CONSTRAINT `fk_bid_target_transfer_line_source_subject` FOREIGN KEY (`source_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `bid_cost_target_transfer_line` ADD CONSTRAINT `fk_bid_target_transfer_line_target_subject` FOREIGN KEY (`target_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `biz_document_default_binding` ADD CONSTRAINT `fk_document_default_template` FOREIGN KEY (`tenant_id`, `template_id`) REFERENCES `biz_document_template` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `biz_document_default_binding` ADD CONSTRAINT `fk_document_default_version` FOREIGN KEY (`tenant_id`, `template_version_id`, `template_id`) REFERENCES `biz_document_template_version` (`tenant_id`, `id`, `template_id`) ON DELETE RESTRICT;
ALTER TABLE `biz_document_generation` ADD CONSTRAINT `fk_document_generation_file` FOREIGN KEY (`file_id`) REFERENCES `sys_file` (`id`) ON DELETE RESTRICT;
ALTER TABLE `biz_document_generation` ADD CONSTRAINT `fk_document_generation_retry` FOREIGN KEY (`retry_of_generation_id`) REFERENCES `biz_document_generation` (`id`) ON DELETE RESTRICT;
ALTER TABLE `biz_document_generation` ADD CONSTRAINT `fk_document_generation_version` FOREIGN KEY (`tenant_id`, `template_version_id`, `template_id`) REFERENCES `biz_document_template_version` (`tenant_id`, `id`, `template_id`) ON DELETE RESTRICT;
ALTER TABLE `biz_document_template_version` ADD CONSTRAINT `fk_document_template_version_template` FOREIGN KEY (`tenant_id`, `template_id`) REFERENCES `biz_document_template` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `budget_ledger` ADD CONSTRAINT `fk_budget_ledger_budget` FOREIGN KEY (`budget_id`) REFERENCES `project_budget` (`id`) ON DELETE RESTRICT;
ALTER TABLE `budget_ledger` ADD CONSTRAINT `fk_budget_ledger_line` FOREIGN KEY (`budget_line_id`) REFERENCES `project_budget_line` (`id`) ON DELETE RESTRICT;
ALTER TABLE `budget_ledger` ADD CONSTRAINT `fk_budget_ledger_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `budget_operation` ADD CONSTRAINT `fk_budget_operation_allocation` FOREIGN KEY (`contract_allocation_id`) REFERENCES `contract_budget_allocation` (`id`) ON DELETE RESTRICT;
ALTER TABLE `budget_operation` ADD CONSTRAINT `fk_budget_operation_from_line` FOREIGN KEY (`from_budget_line_id`) REFERENCES `project_budget_line` (`id`) ON DELETE RESTRICT;
ALTER TABLE `budget_operation` ADD CONSTRAINT `fk_budget_operation_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `budget_operation` ADD CONSTRAINT `fk_budget_operation_to_line` FOREIGN KEY (`to_budget_line_id`) REFERENCES `project_budget_line` (`id`) ON DELETE RESTRICT;
ALTER TABLE `business_matter_registry` ADD CONSTRAINT `fk_business_matter_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `business_matter_registry` ADD CONSTRAINT `fk_business_matter_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cash_forecast` ADD CONSTRAINT `fk_cash_forecast_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cash_forecast_cycle` ADD CONSTRAINT `fk_cash_forecast_cycle_previous` FOREIGN KEY (`previous_cycle_id`) REFERENCES `cash_forecast_cycle` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cash_forecast_cycle` ADD CONSTRAINT `fk_cash_forecast_cycle_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cash_forecast_line` ADD CONSTRAINT `fk_cash_forecast_line_cycle` FOREIGN KEY (`cycle_id`) REFERENCES `cash_forecast_cycle` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cash_funding_action` ADD CONSTRAINT `fk_cash_funding_action_cycle` FOREIGN KEY (`cycle_id`) REFERENCES `cash_forecast_cycle` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cash_funding_action` ADD CONSTRAINT `fk_cash_funding_action_line` FOREIGN KEY (`line_id`) REFERENCES `cash_forecast_line` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cash_funding_action` ADD CONSTRAINT `fk_cash_funding_action_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
...
ALTER TABLE `cash_journal_entry` ADD CONSTRAINT `fk_cash_journal_collection` FOREIGN KEY (`collection_record_id`) REFERENCES `collection_record` (`id`) ON DELETE RESTRICT;
...
ALTER TABLE `closeout_archive_transfer` ADD CONSTRAINT `fk_closeout_archive_closeout` FOREIGN KEY (`closeout_id`) REFERENCES `project_closeout` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_archive_transfer` ADD CONSTRAINT `fk_closeout_archive_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_defect` ADD CONSTRAINT `fk_closeout_defect_closeout` FOREIGN KEY (`closeout_id`) REFERENCES `project_closeout` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_defect` ADD CONSTRAINT `fk_closeout_defect_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_defect` ADD CONSTRAINT `fk_closeout_defect_warranty` FOREIGN KEY (`warranty_id`) REFERENCES `closeout_warranty` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_final_acceptance` ADD CONSTRAINT `fk_closeout_final_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_final_acceptance` ADD CONSTRAINT `fk_closeout_final_closeout` FOREIGN KEY (`closeout_id`) REFERENCES `project_closeout` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_final_acceptance` ADD CONSTRAINT `fk_closeout_final_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_section_acceptance` ADD CONSTRAINT `fk_closeout_section_closeout` FOREIGN KEY (`closeout_id`) REFERENCES `project_closeout` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_section_acceptance` ADD CONSTRAINT `fk_closeout_section_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_section_acceptance` ADD CONSTRAINT `fk_closeout_section_quality` FOREIGN KEY (`quality_inspection_id`) REFERENCES `qs_inspection_record` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_section_acceptance` ADD CONSTRAINT `fk_closeout_section_wbs` FOREIGN KEY (`wbs_task_id`) REFERENCES `project_wbs_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_warranty` ADD CONSTRAINT `fk_closeout_warranty_closeout` FOREIGN KEY (`closeout_id`) REFERENCES `project_closeout` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_warranty` ADD CONSTRAINT `fk_closeout_warranty_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_warranty` ADD CONSTRAINT `fk_closeout_warranty_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `closeout_warranty` ADD CONSTRAINT `fk_closeout_warranty_receivable` FOREIGN KEY (`receivable_id`) REFERENCES `account_receivable` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_allocation` ADD CONSTRAINT `fk_collection_alloc_collection` FOREIGN KEY (`collection_id`) REFERENCES `collection_record` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_allocation` ADD CONSTRAINT `fk_collection_alloc_receivable` FOREIGN KEY (`receivable_id`) REFERENCES `account_receivable` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_forecast` ADD CONSTRAINT `fk_collection_forecast_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_forecast` ADD CONSTRAINT `fk_collection_forecast_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_record` ADD CONSTRAINT `fk_collection_account` FOREIGN KEY (`fund_account_id`) REFERENCES `fund_account` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_record` ADD CONSTRAINT `fk_collection_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_record` ADD CONSTRAINT `fk_collection_customer` FOREIGN KEY (`customer_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_record` ADD CONSTRAINT `fk_collection_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_reversal` ADD CONSTRAINT `fk_collection_reversal_record` FOREIGN KEY (`collection_id`) REFERENCES `collection_record` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_schedule` ADD CONSTRAINT `fk_collection_schedule_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_schedule` ADD CONSTRAINT `fk_collection_schedule_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `collection_schedule` ADD CONSTRAINT `fk_collection_schedule_receivable` FOREIGN KEY (`receivable_id`) REFERENCES `account_receivable` (`id`) ON DELETE RESTRICT;
ALTER TABLE `contract_budget_allocation` ADD CONSTRAINT `fk_contract_budget_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `contract_budget_allocation` ADD CONSTRAINT `fk_contract_budget_line` FOREIGN KEY (`budget_line_id`) REFERENCES `project_budget_line` (`id`) ON DELETE RESTRICT;
ALTER TABLE `contract_budget_allocation` ADD CONSTRAINT `fk_contract_budget_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `contract_revenue` ADD CONSTRAINT `fk_contract_revenue_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `contract_revenue` ADD CONSTRAINT `fk_contract_revenue_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `contract_revenue` ADD CONSTRAINT `fk_contract_revenue_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_corrective_action` ADD CONSTRAINT `fk_cost_corrective_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_corrective_action` ADD CONSTRAINT `fk_cost_corrective_forecast` FOREIGN KEY (`forecast_id`) REFERENCES `cost_forecast` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_corrective_action` ADD CONSTRAINT `fk_cost_corrective_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_forecast` ADD CONSTRAINT `fk_cost_forecast_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_forecast` ADD CONSTRAINT `fk_cost_forecast_target` FOREIGN KEY (`cost_target_id`) REFERENCES `cost_target` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_forecast_item` ADD CONSTRAINT `fk_cost_forecast_item_forecast` FOREIGN KEY (`forecast_id`) REFERENCES `cost_forecast` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_forecast_item` ADD CONSTRAINT `fk_cost_forecast_item_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_forecast_item` ADD CONSTRAINT `fk_cost_forecast_item_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_subject_assignment_rule` ADD CONSTRAINT `fk_cost_subject_rule_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_subject_assignment_rule` ADD CONSTRAINT `fk_cost_subject_rule_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_subject_assignment_rule` ADD CONSTRAINT `fk_cost_subject_rule_version` FOREIGN KEY (`mapping_version_id`) REFERENCES `cost_subject_mapping_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_subject_mapping_item` ADD CONSTRAINT `fk_cost_subject_mapping_source` FOREIGN KEY (`source_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_subject_mapping_item` ADD CONSTRAINT `fk_cost_subject_mapping_target` FOREIGN KEY (`target_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_subject_mapping_item` ADD CONSTRAINT `fk_cost_subject_mapping_version` FOREIGN KEY (`mapping_version_id`) REFERENCES `cost_subject_mapping_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_subject_mapping_version` ADD CONSTRAINT `fk_cost_subject_mapping_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_summary` ADD CONSTRAINT `fk_cost_summary_forecast` FOREIGN KEY (`cost_forecast_id`) REFERENCES `cost_forecast` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_target` ADD CONSTRAINT `fk_cost_target_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_target` ADD CONSTRAINT `fk_cost_target_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_target_item` ADD CONSTRAINT `fk_cost_target_item_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `cost_target_item` ADD CONSTRAINT `fk_cost_target_item_target` FOREIGN KEY (`target_id`) REFERENCES `cost_target` (`id`) ON DELETE RESTRICT;
...
ALTER TABLE `ct_contract_change` ADD CONSTRAINT `fk_ct_change_source_var_order` FOREIGN KEY (`source_var_order_id`) REFERENCES `var_order` (`id`) ON DELETE RESTRICT;
ALTER TABLE `customer_credit_profile` ADD CONSTRAINT `fk_customer_credit_partner` FOREIGN KEY (`customer_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `dashboard_finance_snapshot` ADD CONSTRAINT `fk_dashboard_snapshot_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `expense_application` ADD CONSTRAINT `fk_expense_budget_line` FOREIGN KEY (`budget_line_id`) REFERENCES `project_budget_line` (`id`) ON DELETE RESTRICT;
ALTER TABLE `expense_application` ADD CONSTRAINT `fk_expense_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `expense_application` ADD CONSTRAINT `fk_expense_partner` FOREIGN KEY (`payee_partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `expense_application` ADD CONSTRAINT `fk_expense_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `expense_application` ADD CONSTRAINT `fk_expense_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_account_reconciliation` ADD CONSTRAINT `fk_finance_account_recon_period` FOREIGN KEY (`period_id`) REFERENCES `finance_period` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_alert` ADD CONSTRAINT `fk_finance_alert_log` FOREIGN KEY (`alert_log_id`) REFERENCES `alert_log` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_bank_reconciliation` ADD CONSTRAINT `fk_finance_bank_recon_journal` FOREIGN KEY (`cash_journal_id`) REFERENCES `cash_journal_entry` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_bank_reconciliation` ADD CONSTRAINT `fk_finance_bank_recon_period` FOREIGN KEY (`period_id`) REFERENCES `finance_period` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_bank_reconciliation` ADD CONSTRAINT `fk_finance_bank_recon_receipt` FOREIGN KEY (`bank_receipt_id`) REFERENCES `bank_receipt` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_cost_allocation_batch` ADD CONSTRAINT `fk_finance_cost_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_cost_allocation_batch` ADD CONSTRAINT `fk_finance_cost_reversal` FOREIGN KEY (`reversal_of_id`) REFERENCES `finance_cost_allocation_batch` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_cost_allocation_batch` ADD CONSTRAINT `fk_finance_cost_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_cost_allocation_line` ADD CONSTRAINT `fk_finance_cost_line_batch` FOREIGN KEY (`batch_id`) REFERENCES `finance_cost_allocation_batch` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_cost_allocation_line` ADD CONSTRAINT `fk_finance_cost_line_cost` FOREIGN KEY (`cost_item_id`) REFERENCES `cost_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_cost_allocation_line` ADD CONSTRAINT `fk_finance_cost_line_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_import_batch` ADD CONSTRAINT `fk_finance_import_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_import_row` ADD CONSTRAINT `fk_finance_import_row_batch` FOREIGN KEY (`batch_id`) REFERENCES `finance_import_batch` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_integration_message` ADD CONSTRAINT `fk_finance_message_endpoint` FOREIGN KEY (`endpoint_id`) REFERENCES `finance_integration_endpoint` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_period_check` ADD CONSTRAINT `fk_finance_period_check_period` FOREIGN KEY (`period_id`) REFERENCES `finance_period` (`id`) ON DELETE RESTRICT;
ALTER TABLE `finance_reconciliation_issue` ADD CONSTRAINT `fk_finance_recon_issue_run` FOREIGN KEY (`run_id`) REFERENCES `finance_reconciliation_run` (`id`) ON DELETE RESTRICT;
ALTER TABLE `fund_pool_member` ADD CONSTRAINT `fk_fund_pool_member_account` FOREIGN KEY (`fund_account_id`) REFERENCES `fund_account` (`id`) ON DELETE RESTRICT;
ALTER TABLE `fund_pool_member` ADD CONSTRAINT `fk_fund_pool_member_pool` FOREIGN KEY (`pool_id`) REFERENCES `fund_pool` (`id`) ON DELETE RESTRICT;
ALTER TABLE `fund_pool_transaction` ADD CONSTRAINT `fk_fund_pool_txn_from` FOREIGN KEY (`from_member_id`) REFERENCES `fund_pool_member` (`id`) ON DELETE RESTRICT;
ALTER TABLE `fund_pool_transaction` ADD CONSTRAINT `fk_fund_pool_txn_pool` FOREIGN KEY (`pool_id`) REFERENCES `fund_pool` (`id`) ON DELETE RESTRICT;
ALTER TABLE `fund_pool_transaction` ADD CONSTRAINT `fk_fund_pool_txn_to` FOREIGN KEY (`to_member_id`) REFERENCES `fund_pool_member` (`id`) ON DELETE RESTRICT;
ALTER TABLE `invoice_ocr_review` ADD CONSTRAINT `fk_invoice_ocr_review_invoice` FOREIGN KEY (`invoice_id`) REFERENCES `pay_invoice` (`id`) ON DELETE RESTRICT;
ALTER TABLE `invoice_payment_allocation` ADD CONSTRAINT `fk_invoice_alloc_application` FOREIGN KEY (`pay_application_id`) REFERENCES `pay_application` (`id`) ON DELETE RESTRICT;
ALTER TABLE `invoice_payment_allocation` ADD CONSTRAINT `fk_invoice_alloc_invoice` FOREIGN KEY (`invoice_id`) REFERENCES `pay_invoice` (`id`) ON DELETE RESTRICT;
ALTER TABLE `invoice_payment_allocation` ADD CONSTRAINT `fk_invoice_alloc_record` FOREIGN KEY (`pay_record_id`) REFERENCES `pay_record` (`id`) ON DELETE RESTRICT;
...
ALTER TABLE `mat_purchase_order_item` ADD CONSTRAINT `fk_mat_poi_budget` FOREIGN KEY (`tenant_id`, `budget_line_id`) REFERENCES `project_budget_line` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_purchase_order_item` ADD CONSTRAINT `fk_mat_poi_wbs` FOREIGN KEY (`tenant_id`, `wbs_task_id`) REFERENCES `project_wbs_task` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_purchase_order_item` ADD CONSTRAINT `fk_mpo_item_order` FOREIGN KEY (`order_id`) REFERENCES `mat_purchase_order` (`id`) ON DELETE RESTRICT;
ALTER TABLE `mat_purchase_order_item` ADD CONSTRAINT `fk_mpo_item_request` FOREIGN KEY (`request_item_id`) REFERENCES `mat_purchase_request_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `mat_purchase_request_item` ADD CONSTRAINT `fk_mat_pri_budget` FOREIGN KEY (`tenant_id`, `budget_line_id`) REFERENCES `project_budget_line` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_purchase_request_item` ADD CONSTRAINT `fk_mat_pri_wbs` FOREIGN KEY (`tenant_id`, `wbs_task_id`) REFERENCES `project_wbs_task` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_purchase_request_item` ADD CONSTRAINT `fk_mpr_item_request` FOREIGN KEY (`request_id`) REFERENCES `mat_purchase_request` (`id`) ON DELETE RESTRICT;
ALTER TABLE `mat_purchase_request_item` ADD CONSTRAINT `fk_mpr_item_sub_task` FOREIGN KEY (`sub_task_id`) REFERENCES `sub_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `mat_quality_disposition` ADD CONSTRAINT `fk_quality_disposition_item` FOREIGN KEY (`tenant_id`, `receipt_item_id`) REFERENCES `mat_receipt_item` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_quality_disposition` ADD CONSTRAINT `fk_quality_disposition_receipt` FOREIGN KEY (`tenant_id`, `receipt_id`) REFERENCES `mat_receipt` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_receipt_item` ADD CONSTRAINT `fk_mat_ri_budget` FOREIGN KEY (`tenant_id`, `budget_line_id`) REFERENCES `project_budget_line` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_receipt_item` ADD CONSTRAINT `fk_mat_ri_wbs` FOREIGN KEY (`tenant_id`, `wbs_task_id`) REFERENCES `project_wbs_task` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `mat_receipt_item` ADD CONSTRAINT `fk_receipt_item_header` FOREIGN KEY (`receipt_id`) REFERENCES `mat_receipt` (`id`) ON DELETE RESTRICT;
ALTER TABLE `mat_receipt_item` ADD CONSTRAINT `fk_receipt_item_order` FOREIGN KEY (`order_item_id`) REFERENCES `mat_purchase_order_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `mat_stock_transfer` ADD CONSTRAINT `fk_stock_transfer_source_stock` FOREIGN KEY (`source_stock_id`) REFERENCES `mat_stock` (`id`) ON DELETE RESTRICT;
ALTER TABLE `mat_stock_transfer` ADD CONSTRAINT `fk_stock_transfer_target_stock` FOREIGN KEY (`target_stock_id`) REFERENCES `mat_stock` (`id`) ON DELETE RESTRICT;
ALTER TABLE `md_material` ADD CONSTRAINT `fk_md_material_category` FOREIGN KEY (`tenant_id`, `category_id`) REFERENCES `md_material_category` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `md_material_category` ADD CONSTRAINT `fk_material_category_parent` FOREIGN KEY (`tenant_id`, `parent_id`) REFERENCES `md_material_category` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `measurement_period` ADD CONSTRAINT `fk_measurement_period_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `measurement_period` ADD CONSTRAINT `fk_measurement_period_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `org_user_position` ADD CONSTRAINT `fk_org_user_position_position` FOREIGN KEY (`tenant_id`, `position_id`) REFERENCES `org_position` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `org_user_position` ADD CONSTRAINT `fk_org_user_position_user` FOREIGN KEY (`tenant_id`, `user_id`) REFERENCES `sys_user` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `owner_measurement_review_line` ADD CONSTRAINT `fk_owner_review_measure_line` FOREIGN KEY (`measurement_line_id`) REFERENCES `production_measurement_line` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_measurement_review_line` ADD CONSTRAINT `fk_owner_review_submission` FOREIGN KEY (`submission_id`) REFERENCES `owner_measurement_submission` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_measurement_submission` ADD CONSTRAINT `fk_owner_measure_submission_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_measurement_submission` ADD CONSTRAINT `fk_owner_measure_submission_measure` FOREIGN KEY (`measurement_id`) REFERENCES `production_measurement` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_measurement_submission` ADD CONSTRAINT `fk_owner_measure_submission_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_settlement` ADD CONSTRAINT `fk_owner_settlement_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_settlement` ADD CONSTRAINT `fk_owner_settlement_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_settlement` ADD CONSTRAINT `fk_owner_settlement_customer` FOREIGN KEY (`customer_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_settlement` ADD CONSTRAINT `fk_owner_settlement_measurement` FOREIGN KEY (`production_measurement_id`) REFERENCES `production_measurement` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_settlement` ADD CONSTRAINT `fk_owner_settlement_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_settlement` ADD CONSTRAINT `fk_owner_settlement_revenue` FOREIGN KEY (`revenue_id`) REFERENCES `contract_revenue` (`id`) ON DELETE RESTRICT;
ALTER TABLE `owner_settlement` ADD CONSTRAINT `fk_owner_settlement_submission` FOREIGN KEY (`owner_submission_id`) REFERENCES `owner_measurement_submission` (`id`) ON DELETE RESTRICT;
...
ALTER TABLE `payment_application_source` ADD CONSTRAINT `fk_payment_source_application` FOREIGN KEY (`pay_application_id`) REFERENCES `pay_application` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_application_source` ADD CONSTRAINT `fk_payment_source_expense` FOREIGN KEY (`expense_id`) REFERENCES `expense_application` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_application_source` ADD CONSTRAINT `fk_payment_source_settlement` FOREIGN KEY (`settlement_id`) REFERENCES `stl_settlement` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_application_source` ADD CONSTRAINT `fk_payment_source_sub_measure` FOREIGN KEY (`sub_measure_id`) REFERENCES `sub_measure` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_record_source_allocation` ADD CONSTRAINT `fk_pay_alloc_record` FOREIGN KEY (`pay_record_id`) REFERENCES `pay_record` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_record_source_allocation` ADD CONSTRAINT `fk_pay_alloc_source` FOREIGN KEY (`payment_source_id`) REFERENCES `payment_application_source` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_schedule` ADD CONSTRAINT `fk_payment_schedule_application` FOREIGN KEY (`pay_application_id`) REFERENCES `pay_application` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_schedule` ADD CONSTRAINT `fk_payment_schedule_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payment_schedule` ADD CONSTRAINT `fk_payment_schedule_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `production_measurement` ADD CONSTRAINT `fk_production_measure_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `production_measurement` ADD CONSTRAINT `fk_production_measure_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `production_measurement` ADD CONSTRAINT `fk_production_measure_period` FOREIGN KEY (`period_id`) REFERENCES `measurement_period` (`id`) ON DELETE RESTRICT;
ALTER TABLE `production_measurement` ADD CONSTRAINT `fk_production_measure_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `production_measurement_line` ADD CONSTRAINT `fk_production_measure_line_change` FOREIGN KEY (`contract_change_id`) REFERENCES `ct_contract_change` (`id`) ON DELETE RESTRICT;
ALTER TABLE `production_measurement_line` ADD CONSTRAINT `fk_production_measure_line_header` FOREIGN KEY (`measurement_id`) REFERENCES `production_measurement` (`id`) ON DELETE RESTRICT;
ALTER TABLE `production_measurement_line` ADD CONSTRAINT `fk_production_measure_line_item` FOREIGN KEY (`contract_item_id`) REFERENCES `ct_contract_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_budget` ADD CONSTRAINT `fk_project_budget_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_budget_line` ADD CONSTRAINT `fk_budget_line_budget` FOREIGN KEY (`budget_id`) REFERENCES `project_budget` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_budget_line` ADD CONSTRAINT `fk_budget_line_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_budget_line` ADD CONSTRAINT `fk_budget_line_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_closeout` ADD CONSTRAINT `fk_project_closeout_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_closeout` ADD CONSTRAINT `fk_project_closeout_settlement` FOREIGN KEY (`final_owner_settlement_id`) REFERENCES `owner_settlement` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_corrective_action` ADD CONSTRAINT `fk_project_corrective_alert` FOREIGN KEY (`alert_id`) REFERENCES `alert_log` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_corrective_action` ADD CONSTRAINT `fk_project_corrective_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_corrective_action` ADD CONSTRAINT `fk_project_corrective_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_corrective_action` ADD CONSTRAINT `fk_project_corrective_revision` FOREIGN KEY (`generated_revision_plan_id`) REFERENCES `project_schedule_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_corrective_action` ADD CONSTRAINT `fk_project_corrective_schedule` FOREIGN KEY (`schedule_plan_id`) REFERENCES `project_schedule_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_corrective_action` ADD CONSTRAINT `fk_project_corrective_snapshot` FOREIGN KEY (`snapshot_id`) REFERENCES `project_progress_snapshot` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_cost_subject_scope` ADD CONSTRAINT `fk_project_cost_subject_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_cost_subject_scope` ADD CONSTRAINT `fk_project_cost_subject_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_period_plan` ADD CONSTRAINT `fk_project_period_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_period_plan` ADD CONSTRAINT `fk_project_period_parent` FOREIGN KEY (`parent_period_plan_id`) REFERENCES `project_period_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_period_plan` ADD CONSTRAINT `fk_project_period_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_period_plan` ADD CONSTRAINT `fk_project_period_schedule` FOREIGN KEY (`schedule_plan_id`) REFERENCES `project_schedule_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_period_plan_item` ADD CONSTRAINT `fk_project_period_item_plan` FOREIGN KEY (`period_plan_id`) REFERENCES `project_period_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_period_plan_item` ADD CONSTRAINT `fk_project_period_item_task` FOREIGN KEY (`wbs_task_id`) REFERENCES `project_wbs_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_progress_snapshot` ADD CONSTRAINT `fk_project_snapshot_log` FOREIGN KEY (`source_daily_log_id`) REFERENCES `site_daily_log` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_progress_snapshot` ADD CONSTRAINT `fk_project_snapshot_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_progress_snapshot` ADD CONSTRAINT `fk_project_snapshot_schedule` FOREIGN KEY (`schedule_plan_id`) REFERENCES `project_schedule_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_schedule_plan` ADD CONSTRAINT `fk_project_schedule_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_schedule_plan` ADD CONSTRAINT `fk_project_schedule_corrective` FOREIGN KEY (`corrective_action_id`) REFERENCES `project_corrective_action` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_schedule_plan` ADD CONSTRAINT `fk_project_schedule_parent` FOREIGN KEY (`parent_plan_id`) REFERENCES `project_schedule_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_schedule_plan` ADD CONSTRAINT `fk_project_schedule_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_wbs_task` ADD CONSTRAINT `fk_project_wbs_parent` FOREIGN KEY (`parent_task_id`) REFERENCES `project_wbs_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_wbs_task` ADD CONSTRAINT `fk_project_wbs_predecessor` FOREIGN KEY (`predecessor_task_id`) REFERENCES `project_wbs_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_wbs_task` ADD CONSTRAINT `fk_project_wbs_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `project_wbs_task` ADD CONSTRAINT `fk_project_wbs_schedule` FOREIGN KEY (`schedule_plan_id`) REFERENCES `project_schedule_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_consequence` ADD CONSTRAINT `fk_qs_consequence_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_consequence` ADD CONSTRAINT `fk_qs_consequence_cost` FOREIGN KEY (`cost_item_id`) REFERENCES `cost_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_consequence` ADD CONSTRAINT `fk_qs_consequence_evaluation` FOREIGN KEY (`evaluation_id`) REFERENCES `qs_partner_evaluation` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_consequence` ADD CONSTRAINT `fk_qs_consequence_issue` FOREIGN KEY (`issue_id`) REFERENCES `qs_issue` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_consequence` ADD CONSTRAINT `fk_qs_consequence_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_consequence` ADD CONSTRAINT `fk_qs_consequence_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_consequence` ADD CONSTRAINT `fk_qs_consequence_subject` FOREIGN KEY (`cost_subject_id`) REFERENCES `cost_subject` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_inspection_plan` ADD CONSTRAINT `fk_qs_plan_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_inspection_record` ADD CONSTRAINT `fk_qs_inspection_plan` FOREIGN KEY (`plan_id`) REFERENCES `qs_inspection_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_inspection_record` ADD CONSTRAINT `fk_qs_inspection_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_issue` ADD CONSTRAINT `fk_qs_issue_inspection` FOREIGN KEY (`inspection_id`) REFERENCES `qs_inspection_record` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_issue` ADD CONSTRAINT `fk_qs_issue_partner` FOREIGN KEY (`responsible_partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_issue` ADD CONSTRAINT `fk_qs_issue_plan` FOREIGN KEY (`plan_id`) REFERENCES `qs_inspection_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_issue` ADD CONSTRAINT `fk_qs_issue_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_partner_evaluation` ADD CONSTRAINT `fk_qs_eval_consequence` FOREIGN KEY (`consequence_id`) REFERENCES `qs_consequence` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_partner_evaluation` ADD CONSTRAINT `fk_qs_eval_issue` FOREIGN KEY (`issue_id`) REFERENCES `qs_issue` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_partner_evaluation` ADD CONSTRAINT `fk_qs_eval_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_partner_evaluation` ADD CONSTRAINT `fk_qs_eval_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_rectification` ADD CONSTRAINT `fk_qs_rectification_issue` FOREIGN KEY (`issue_id`) REFERENCES `qs_issue` (`id`) ON DELETE RESTRICT;
ALTER TABLE `qs_rectification` ADD CONSTRAINT `fk_qs_rectification_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `receivable_adjustment` ADD CONSTRAINT `fk_receivable_adjustment_ar` FOREIGN KEY (`receivable_id`) REFERENCES `account_receivable` (`id`) ON DELETE RESTRICT;
ALTER TABLE `revenue_dashboard_snapshot` ADD CONSTRAINT `fk_revenue_snapshot_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `revenue_external_sync` ADD CONSTRAINT `fk_revenue_sync_endpoint` FOREIGN KEY (`endpoint_id`) REFERENCES `finance_integration_endpoint` (`id`) ON DELETE RESTRICT;
ALTER TABLE `revenue_external_sync` ADD CONSTRAINT `fk_revenue_sync_message` FOREIGN KEY (`message_id`) REFERENCES `finance_integration_message` (`id`) ON DELETE RESTRICT;
ALTER TABLE `revenue_import_batch` ADD CONSTRAINT `fk_revenue_import_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `revenue_import_row` ADD CONSTRAINT `fk_revenue_import_row_batch` FOREIGN KEY (`batch_id`) REFERENCES `revenue_import_batch` (`id`) ON DELETE RESTRICT;
ALTER TABLE `revenue_reconciliation_issue` ADD CONSTRAINT `fk_revenue_recon_issue_run` FOREIGN KEY (`run_id`) REFERENCES `revenue_reconciliation_run` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sales_invoice` ADD CONSTRAINT `fk_sales_invoice_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sales_invoice` ADD CONSTRAINT `fk_sales_invoice_customer` FOREIGN KEY (`customer_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sales_invoice` ADD CONSTRAINT `fk_sales_invoice_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sales_invoice_allocation` ADD CONSTRAINT `fk_sales_alloc_invoice` FOREIGN KEY (`invoice_id`) REFERENCES `sales_invoice` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sales_invoice_allocation` ADD CONSTRAINT `fk_sales_alloc_receivable` FOREIGN KEY (`receivable_id`) REFERENCES `account_receivable` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sales_invoice_review` ADD CONSTRAINT `fk_sales_invoice_review_invoice` FOREIGN KEY (`invoice_id`) REFERENCES `sales_invoice` (`id`) ON DELETE RESTRICT;
ALTER TABLE `settlement_sub_measure` ADD CONSTRAINT `fk_settlement_measure_measure` FOREIGN KEY (`sub_measure_id`) REFERENCES `sub_measure` (`id`) ON DELETE RESTRICT;
ALTER TABLE `settlement_sub_measure` ADD CONSTRAINT `fk_settlement_measure_settlement` FOREIGN KEY (`settlement_id`) REFERENCES `stl_settlement` (`id`) ON DELETE RESTRICT;
ALTER TABLE `site_daily_progress` ADD CONSTRAINT `fk_site_daily_progress_log` FOREIGN KEY (`daily_log_id`) REFERENCES `site_daily_log` (`id`) ON DELETE RESTRICT;
ALTER TABLE `site_daily_progress` ADD CONSTRAINT `fk_site_daily_progress_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `site_daily_progress` ADD CONSTRAINT `fk_site_daily_progress_schedule` FOREIGN KEY (`schedule_plan_id`) REFERENCES `project_schedule_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `site_daily_progress` ADD CONSTRAINT `fk_site_daily_progress_task` FOREIGN KEY (`wbs_task_id`) REFERENCES `project_wbs_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `site_daily_progress` ADD CONSTRAINT `fk_site_daily_progress_week` FOREIGN KEY (`weekly_plan_id`) REFERENCES `project_period_plan` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_bid_evaluation` ADD CONSTRAINT `fk_sp_bid_event` FOREIGN KEY (`sourcing_event_id`) REFERENCES `sp_sourcing_event` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_bid_evaluation` ADD CONSTRAINT `fk_sp_bid_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_bid_evaluation` ADD CONSTRAINT `fk_sp_bid_quote` FOREIGN KEY (`quote_id`) REFERENCES `sp_supplier_quote` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_blacklist_record` ADD CONSTRAINT `fk_sp_blacklist_evaluation` FOREIGN KEY (`performance_evaluation_id`) REFERENCES `sp_performance_evaluation` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_blacklist_record` ADD CONSTRAINT `fk_sp_blacklist_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_blacklist_record` ADD CONSTRAINT `fk_sp_blacklist_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_performance_evaluation` ADD CONSTRAINT `fk_sp_performance_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_performance_evaluation` ADD CONSTRAINT `fk_sp_performance_order` FOREIGN KEY (`purchase_order_id`) REFERENCES `mat_purchase_order` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_performance_evaluation` ADD CONSTRAINT `fk_sp_performance_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_performance_evaluation` ADD CONSTRAINT `fk_sp_performance_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_sourcing_event` ADD CONSTRAINT `fk_sp_sourcing_award_quote` FOREIGN KEY (`awarded_quote_id`) REFERENCES `sp_supplier_quote` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_sourcing_event` ADD CONSTRAINT `fk_sp_sourcing_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_sourcing_event` ADD CONSTRAINT `fk_sp_sourcing_partner` FOREIGN KEY (`awarded_partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_sourcing_event` ADD CONSTRAINT `fk_sp_sourcing_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_sourcing_event` ADD CONSTRAINT `fk_sp_sourcing_request` FOREIGN KEY (`purchase_request_id`) REFERENCES `mat_purchase_request` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_sourcing_supplier` ADD CONSTRAINT `fk_sp_supplier_event` FOREIGN KEY (`sourcing_event_id`) REFERENCES `sp_sourcing_event` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_sourcing_supplier` ADD CONSTRAINT `fk_sp_supplier_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_quote` ADD CONSTRAINT `fk_sp_quote_event` FOREIGN KEY (`sourcing_event_id`) REFERENCES `sp_sourcing_event` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_quote` ADD CONSTRAINT `fk_sp_quote_invitation` FOREIGN KEY (`sourcing_supplier_id`) REFERENCES `sp_sourcing_supplier` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_quote` ADD CONSTRAINT `fk_sp_quote_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return` ADD CONSTRAINT `fk_sp_return_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return` ADD CONSTRAINT `fk_sp_return_order` FOREIGN KEY (`purchase_order_id`) REFERENCES `mat_purchase_order` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return` ADD CONSTRAINT `fk_sp_return_partner` FOREIGN KEY (`partner_id`) REFERENCES `md_partner` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return` ADD CONSTRAINT `fk_sp_return_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return` ADD CONSTRAINT `fk_sp_return_receipt` FOREIGN KEY (`receipt_id`) REFERENCES `mat_receipt` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return_item` ADD CONSTRAINT `fk_supplier_return_item_cost` FOREIGN KEY (`original_cost_item_id`) REFERENCES `cost_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return_item` ADD CONSTRAINT `fk_supplier_return_item_disposition` FOREIGN KEY (`quality_disposition_id`) REFERENCES `mat_quality_disposition` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return_item` ADD CONSTRAINT `fk_supplier_return_item_head` FOREIGN KEY (`tenant_id`, `return_id`) REFERENCES `sp_supplier_return` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return_item` ADD CONSTRAINT `fk_supplier_return_item_material` FOREIGN KEY (`material_id`) REFERENCES `md_material` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return_item` ADD CONSTRAINT `fk_supplier_return_item_receipt` FOREIGN KEY (`tenant_id`, `receipt_item_id`) REFERENCES `mat_receipt_item` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `sp_supplier_return_item` ADD CONSTRAINT `fk_supplier_return_item_stock` FOREIGN KEY (`original_stock_txn_id`) REFERENCES `mat_stock_txn` (`id`) ON DELETE RESTRICT;
...
ALTER TABLE `sub_task` ADD CONSTRAINT `fk_sub_task_predecessor` FOREIGN KEY (`predecessor_task_id`) REFERENCES `sub_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `sys_role_menu` ADD CONSTRAINT `fk_sys_role_menu_menu` FOREIGN KEY (`tenant_id`, `menu_id`) REFERENCES `sys_menu` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `sys_role_menu` ADD CONSTRAINT `fk_sys_role_menu_role` FOREIGN KEY (`tenant_id`, `role_id`) REFERENCES `sys_role` (`tenant_id`, `id`) ON DELETE CASCADE;
ALTER TABLE `sys_user_role` ADD CONSTRAINT `fk_sys_user_role_role` FOREIGN KEY (`tenant_id`, `role_id`) REFERENCES `sys_role` (`tenant_id`, `id`) ON DELETE RESTRICT;
ALTER TABLE `sys_user_role` ADD CONSTRAINT `fk_sys_user_role_user` FOREIGN KEY (`tenant_id`, `user_id`) REFERENCES `sys_user` (`tenant_id`, `id`) ON DELETE CASCADE;
ALTER TABLE `tech_acceptance_archive` ADD CONSTRAINT `fk_tech_archive_inspection` FOREIGN KEY (`quality_inspection_id`) REFERENCES `qs_inspection_record` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_acceptance_archive` ADD CONSTRAINT `fk_tech_archive_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_acceptance_archive` ADD CONSTRAINT `fk_tech_archive_reference` FOREIGN KEY (`construction_reference_id`) REFERENCES `tech_construction_reference` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_acceptance_archive` ADD CONSTRAINT `fk_tech_archive_version` FOREIGN KEY (`drawing_version_id`) REFERENCES `tech_drawing_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_construction_reference` ADD CONSTRAINT `fk_tech_reference_daily_log` FOREIGN KEY (`daily_log_id`) REFERENCES `site_daily_log` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_construction_reference` ADD CONSTRAINT `fk_tech_reference_disclosure` FOREIGN KEY (`disclosure_id`) REFERENCES `tech_disclosure` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_construction_reference` ADD CONSTRAINT `fk_tech_reference_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_construction_reference` ADD CONSTRAINT `fk_tech_reference_version` FOREIGN KEY (`drawing_version_id`) REFERENCES `tech_drawing_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_construction_reference` ADD CONSTRAINT `fk_tech_reference_wbs` FOREIGN KEY (`wbs_task_id`) REFERENCES `project_wbs_task` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_disclosure` ADD CONSTRAINT `fk_tech_disclosure_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_disclosure` ADD CONSTRAINT `fk_tech_disclosure_scheme` FOREIGN KEY (`scheme_id`) REFERENCES `technical_scheme` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_disclosure` ADD CONSTRAINT `fk_tech_disclosure_version` FOREIGN KEY (`drawing_version_id`) REFERENCES `tech_drawing_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_drawing` ADD CONSTRAINT `fk_tech_drawing_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_drawing_review` ADD CONSTRAINT `fk_tech_drawing_review_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_drawing_review` ADD CONSTRAINT `fk_tech_drawing_review_version` FOREIGN KEY (`drawing_version_id`) REFERENCES `tech_drawing_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_drawing_version` ADD CONSTRAINT `fk_tech_drawing_version_drawing` FOREIGN KEY (`drawing_id`) REFERENCES `tech_drawing` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_drawing_version` ADD CONSTRAINT `fk_tech_drawing_version_previous` FOREIGN KEY (`previous_version_id`) REFERENCES `tech_drawing_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_drawing_version` ADD CONSTRAINT `fk_tech_drawing_version_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_drawing_version` ADD CONSTRAINT `fk_tech_drawing_version_source_rfi` FOREIGN KEY (`source_rfi_id`) REFERENCES `tech_rfi` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_rfi` ADD CONSTRAINT `fk_tech_rfi_drawing_version` FOREIGN KEY (`drawing_version_id`) REFERENCES `tech_drawing_version` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_rfi` ADD CONSTRAINT `fk_tech_rfi_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_rfi` ADD CONSTRAINT `fk_tech_rfi_review` FOREIGN KEY (`review_id`) REFERENCES `tech_drawing_review` (`id`) ON DELETE RESTRICT;
ALTER TABLE `tech_rfi_response` ADD CONSTRAINT `fk_tech_rfi_response_rfi` FOREIGN KEY (`rfi_id`) REFERENCES `tech_rfi` (`id`) ON DELETE RESTRICT;
ALTER TABLE `technical_scheme` ADD CONSTRAINT `fk_technical_scheme_approval` FOREIGN KEY (`approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `technical_scheme` ADD CONSTRAINT `fk_technical_scheme_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `var_order` ADD CONSTRAINT `fk_var_order_generated_change` FOREIGN KEY (`generated_contract_change_id`) REFERENCES `ct_contract_change` (`id`) ON DELETE RESTRICT;
ALTER TABLE `var_order` ADD CONSTRAINT `fk_var_order_internal_approval` FOREIGN KEY (`internal_approval_instance_id`) REFERENCES `wf_instance` (`id`) ON DELETE RESTRICT;
ALTER TABLE `variation_owner_submission` ADD CONSTRAINT `fk_variation_owner_submission_change` FOREIGN KEY (`generated_contract_change_id`) REFERENCES `ct_contract_change` (`id`) ON DELETE RESTRICT;
ALTER TABLE `variation_owner_submission` ADD CONSTRAINT `fk_variation_owner_submission_contract` FOREIGN KEY (`contract_id`) REFERENCES `ct_contract` (`id`) ON DELETE RESTRICT;
ALTER TABLE `variation_owner_submission` ADD CONSTRAINT `fk_variation_owner_submission_order` FOREIGN KEY (`var_order_id`) REFERENCES `var_order` (`id`) ON DELETE RESTRICT;
ALTER TABLE `variation_owner_submission` ADD CONSTRAINT `fk_variation_owner_submission_project` FOREIGN KEY (`project_id`) REFERENCES `pm_project` (`id`) ON DELETE RESTRICT;
ALTER TABLE `variation_owner_submission_item` ADD CONSTRAINT `fk_variation_owner_item_source` FOREIGN KEY (`var_order_item_id`) REFERENCES `var_order_item` (`id`) ON DELETE RESTRICT;
ALTER TABLE `variation_owner_submission_item` ADD CONSTRAINT `fk_variation_owner_item_submission` FOREIGN KEY (`submission_id`) REFERENCES `variation_owner_submission` (`id`) ON DELETE RESTRICT;
