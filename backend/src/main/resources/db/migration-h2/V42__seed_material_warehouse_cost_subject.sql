...
-- 物料字典种子数据 (P2-05)
-- ----------------------------
INSERT INTO md_material (id, tenant_id, material_code, material_name, spec, unit, unit_price, status, created_at, updated_at, deleted_flag)
SELECT 1, 0, 'MAT-001', '螺纹钢 HRB400 Φ12', 'Φ12mm×9m', '吨', 4200.00, 'ENABLE', NOW(), NOW(), 0
WHERE NOT EXISTS (SELECT 1 FROM md_material WHERE id = 1);

INSERT INTO md_material (id, tenant_id, material_code, material_name, spec, unit, unit_price, status, created_at, updated_at, deleted_flag)
SELECT 2, 0, 'MAT-002', '商品混凝土 C30', 'C30', '立方米', 380.00, 'ENABLE', NOW(), NOW(), 0
WHERE NOT EXISTS (SELECT 1 FROM md_material WHERE id = 2);

INSERT INTO md_material (id, tenant_id, material_code, material_name, spec, unit, unit_price, status, created_at, updated_at, deleted_flag)
SELECT 3, 0, 'MAT-003', '水泥 P.O 42.5', 'P.O 42.5', '吨', 350.00, 'ENABLE', NOW(), NOW(), 0
WHERE NOT EXISTS (SELECT 1 FROM md_material WHERE id = 3);

INSERT INTO md_material (id, tenant_id, material_code, material_name, spec, unit, unit_price, status, created_at, updated_at, deleted_flag)
SELECT 4, 0, 'MAT-004', '砂子 中砂', '中砂', '吨', 80.00, 'ENABLE', NOW(), NOW(), 0
WHERE NOT EXISTS (SELECT 1 FROM md_material WHERE id = 4);

INSERT INTO md_material (id, tenant_id, material_code, material_name, spec, unit, unit_price, status, created_at, updated_at, deleted_flag)
SELECT 5, 0, 'MAT-005', '碎石 5-31.5mm', '5-31.5mm', '吨', 90.00, 'ENABLE', NOW(), NOW(), 0
