...
CREATE TABLE mat_purchase_request_item_approval_change (
 id BIGINT NOT NULL, tenant_id BIGINT NOT NULL DEFAULT 0, request_id BIGINT NOT NULL, request_item_id BIGINT NOT NULL,
 workflow_instance_id BIGINT, workflow_task_id BIGINT, old_quantity DECIMAL(18,4) NOT NULL, new_quantity DECIMAL(18,4) NOT NULL,
 change_reason VARCHAR(500) NOT NULL, changed_by BIGINT, changed_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
 PRIMARY KEY (id), CONSTRAINT ck_mpr_item_approval_change_qty CHECK (old_quantity >= 0 AND new_quantity > 0));
