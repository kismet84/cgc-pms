CREATE LOCAL TEMPORARY TABLE m302_tenant(tenant_id BIGINT PRIMARY KEY);
INSERT INTO m302_tenant VALUES(0);
INSERT INTO m302_tenant SELECT DISTINCT tenant_id FROM sys_role WHERE tenant_id<>0;

CREATE LOCAL TEMPORARY TABLE m302_permission(permission_code VARCHAR(200) PRIMARY KEY,menu_name VARCHAR(200));
...
SELECT 302000000000000000+ROW_NUMBER() OVER(ORDER BY t.tenant_id,p.permission_code),t.tenant_id,
 COALESCE((SELECT MIN(m.id) FROM sys_menu m WHERE m.tenant_id=t.tenant_id AND m.deleted_flag=0
  AND m.path='/cost/subject/rules'),0),p.menu_name,'BUTTON',NULL,NULL,p.permission_code,NULL,92,'ENABLE',0,
...
SELECT 302100000000000000+ROW_NUMBER() OVER(ORDER BY r.tenant_id,r.id,m.id),r.tenant_id,r.id,m.id
...
DROP TABLE m302_permission;
DROP TABLE m302_tenant;
