CREATE TABLE project_file_code_scope (
    tenant_id BIGINT NOT NULL PRIMARY KEY,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

MERGE INTO project_file_code_scope(tenant_id)
KEY(tenant_id)
SELECT DISTINCT tenant_id FROM project_file_catalog;

ALTER TABLE project_file_catalog DROP CONSTRAINT uk_project_file_catalog_code;
ALTER TABLE project_file_catalog
    ADD CONSTRAINT uk_project_file_catalog_code UNIQUE (tenant_id, file_code);

CREATE TABLE document_template_code_scope (
    tenant_id BIGINT NOT NULL PRIMARY KEY,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

MERGE INTO document_template_code_scope(tenant_id)
KEY(tenant_id)
SELECT DISTINCT tenant_id FROM biz_document_template;
