...
SELECT s.id, 0, 10001, s.code, 30001,
       'APPROVING', 'DRAFT', 1, 1, DATEADD('DAY', -s.age, CURRENT_TIMESTAMP), CURRENT_TIMESTAMP, 0, '采购经理驾驶舱多行演示申请'
FROM (
...
SELECT s.id, 0, s.request_id, 1, s.qty, '吨',
       DATEADD('DAY', s.plan_days, CURRENT_DATE), 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, '采购经理驾驶舱多行申请明细'
FROM (
...
SELECT s.id, 0, 10001, s.request_id, 30001, 20002, s.code,
       'MATERIAL', DATEADD('DAY', -20, CURRENT_DATE), DATEADD('DAY', -s.overdue_days, CURRENT_DATE),
       s.amount, 'APPROVED', 'APPROVED', 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, '采购经理驾驶舱多行逾期交货演示数据'
FROM (
...
SELECT s.id, 0, s.order_id, 10001, 1, s.material_name,
       ...
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, '采购经理驾驶舱多行逾期订单明细'
FROM (
...
SELECT s.id, 0, 10001, s.order_id, 30001, 20002, s.code,
       DATEADD('DAY', -s.pending_days, CURRENT_DATE), 1, 1, 'PENDING',
       s.amount, 'APPROVING', 0, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, '采购经理驾驶舱多行待验收入库演示数据'
FROM (
...
SELECT s.id, 0, s.receipt_id, s.order_item_id, 1, s.qty, s.qty,
       s.price, s.amount, '采购经理驾驶舱验收排序演示区域', s.batch_no,
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, '采购经理驾驶舱多行待验收明细'
FROM (
