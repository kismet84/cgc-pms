...
-- H2 对等版本。只补 admin(userId=1) 的我发起实例样本。
...
SELECT s.contract_id, 0, 10001, 20001, 20002,
...
SELECT s.request_id, 0, 10001, s.request_code, 30001,
       s.approval_status, s.request_status, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V108审批中心我发起增强采购申请样本'
FROM (
...
SELECT s.item_id, 0, pr.id, 970000000000005001, s.quantity, '批', DATEADD('DAY', s.plan_offset, CURRENT_DATE),
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V108审批中心我发起增强采购申请明细'
FROM (
...
JOIN mat_purchase_request pr
 ...
 AND pr.request_code = s.request_code
 AND pr.deleted_flag = 0
WHERE EXISTS (SELECT 1 FROM md_material WHERE id = 970000000000005001 AND deleted_flag = 0)
  AND NOT EXISTS (
      SELECT 1
...
SELECT s.measure_id, 0, 10001, 30001, 20002,
       s.measure_code, FORMATDATETIME(CURRENT_DATE, 'yyyy-MM'), CURRENT_DATE,
       s.net_amount, s.approved_amount, s.deduction_amount, s.net_amount, s.approval_status,
       0, s.measure_status, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V108审批中心我发起增强分包计量样本'
FROM (
...
SELECT s.item_id, 0, sm.id, s.item_name, '项',
       ...
       1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0, 'V108审批中心我发起增强分包计量明细'
FROM (
...
SELECT s.instance_id, 0, 50001,
       'CONTRACT_APPROVAL', s.contract_id, 10001, s.contract_id,
       s.title, s.amount, s.instance_status,
       1, 0, 1,
       1, NULL, NULL,
       DATEADD('MINUTE', -(s.offset_minute + 5), CURRENT_TIMESTAMP),
       CASE WHEN s.instance_status = 'RUNNING' THEN NULL ELSE DATEADD('MINUTE', -s.offset_minute, CURRENT_TIMESTAMP) END,
       1, DATEADD('MINUTE', -(s.offset_minute + 5), CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -s.offset_minute, CURRENT_TIMESTAMP),
       0, 'V108审批中心我发起增强实例样本'
FROM (
...
WHERE EXISTS (SELECT 1 FROM ct_contract WHERE id = s.contract_id AND deleted_flag = 0)
  AND NOT EXISTS (
      SELECT 1
      FROM wf_instance i
...
SELECT s.instance_id, 0, 50010,
       'PURCHASE_REQUEST', s.request_id, 10001, 30001,
       s.title, CAST(NULL AS DECIMAL(18,2)), s.instance_status,
       1, 0, 1,
       1, NULL, NULL,
       DATEADD('MINUTE', -(s.offset_minute + 5), CURRENT_TIMESTAMP),
       CASE WHEN s.instance_status = 'RUNNING' THEN NULL ELSE DATEADD('MINUTE', -s.offset_minute, CURRENT_TIMESTAMP) END,
...
WHERE EXISTS (SELECT 1 FROM mat_purchase_request WHERE id = s.request_id AND deleted_flag = 0)
  AND NOT EXISTS (
      SELECT 1
      FROM wf_instance i
...
SELECT s.instance_id, 0, 50004,
       'SUB_MEASURE', s.measure_id, 10001, 30001,
       s.title, s.amount, s.instance_status,
       1, 0, 1,
       1, NULL, NULL,
       DATEADD('MINUTE', -(s.offset_minute + 5), CURRENT_TIMESTAMP),
...
WHERE EXISTS (SELECT 1 FROM sub_measure WHERE id = s.measure_id AND deleted_flag = 0)
  AND NOT EXISTS (
      SELECT 1
      FROM wf_instance i
...
SELECT 978000000000001101, 0, 978000000000001001, tn.id,
       ...
       DATEADD('MINUTE', -4, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -4, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -4, CURRENT_TIMESTAMP),
       0, 'V108审批中心我发起合同运行节点'
FROM (
...
SELECT 978000000000002101, 0, 978000000000002001, tn.id,
       ...
       'ACTIVE',
       1, tn.pass_rule_json, tn.reject_rule_json,
       DATEADD('MINUTE', -3, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -3, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -3, CURRENT_TIMESTAMP),
...
SELECT 978000000000002105, 0, 978000000000002005, tn.id,
       ...
       'ACTIVE',
       1, tn.pass_rule_json, tn.reject_rule_json,
       DATEADD('MINUTE', -1, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -1, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -1, CURRENT_TIMESTAMP),
...
SELECT 978000000000003101, 0, 978000000000003001, tn.id,
       ...
       'ACTIVE',
       1, tn.pass_rule_json, tn.reject_rule_json,
       DATEADD('MINUTE', -2, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -2, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -2, CURRENT_TIMESTAMP),
...
SELECT 978000000000001201, 0, 978000000000001001, 978000000000001101,
       'CONTRACT_APPROVAL', 978000000000000101,
       1, '系统管理员', 'PENDING',
       1, 1, DATEADD('MINUTE', -4, CURRENT_TIMESTAMP),
       1, DATEADD('MINUTE', -4, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -4, CURRENT_TIMESTAMP),
       0, 'V108审批中心我发起合同运行待办'
...
SELECT 978000000000002201, 0, 978000000000002001, 978000000000002101,
       'PURCHASE_REQUEST', 978000000000000201,
       1, '系统管理员', 'PENDING',
       1, 1, DATEADD('MINUTE', -3, CURRENT_TIMESTAMP),
       1, DATEADD('MINUTE', -3, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -3, CURRENT_TIMESTAMP),
...
SELECT 978000000000002205, 0, 978000000000002005, 978000000000002105,
       'PURCHASE_REQUEST', 978000000000000205,
       1, '系统管理员', 'PENDING',
       1, 1, DATEADD('MINUTE', -1, CURRENT_TIMESTAMP),
       1, DATEADD('MINUTE', -1, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -1, CURRENT_TIMESTAMP),
...
SELECT 978000000000003201, 0, 978000000000003001, 978000000000003101,
       'SUB_MEASURE', 978000000000000301,
       1, '系统管理员', 'PENDING',
       1, 1, DATEADD('MINUTE', -2, CURRENT_TIMESTAMP),
       1, DATEADD('MINUTE', -2, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -2, CURRENT_TIMESTAMP),
...
SELECT s.record_id, 0, s.instance_id, NULL, NULL, 1,
       ...
       1, DATEADD('MINUTE', -s.offset_minute, CURRENT_TIMESTAMP), 1, DATEADD('MINUTE', -s.offset_minute, CURRENT_TIMESTAMP), 0, 'V108审批中心我发起增强审批记录'
FROM (
...
WHERE EXISTS (SELECT 1 FROM wf_instance WHERE id = s.instance_id AND deleted_flag = 0)
  AND NOT EXISTS (SELECT 1 FROM wf_record WHERE id = s.record_id);
