...
ALTER TABLE sp_supplier_return ADD COLUMN warehouse_id BIGINT;
ALTER TABLE sp_supplier_return ADD COLUMN idempotency_key VARCHAR(128);
ALTER TABLE sp_supplier_return ADD COLUMN reversed_by BIGINT;
ALTER TABLE sp_supplier_return ADD COLUMN reversed_at TIMESTAMP;
ALTER TABLE sp_supplier_return ADD COLUMN reversal_reason VARCHAR(500);
ALTER TABLE sp_supplier_return DROP CONSTRAINT IF EXISTS ck_sp_return_status;
ALTER TABLE sp_supplier_return ADD CONSTRAINT ck_sp_return_status CHECK(status IN ('DRAFT','CONFIRMED','REVERSED'));
CREATE UNIQUE INDEX uk_sp_supplier_return_idem ON sp_supplier_return(tenant_id,idempotency_key);
CREATE UNIQUE INDEX uk_sp_supplier_return_tenant_id ON sp_supplier_return(tenant_id,id);
...
CREATE TABLE sp_supplier_return_item(
 ...
 updated_by BIGINT,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,deleted_flag INT DEFAULT 0 NOT NULL,remark VARCHAR(500),
 FOREIGN KEY(tenant_id,return_id) REFERENCES sp_supplier_return(tenant_id,id),FOREIGN KEY(tenant_id,receipt_item_id) REFERENCES mat_receipt_item(tenant_id,id),
...
CREATE UNIQUE INDEX uk_supplier_return_receipt_line ON sp_supplier_return_item(tenant_id,return_id,receipt_item_id);
CREATE INDEX idx_supplier_return_item_source ON sp_supplier_return_item(tenant_id,receipt_item_id,return_source);
