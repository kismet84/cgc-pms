-- H2 mirror: enable the existing stock -> purchase request flow for PURCHASE_MANAGER.

INSERT INTO sys_menu
    (id, tenant_id, parent_id, menu_name, menu_type, path, component, perms, icon, order_num, status, visible)
SELECT 956, 0, 734, '编辑采购申请', 'BUTTON', NULL, NULL, 'purchase:request:edit', NULL, 3, 'ENABLE', 1
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE id = 956);

INSERT INTO sys_menu
    (id, tenant_id, parent_id, menu_name, menu_type, path, component, perms, icon, order_num, status, visible)
SELECT 957, 0, 734, '删除采购申请', 'BUTTON', NULL, NULL, 'purchase:request:delete', NULL, 4, 'ENABLE', 1
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE id = 957);
