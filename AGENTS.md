...
- 未获明确授权，只做分析、计划、评审、验收或只读诊断；“修改、修复、实现、执行”授权目标所需的本地实施和必要验证，不自动授权 commit、push、PR、合并、发布或生产变更。“运行测试”仅授权指定测试及必要测试产物，不自动授权业务修复或数据重置。
- 普通任务不强制进入 Ready；用户指定主线、Backlog 或治理流程时使用对应载体；AutoPilot 连续迭代只处理合格 Ready Issue。
- 只做目标所需最小改动，不扩大业务、权限、环境、生产或 Git 范围；不确定事实标记“待核实”并先检索证据，不得猜测。仅当缺失信息实质影响结果且无法从上下文或只读检查取得时询问用户；授权判断遵守用户级规则及本文件明确批准门。
...
- 改代码前按目标模块定向查询 `docs/codemap/codemap.json` 与 `docs/codemap/codemap.lock`，结合当前调用点回答：谁调用、影响什么、哪些测试覆盖；不默认全文读取地图或扫描无关模块。
- 纯文案、局部样式且不改变接口、模块、路由、数据或权限语义时，可用目标组件与相关测试的直接证据完成这三问，无需因无关地图过期重建。
- 相关地图关系已过期、与源码矛盾或证据不足时，先补当前调用链证据；涉及模块关系或无法确认影响面时，重建并核对 `docs/codemap/codemap.html`、JSON和lock 后再改代码。不得将失败或未核实称为不适用。
- 模块边界、依赖、路由、数据库、队列或主要数据流变化时，同一任务更新地图；获准提交时与代码一同提交。只读审核和规则文档修改不触发无关地图重建。
...
- 主线任务必须依次取得 G0～G5 门禁证据；G0 按当前范围逐项判定适用性，不适用项记录范围与代码依据，适用项必须通过；失败、缺权限或未验证不得改称不适用。缺任一门禁记录只能报告未完成或阻塞，不得以局部测试、页面可访问或容器启动代替正式完成。

## Git

- 除下一条完整生命周期例外及用户级“已授权普通交互式本地实施的新建隔离分支/worktree”例外外，创建/切换分支、commit、push、PR、合并、Tag、Release 和分支删除分别需要用户明确授权；隔离例外不得切换其他活动工作区、覆盖已有分支/目录或绕过 AutoPilot 当次授权。禁止直接推送 `master/main`，禁止绕过保护、强推或改写历史。
...
- 以下正式承接、写回和净变化统计要求适用于已授权的治理实施与正式收口。只读分析、评审以完成约定检查并报告发现项为任务完成标准；报告发现问题不自动授权修复或写入正式载体，完成评审也不代表被评对象通过验收。
- 普通任务的范围外问题应在报告中说明；除非直接影响本轮结果的安全、正确性或验收标准，不阻塞本轮任务完成。需要扩大范围或写入载体时，仍须具备相应授权。
- 已授权的治理实施与正式收口中，每个发现项只能归入：`本轮修复并复验`、`超出范围并正式承接`、`证据不足或无明确价值而关闭`。
- 已授权实施中，与当前目标、验收标准、当前 diff 根因或本轮直接引入风险相关的问题必须本轮处理；超出授权时停止受影响步骤并说明，不得以“非阻塞”带病通过。
- 上述治理任务的超出范围项必须在获准的唯一正式载体中去重写入，具备证据、价值、优先级、延期原因/前置和验收标准；无价值或不可验收建议直接关闭，不制造 backlog。
- 上述正式收口必须统计 `新增后续项`、`关闭后续项`、`后续项净变化`；存在应正式承接而无载体的遗留项时结论为“不通过”。
