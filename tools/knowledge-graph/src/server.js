...
import { z } from "zod";
import { listMcpCalls, recordMcpCall } from "./call-audit.js";
...
const audited = (name, handler) => async (input = {}) => {
  const startedAt = new Date().toISOString();
  const started = Date.now();
  try {
    const response = await handler(input);
    await recordMcpCall(config.runtimeDir, { startedAt, tool: name, status: "SUCCEEDED", durationMs: Date.now() - started });
    return response;
  } catch (error) {
    await recordMcpCall(config.runtimeDir, { startedAt, tool: name, status: "FAILED", durationMs: Date.now() - started }).catch(() => {});
    throw error;
  }
};
const registerTool = (name, definition, handler) => server.registerTool(name, definition, audited(name, handler));

registerTool("kg_status", { description: "Return cgc-pms knowledge graph coverage and last indexing time." }, async () => result(await status(driver, config)));
registerTool("kg_search", {
...
registerTool("kg_list_issues", {
  ...
  inputSchema: {
    ...
    classification: z.enum(ISSUE_CLASSIFICATIONS).optional(),
...
registerTool("kg_record_episode", {
...
registerTool("kg_collection_runs", {
...
registerTool("kg_unresolved_references", {
...
registerTool("kg_call_history", {
  description: "Return recent MCP tool calls with timestamps, outcome, duration, and process id; inputs are never recorded.",
  inputSchema: { limit: z.number().int().min(1).max(200).optional() },
}, async ({ limit }) => result(await listMcpCalls(config.runtimeDir, limit)));
