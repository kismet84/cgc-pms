...
test("current issue register is valid, unique, and preserves the A-01 16+11 invariant", () => {
  const config = loadConfig();
  const { register, sha256 } = loadIssueRegister(config.repoRoot);
  assert.equal(register.schemaVersion, 1);
  assert.match(sha256, /^[a-f0-9]{64}$/);
  assert.equal(register.issues.length, 49);

  const a01Children = register.issues.filter((issue) => issue.parentIssueKey === "A-01");
  assert.equal(a01Children.filter((issue) => issue.status === "OPEN").length, 16);
