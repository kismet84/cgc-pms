...
const client = new Client({ name: "cgc-pms-knowledge-graph-smoke", version: "0.4.0" });
...
try {
  ...
  for (const scope of ["current", "historical", "all"]) {
    const search = await client.callTool({ name: "kg_search", arguments: { query: "risk", limit: 5, scope } });
    if (search.isError) throw new Error(`kg_search(${scope}) returned an MCP error`);
  }
  const issues = await client.callTool({ name: "kg_list_issues", arguments: { view: "summary" } });
  if (issues.isError) throw new Error(`kg_list_issues returned an MCP error: ${issues.content?.[0]?.text ?? "unknown"}`);
