...
| 编号 | 任务 | 优先级 | 负责人 | 前置依赖 | 验收标准 |
|---|---|---|---|---|---|
| ENV-001 | 建立 Git 仓库和目录结构 | P0 | 技术负责人 | 无 | 仓库包含 backend、frontend-admin、database、deploy、docs |
| ENV-002 | 建立分支规范 | P0 | 技术负责人 | ENV-001 | main、develop、feature 分支可用 |
| ENV-003 | 建立本地 Docker Compose | P0 | 后端 / 运维 | ENV-001 | MySQL、Redis、MinIO 可本地启动 |
| ENV-004 | 整理 README | P0 | 技术负责人 | ENV-001 | 新人可按 README 启动项目 |
| ENV-005 | 配置代码格式化规范 | P1 | 前后端负责人 | ENV-001 | 前后端格式化规则生效 |
...
| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| BE-001 | 初始化 Spring Boot 工程 | P0 | ENV-001 | 后端服务可启动 |
| BE-002 | 配置多环境 application.yml | P0 | BE-001 | dev/test/prod 配置分离 |
| BE-003 | 集成 MyBatis-Plus | P0 | BE-001 | 可访问数据库 |
| BE-004 | 集成 Flyway | P0 | DB-001 | 启动时可执行迁移脚本 |
| BE-005 | 统一响应结构 | P0 | BE-001 | 所有接口返回 code/message/data/traceId |
| BE-006 | 全局异常处理 | P0 | BE-001 | 业务异常、参数异常统一返回 |
| BE-007 | 分页对象封装 | P0 | BE-003 | 支持 pageNo/pageSize/total/records |
| BE-008 | Swagger / OpenAPI | P0 | BE-001 | Swagger 页面可访问 |
| BE-009 | 操作日志基础拦截器 | P1 | BE-001 | 可记录关键操作日志 |

### 9.2 登录鉴权与权限

| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| AUTH-001 | 用户登录接口 | P0 | BE-001、DB-001 | 用户可登录并返回 Token |
| AUTH-002 | JWT 校验过滤器 | P0 | AUTH-001 | 未登录请求被拦截 |
| AUTH-003 | 当前用户上下文 | P0 | AUTH-002 | 后端可获取当前用户 ID |
| AUTH-004 | 用户管理 CRUD | P1 | DB-001 | 可新增、编辑、禁用用户 |
| AUTH-005 | 角色管理 CRUD | P1 | DB-001 | 可维护角色 |
| AUTH-006 | 菜单管理 CRUD | P1 | DB-001 | 可维护菜单 |
| AUTH-007 | 用户角色分配 | P1 | AUTH-004、AUTH-005 | 用户可绑定角色 |
| AUTH-008 | 菜单权限返回接口 | P1 | AUTH-006 | 前端可渲染菜单 |
| AUTH-009 | 按钮权限返回接口 | P1 | AUTH-006 | 前端可控制按钮展示 |

### 9.3 项目管理

| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| PM-001 | 项目表实体和 Mapper | P0 | DB-002 | 可访问 pm_project |
| PM-002 | 项目分页查询 | P0 | PM-001 | 支持项目列表分页 |
| PM-003 | 新建项目 | P1 | PM-001 | 可创建项目 |
| PM-004 | 编辑项目 | P1 | PM-001 | 可修改项目信息 |
| PM-005 | 项目详情 | P1 | PM-001 | 可查看项目详情 |
| PM-006 | 项目状态字典 | P1 | DB-005 | 项目状态来自字典 |

### 9.4 合作方管理

| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| PARTNER-001 | 合作方实体和 Mapper | P0 | DB-002 | 可访问 md_partner |
| PARTNER-002 | 合作方分页查询 | P0 | PARTNER-001 | 支持供应商、分包商筛选 |
| PARTNER-003 | 新建合作方 | P1 | PARTNER-001 | 可新增合作方 |
| PARTNER-004 | 编辑合作方 | P1 | PARTNER-001 | 可编辑合作方 |
| PARTNER-005 | 黑名单字段控制 | P1 | PARTNER-001 | 黑名单合作方可被识别 |

### 9.5 合同管理

| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| CT-001 | 合同实体和 Mapper | P0 | DB-002 | 可访问 ct_contract |
| CT-002 | 合同台账分页查询 | P0 | CT-001 | 支持合同编号、名称、类型、状态筛选 |
| CT-003 | 合同详情接口 | P1 | CT-001 | 可查看合同详情 |
| CT-004 | 新建合同基础信息 | P1 | CT-001、PM-001、PARTNER-001 | 可保存合同草稿 |
| CT-005 | 合同清单保存 | P1 | CT-004 | 可保存合同清单 |
| CT-006 | 合同付款条件保存 | P1 | CT-004 | 可保存付款条件 |
| CT-007 | 合同附件关联 | P1 | FILE-001 | 可上传并关联合同附件 |
| CT-008 | 合同提交审批 | P0 | WF-002 | 合同可调用审批引擎提交审批 |
| CT-009 | 合同审批回调 Handler | P0 | WF-010 | 审批通过后合同状态变更 |
| CT-010 | 合同锁定成本生成 | P1 | COST-001 | 合同通过后可生成锁定成本 |
...
| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| DB-001 | 初始化系统权限表 | P0 | ENV-003 | sys_user、sys_role、sys_menu 等表可创建 |
| DB-002 | 初始化项目、合作方、合同表 | P0 | ENV-003 | pm_project、md_partner、ct_contract 可创建 |
| DB-003 | 初始化审批 wf_* 表 | P0 | ENV-003 | wf_instance、wf_task 等表可创建 |
| DB-004 | 初始化 cost_item 表 | P0 | ENV-003 | cost_item 含 source_item_id |
| DB-005 | 初始化字典数据 | P0 | DB-001 | 合同类型、审批状态等字典可用 |
| DB-006 | 初始化测试数据 | P1 | DB-001~DB-005 | 可登录并看到演示项目、合同 |
| DB-007 | 增加核心索引 | P0 | DB-001~DB-004 | 查询字段具备索引 |
| DB-008 | 增加成本来源唯一索引 | P0 | DB-004 | cost_item 防重复生成 |
...
| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| WF-001 | 审批模板表建模 | P0 | DB-003 | 可保存模板和节点 |
| WF-002 | 提交审批接口 | P0 | WF-001 | 可生成实例、节点实例、任务 |
| WF-003 | 我的待办接口 | P0 | WF-002 | 当前审批人可看到待办 |
| WF-004 | 审批详情接口 | P0 | WF-002 | 可返回实例、节点、任务、记录 |
| WF-005 | 同意接口 | P0 | WF-002 | 当前节点可流转下一节点 |
| WF-006 | 驳回接口 | P0 | WF-002 | 实例状态变为 REJECTED |
| WF-007 | 重新提交接口 | P0 | WF-006 | currentRound + 1，生成新任务 |
| WF-008 | 撤回接口 | P0 | WF-002 | 未处理任务取消，记录保留 |
| WF-009 | 转办接口 | P1 | WF-003 | 生成新任务，旧任务转办 |
| WF-010 | 加签接口 | P1 | WF-003 | 支持前加签、后加签、并行加签 |
| WF-011 | 会签规则 | P0 | WF-005 | ALL 通过后节点通过 |
| WF-012 | 或签规则 | P0 | WF-005 | 任一人同意后其他任务取消 |
| WF-013 | availableActions | P0 | WF-004 | 后端返回当前用户可操作按钮 |
| WF-014 | taskVersion 乐观锁 | P0 | WF-005 | 并发审批只能成功一次 |
| WF-015 | idempotencyKey 幂等 | P0 | WF-005 | 重复请求不重复写记录 |
| WF-016 | 审批记录留痕 | P0 | WF-005~WF-010 | 所有动作写入 wf_record |
| WF-017 | WorkflowBusinessHandler 注册机制 | P0 | WF-005 | 业务模块可注册审批回调 |
...
| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| FE-001 | 初始化 Vue 3 + Vite 工程 | P0 | ENV-001 | 前端服务可启动 |
| FE-002 | 集成 Ant Design Vue | P0 | FE-001 | 可使用 Button、Table、Form 等组件 |
| FE-003 | 集成 Pinia | P0 | FE-001 | 可保存用户和系统状态 |
| FE-004 | 集成 Vue Router | P0 | FE-001 | 路由可跳转 |
| FE-005 | Axios 请求封装 | P0 | FE-001 | 支持 Token、错误提示、统一响应 |
| FE-006 | 主布局 Layout | P0 | FE-002、FE-004 | 左侧菜单、顶部栏、内容区可用 |
| FE-007 | 登录页 | P0 | FE-005、AUTH-001 | 可登录并进入后台 |
| FE-008 | 权限菜单渲染 | P1 | AUTH-008 | 根据后端菜单渲染路由 |
...
| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| FE-CT-001 | 合同台账路由和页面骨架 | P0 | FE-006 | 可进入合同台账页面 |
| FE-CT-002 | 筛选区组件 | P0 | FE-CT-001 | 支持合同编号、类型、状态筛选 |
| FE-CT-003 | KPI 卡片组件 | P1 | FE-CT-001 | 显示合同数量、金额等指标 |
| FE-CT-004 | VxeTable 表格组件 | P0 | FE-CT-001 | 支持宽表格展示 |
| FE-CT-005 | 列设置组件 | P1 | FE-CT-004 | 可控制列显示隐藏 |
| FE-CT-006 | 状态 Tag 组件 | P1 | FE-CT-004 | 不同状态显示不同标签 |
| FE-CT-007 | 分页组件 | P0 | FE-CT-004 | 可切换分页 |
| FE-CT-008 | 右侧图表组件 | P2 | FE-CT-001 | 显示合同类型、金额分布 |
| FE-CT-009 | 合同台账接口联调 | P0 | CT-002 | 页面可展示真实合同数据 |
...
| 编号 | 任务 | 优先级 | 依赖 | 验收标准 |
|---|---|---|---|---|
| FILE-001 | MinIO 配置 | P1 | ENV-003 | 后端可连接 MinIO |
| FILE-002 | 文件上传接口 | P1 | FILE-001 | 可上传文件并返回 fileId |
| FILE-003 | 文件下载 / 预览接口 | P1 | FILE-001 | 可访问文件 URL |
| FILE-004 | 业务附件关联表 | P1 | DB-001 | 文件可关联业务单据 |
| FILE-005 | 前端上传组件 | P1 | FILE-002 | 合同页面可上传附件 |
...
7. 合同台账页面可查询真实数据。
8. 合同可新建、保存草稿、提交审批。
9. 审批引擎 POC 跑通顺序、会签、或签、驳回、撤回、重提。
10. availableActions 由后端返回。
11. taskVersion 和 idempotencyKey 验证通过。
12. WorkflowBusinessHandler 可支持合同审批回调。
13. 成本生成幂等机制设计完成并具备基础实现。
```
