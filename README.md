...
当前治理基线：第93条主线已完成项目成员 API 审计字段与同根写入边界整改、本地 G0～G5 验证和受保护实现合并；以[第93条计划](docs/plans/第93条主线-项目成员API审计字段防篡改与同根写入边界整改任务计划书.md)及[质量报告](docs/quality/2026-08-13-issue-093-项目成员API审计字段与写入边界整改.md)为事实载体，不代表生产发布。

## 核心能力范围

错误监控为可选接入：后端通过 `SENTRY_DSN`、前端构建通过 `VITE_SENTRY_DSN` 启用；空值默认禁用。环境与版本使用对应 `SENTRY_ENVIRONMENT` / `SENTRY_RELEASE`（前端加 `VITE_` 前缀）。禁止将实际凭据写入仓库；接入不代表任何非本地环境已部署。治理规则的授权边界、门禁适用性与通知开关范围见仓库规则及[长任务门禁手册](docs/manuals/codex-long-task-gate.md)。

Sentry错误事件包含原始异常正文与堆栈。关闭默认PII及HTTP Body采集不等于对任意异常文本自动脱敏；启用前应确认接收方与数据策略，业务异常不得拼入密码、令牌等秘密。现有 `/client-errors` 仍只接收来源、类型和加盐指纹；前端Sentry捕获统一经过该上报器的去重与限流，不另启用SDK自动回调捕获。
...
- 日型三位业务编号统一经 `CodeGenerationService` 生成：同租户、同日按 `001..999` 递增，软删除记录继续占号，异常历史或容量耗尽时失败关闭
- 资金账户显式绑定 `1001` 或 `1002.01/1002.02/1002.03` 正式总账科目；历史银行账户不再默认归入一般户，须逐户分类后才能生成新凭证
- 管理层驾驶舱经营健康度采用 270° 弧形仪表盘，项目利润排名支持图表/列表切换；目标成本草稿始终由服务端复核项目经理系统角色
- 显式 `long-task-gate` 的 Completion Contract 检查失败后从首项全量复验；仅终态通知恢复不重跑已通过检查
- CI 保留功能分支完整门禁与同 SHA PR 复用；供应链复扫使用不可变 action SHA；临时构建制品使用短保留期，Trivy 不写分支级数据库缓存，checkout 凭据不跨步骤持久化
- 原子备份批次先完整采集 MySQL/MinIO 工具版本，再用 shell 内建提取首行；避免 `pipefail` 下早退管道把已成功备份误判为失败，并由原子性契约与真实恢复演练验真
