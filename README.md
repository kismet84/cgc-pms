...
- **合同台账列设置统一**: 合同台账页的列设置收敛为通用 `ColumnSettingsButton`，列可见性改由 `useColumnSettings` 统一管理，默认显示策略和本地持久化逻辑保持一致。
- **系统数据清空防误操作**: 系统管理的数据清空页补充了高风险提示区、确认文案和结果提示，降低误删业务数据的风险。
- **台账页交互对齐**: 合同台账页面继续对齐 `lg-*` 工作台规范，表格列、分析侧栏和移动端/桌面端交互保持一致。
