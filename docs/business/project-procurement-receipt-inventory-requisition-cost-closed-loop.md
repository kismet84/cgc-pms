...
历史实施状态（非现行）：P0 核心代码闭环已完成 / 上线数据与 E2E 门禁待验
版本：v1.2
历史基线日期：2026-07-16
适用范围：建筑工程总承包项目的材料需求、采购、到货验收、库存、领退料与材料成本管理
历史事实基线：`codex/procurement-closed-loop-p0` 分支源码、数据库迁移与自动化测试

> 本文是本闭环后续产品设计、数据库设计、接口开发、审批配置、测试和验收的唯一业务标准。本文中的“目标要求”不代表当前源码已经实现；第 2 章和第 12 章只记录历史实施事实，不是当前状态源。
