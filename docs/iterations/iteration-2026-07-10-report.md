...
Issue：READY_SPLIT 剩余 2 个实施型额度拆单

目标：
- 在本轮 `8/10` 后 Ready 队列清空的前提下，从长期总任务池按 current focus 补拆最多 2 条合格 Ready。
- 只更新 backlog、current focus、iteration 与 AutoPilot state；拆单轮不修改业务代码、不计入实施型完成数。

修改范围摘要：
- `docs/backlog/ready-issues.md`：新增 `ISSUE-008-030` 与 `ISSUE-008-031`。
- `docs/backlog/current-focus.md`：同步新增 Ready、串行顺序、未选范围和 READY_SPLIT 计数口径。
- `.codex-autopilot/state.json`：记录 `READY_SPLIT` 并刷新 `lastHeartbeatAt`；`iterationCompleted=8`、`remainingIterations=2` 保持不变。
...
未选范围及原因：
- 规则治理中心规则设计器 / 执行日志 / 效果分析：需要新增模型或更大范围规则平台裁决，本轮不选。
- WBS 完整 schedule、任务依赖、拖拽排程：需要新增 schedule 表或更大前端交互，本轮不选。
- 供应商询价 / 比价 / 定标 / 黑名单 / 补货建议：需要新增业务模型、migration 或采购业务口径裁决，本轮不选。
- P3、财务生产集成、BIM、AI、总工程师技术域：仍属冻结或生产前置范围，本轮不选。

验证命令摘要：
- `git branch --show-current`：`master`。
- `git status --short`：确认存在既有 `AGENTS.override.md` 脏改动；本轮未触碰、未暂存。
- `git diff --check`：通过。

失败分类或非失败分类：非失败分类；本轮为 READY_SPLIT backlog 正式拆题，不涉及实现验收
是否自动合并：否
是否推送：否
结论：通过
阻塞：无
...
Issue：ISSUE-008-032 后端业务接口与前端入口治理
...
Issue：ISSUE-008-034 供应商交期表现摘要与采购订单下钻

- 收口：通过，无阻塞；计入新的 `启动迭代-3` 第 `3/3` 条，达到上限后停止下一任务派发。
- 实现：供应商摘要有 `partnerId` 时通过现有 link button 下钻 `/purchase/order?partnerId=<id>`，无 ID 时保持文本降级；采购订单页复用既有 query 恢复与同步链路，生产代码 no-op。
- 验收：TDD 首次 RED `1/5`；D 验收 2 个目标测试文件共 `24/24`，type-check、build、diff-check 均 exit 0；E 审查 PASS。
- 权限边界：采购订单路由继续受既有 `purchase:order:query` 守卫保护。
- 剩余风险：未执行浏览器视觉验收与真实低权限账号点击 E2E，属非阻塞后续。
- 范围声明：只完成采购订单交期履约摘要与下钻，不代表完整供应商履约档案、综合评级或供应商平台已完成。
