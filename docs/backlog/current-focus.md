...
## 2026-07-16 增量：V155 H2 迁移镜像阻塞修复

- `ISSUE-040-051` 首轮后端服务测试已完成失败分类：MySQL V155 已进入实现，但项目测试 profile 只加载 `migration-h2`，对应 V155 缺失导致 `default_lead_days` 列不存在。
- 已派发 Ready 的允许路径不含 H2 迁移，冻结契约不得事后扩容；当前实现保存在专用分支，先以 `ISSUE-047-002` 独立闭环双数据库迁移镜像与 H2 测试隔离，再基于新 master 恢复产品任务。
- 该阻塞修复只增加同版本、同列语义的 MySQL/H2 Flyway 迁移与完整性断言，不借机修改合作方或采购业务逻辑。
