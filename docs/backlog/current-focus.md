...
## 2026-07-17 增量：现场日报当日质量安全检查只读联动进入 Ready

- `ISSUE-048-006` 复用现有已提交质量安全检查和问题单，在日报详情只读展示检查编号、地点、结论及问题计数，不复制质量安全写模型。
- 独立端点同时要求日报查询与质量安全查询，并从日报服务端反查租户、项目和日期；只计 `SUBMITTED` 检查，问题查询再次限定租户、项目和检查ID。
- 本轮无迁移、无质量安全写入、不展示问题描述/责任人/合作方/金额/整改内容；完成后关闭唯一叶子，A-03其余人员设备、自动天气、定位离线和统计继续保留。
