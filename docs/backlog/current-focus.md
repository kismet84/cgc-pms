...
## 2026-08-13 第94条主线：全仓代码职责拆分与重复实现收敛已规划

- 目标：在不改变路由、API、权限、租户、金额、状态机、事务和数据库结果的前提下，分批拆解多路由页面、God Service、重复政策源与文件服务循环，并删除有证据的透明包装和等价重复。
- 状态：`AUTHORIZED / G0_IN_PROGRESS / GIT_DELIVERY_AUTHORIZED`；唯一状态与验收载体为[`第94条计划`](../plans/第94条主线-全仓代码职责拆分与重复实现收敛任务计划书.md)。
- 基线：编制时为干净 `master@09b7e59bf72c`，与 `origin/master` 一致；现有 Codemap lock 落后当前 HEAD，实施前必须在隔离工作树重生成并回答文件级三问。
