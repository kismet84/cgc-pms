...
## 2026-07-16 增量：投标成本标记未中标入口已选定

- `ISSUE-040-040` 复用已验收的投标成本页面、V154 `bid:status` 权限和后端失标状态机，只补桌面/移动入口、精确 API 契约与必要回归；浏览器只打开并取消确认，不写业务数据。
- 当前同级候选包括间接费规则写入口、补货负向测试和库存并发测试。投标域虽已连续超过五条 Ready，本条是该域最后一个明确叶子，且无需迁移或新数据模型；完成后立即切换间接费域，不再继续拆投标能力。
- 验收要求聚焦 `bid:status`、BIDDING、认证租户、费用一次核销与失败无部分写入；`A-01-BID-LOST` 在正式收口前继续保留为唯一 OPEN 载体。
