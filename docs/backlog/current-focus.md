...
- M2 两批已完成：A-08、V-05、V-07、V-08 通过；A-01 已接入预警处理报告及会计凭证列表/详情/过账/冲销，重分类发票核验与凭证生成接口，当前剩余24个需补入口、11个需要确认。生产存量文件复扫属于 A-08 发布前置，不得直接批量标记 `CLEAN`。
...
- A-01 当前增量状态：`ISSUE-040-007` 和 `ISSUE-040-012～016` 共处理7个子项；有用户入口224、前端调用无独立页面58、内部4、需补入口24、需要确认11，总数仍为321。
