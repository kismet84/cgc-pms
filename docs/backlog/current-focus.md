...
## 2026-07-17 增量：同项目跨仓库存调拨过账进入 Ready

- `ISSUE-048-007` 把既有跨仓可调拨余量提示闭合为真实过账：同租户、同项目、同物料、不同启用仓库之间原子转移数量与库存价值。
- 独立写端点要求库存编辑与库存事务双权限；调拨事实以同租户幂等键唯一，来源安全库存、项目范围、稳定锁顺序、成对流水和价值守恒均为硬门禁。
- 本轮不开放手工入出库、不做审批/在途/运输/批次/跨项目；完成后关闭唯一叶子，A-02继续承接工作日历和需求预测。
