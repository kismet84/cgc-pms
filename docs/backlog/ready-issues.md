...
补货核验：知识图谱只返回该 P0、OPEN、STILL_APPLICABLE、非阻塞叶子项；当前代码仍有受 `system:menu:edit` 保护的更新接口，但前端 API 与权限清单页没有同方法入口，页面测试还明确断言不存在 updateMenu；Ready、Done、Blocked 均无相同存量标记或同接口任务。
