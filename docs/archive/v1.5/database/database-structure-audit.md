...
> 当前合并结构：181 张业务表、3193 个基础表物理字段、24 个视图字段（合计 3217）、366 个外键列、2 个只读治理视图；Flyway 失败记录 0
> 实施结论：本地实现与升级验收通过；生产发布仍须执行本报告规定的 preflight、备份、审批和变更窗口，不因本地通过而自动获准。

## 2026-07-17 修复实施结论

原审计以下正文保留为 **V180 发现基线**，便于追溯“为什么改”；当前 V210 的完整物理结构以自动生成的
[结构字典](../../../database/generated/schema-dictionary.md)和 [ER 关系](../../../database/generated/schema-relationships.md)为权威派生产物。两份文件均来自当前
临时验收库 `cgc_pms_merge_v210.INFORMATION_SCHEMA`，可通过 `scripts/database/generate-schema-docs.ps1` 重复生成，禁止手工维护副本。
...
|V210 当前基础表物理字段|3193|181 张业务表中的物理列；另有 Flyway 内部表，不计入业务字典|
|V210 当前视图字段|24|来自 2 个只读治理视图；业务表与视图合计 3217 个字段位置|
|V210 当前独立字段名|991|同名字段在不同表中重复出现是关系模型的正常现象；3217 个字段位置并不代表 3217 种字段语义|
|V210 当前业务表|181|全部进入自动结构字典；另有 1 张 Flyway 内部表和 2 个只读治理视图|

因此，答案仍是：**不是，不能认定 2018 个字段全都有业务引用。** 当前合并结构有 3217 个业务字段位置，但只有 991 个独立字段名；其余 2226 个是跨表复用的同名位置，主要承担主外键、租户、状态、金额和审计语义，不能据此直接合并为一列。原 V180 逐字段对账已确认 8 个字段无运行时或现行约束证据、4 个字段只有表级返回证据。V210 已退役 14 个明确无效的 `deleted_token`，同时保留 15 个承担活动行唯一性的 `active_unique_token`；供应商退货复用唯一头表 `sp_supplier_return`，没有再创建重复的 `mat_supplier_return`。`overhead_allocation_record` 的退役仍需生产等价环境确认外部依赖。当前新增字段均有 migration、约束、实体/服务或测试证据，但“结构有效”不等于“必须由业务代码逐字段显式读取”。本报告不伪造 3217 个字段位置全部被业务消费的结论。
...
[`docs/archive/v1.5/quality/database-model-remediation-acceptance.md`](../quality/database-model-remediation-acceptance.md)。
