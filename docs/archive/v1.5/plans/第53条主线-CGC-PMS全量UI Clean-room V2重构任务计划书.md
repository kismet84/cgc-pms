...
状态：M0～M8 已完成并通过；`ISSUE-053-036～046`全部 Done，路由台账`0/87/0`。第53条主线本地实施链正式收口；未切正式入口、退役 Legacy 或发布生产。M8报告见`docs/archive/v1.5/quality/第53条主线-M8-全量对等切换与回滚演练正式验收报告.md`。
...
| DR-04 | 设计来源 | 已确认 V2 基线是默认视觉源；只有新增视觉方向或现有模式无法覆盖时才补 Stitch 可编辑设计，截图仅作验收参考。 |
...
4. 以已确认 V2 基线为默认视觉真实源；新增视觉方向使用 Stitch 确认后再重建当前可达路由视图。
...
复用现有 V2 信息架构、组件与页面模式的迁移，不重复创建 Stitch 设计。只有新增视觉方向、复杂交互或现有模式无法覆盖时，阶段实施前必须在 Stitch 交付可编辑设计，至少包含：
...
下列是整站能力目录，不是每个阶段的前置大爆炸清单。业务页面只实现本阶段实际使用且公共组件尚未覆盖的最小组件；组件必须先进入设计系统预览和自动门禁，再被业务页引用：
...
- 正式证据见 `docs/archive/v1.5/quality/第53条主线-M0-Clean-room隔离底座验收报告.md`。
...
实施结果：已通过。`ISSUE-053-010～016`完成，十路由台账`68/19/0`；正式证据见`docs/archive/v1.5/quality/第53条主线-M3-项目履约全量退出门验收报告.md`。

### M4：商务合约

范围：合同、变更、投标、目标成本、成本台账、成本核对、动态利润、预算和产值。

实施结果：已通过。`ISSUE-053-017～023`完成，18条商务合约路由全部达到`V2_ACCEPTED`，台账`50/37/0`；正式证据见`docs/archive/v1.5/quality/第53条主线-M4-商务合约全量退出门验收报告.md`。
...
实施结果：已通过。`ISSUE-053-024～029`完成，10条供应链与物资路由全部由真实V2页面或受控重定向承接，台账`40/47/0`；正式证据见`docs/archive/v1.5/quality/第53条主线-M5-供应链与物资全量退出门验收报告.md`。
...
实施结果：2026-07-26 M6按`ISSUE-053-030～035`全部完成；16条路由由真实V2或受控重定向承接，台账`24/63/0`，后端全量2263项、V2全量351项、M6 E2E 12/12、demo与内置浏览器通过。正式证据见`docs/archive/v1.5/quality/第53条主线-M6-分包结算与资金财务全量退出门验收报告.md`；M7未启动。
...
实施结果：2026-07-27 `ISSUE-053-036～042`全部通过；M7全部24条路由完成V2承接，系统管理双门禁、审计只读、用户管理员连续性、改密全会话撤销、数据维护三重门和V234审计权限注册成立，台账`0/87/0`。后端全量2324项、V2 unit 397项、M7 E2E 17/17、全站 migration gate 91/91、H2/MySQL迁移、23阶段Demo和治理收口通过。M8尚未启动，须另行授权。

### M8：全量对等、切换演练和 Legacy 退役门

实施结果：2026-07-27 `ISSUE-053-043～046`全部通过；台账`0/87/0`，Clean-room、路由/导航、类型/Lint/构建/包体、V2 unit 397/397、design 77/77、后端2324项、migration gate 91/91、真实页面E2E 78/78、23阶段Demo和代表性数据对账成立。同一V2根路径镜像完成`V2_ROOT→LEGACY_ROLLBACK→V2_RESTORE`。正式入口、Legacy退役、目标环境和生产操作未授权。
...
4. 在本地 dev 运行态与 demo 数据演练 V2 接管根路径、回滚 Legacy、再次恢复 V2。
5. 生成正式验收报告和上线决策清单。
...
| 视觉 | 已批准视觉源对照、1440/1024/390、全部状态；新增视觉方向附 Stitch 对照 |
...
每阶段正式证据至少包含：迁移台账、命令结果摘要、已批准视觉源标识、视觉 QA、权限与数据风险结论、阻塞/剩余风险和回滚路径；只有本阶段新增视觉方向时才要求 Stitch 标识。
...
- 新增视觉方向缺少关键状态设计，且实现选择会改变用户流程或权限。
...
- 当前分支和工作区状态。
- 上阶段结论为通过。
- 已批准视觉源、功能契约、权限矩阵和验收数据已准备；新增视觉方向另需 Stitch 设计。
...
9. 本地 dev 运行态与 demo 数据整站切换和回滚演练通过。
