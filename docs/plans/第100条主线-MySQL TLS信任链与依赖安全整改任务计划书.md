...
>
> 当前状态（2026-09-06 M6实施）：`G0_G4_PASSED / G5_GIT_DELIVERY_PENDING`；新组合8.4.12/26.7的 `7c14dca1`完整Push CI通过；G4健康UP、真实登录、两条合作方样本列表/详情、API及数据库回读通过。最终证据提交后的同SHA CI、PR合并、post-merge及安全清理仍待完成。最新证据见质量报告M6更新。
...
- 当前状态：G0～G4已通过，M6第8节及质量报告为权威结果。TLS复核缺口、支持组合、跨引擎/原生恢复、完整同SHA CI、SPDX及真实样本浏览器已验证。仅G5最终证据提交后的同SHA CI、受保护合并、post-merge及安全清理待完成；无重复授权或厂商澄清待答复，Issue保持开放。
