...
>
> 当前状态（2026-09-06 M6实施）：`G0_G3_PASSED / G4_ENVIRONMENT_BLOCKED / G5_NOT_READY`；新组合8.4.12/26.7实现 `c2b81a21` 已推送，同SHA CI与pre-PR verifier通过；G4测试桶创建被执行策略拒绝，健康及有样本浏览器未通过，尚未合并。最新证据见质量报告M6更新。
...
- G0 已在隔离工作树确认任务归属；8.4.0 只保留为已测试的临时候选。2026-09-06 独立复核后重新打开 G1：Oracle CPU 只列受影响的受支持版本，旧 8.4.0 未列出不能证明安全；26.7 官方主指南的 MySQL 8.0+ 与兼容性页的 8.4+ 口径仍冲突。F02 未关闭，未升级 MySQL，不把测试通过当作厂商支持证明。
- 当前 Ready 保持 0；本轮来自用户直接授权，不进入 AutoPilot Ready。
- 当前状态：本地 TLS/backend health、真实登录、合同列表 DOM/API 与 console 已补齐；浏览器契约 CI 不替代该真实链路。TLS 复核新增的额外 CA、私钥可读性、smoke 私钥隔离和 hostname 假阳性缺口已本轮修复并聚焦复验。G1 版本安全支持证据阻塞，G3 同 SHA SPDX/CI 与 G5 Git 尚未完成；所有旧证据保留但不能使后续门禁越过 G1。用户需决定是否调整兼容性验收约束；否则等待厂商一致支持证据，禁止静默引入 MySQL 8.4。
