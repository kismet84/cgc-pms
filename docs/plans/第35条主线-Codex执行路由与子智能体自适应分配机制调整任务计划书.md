...
**Goal:** 在不削弱项目安全、验收与 AutoPilot 治理边界的前提下，将 Codex 主线程的执行路由从“实施动作默认必须派工”调整为“通过授权门后，依据任务风险、耦合度、并行收益、上下文传递成本和独立证据需要，自主选择直接执行、单子智能体派工或多子智能体派工”，并形成可静态验收的一致规则。
**Architecture:** 采用最小规则调整方案：根规则定义统一授权门与路由原则，项目 Skill 和派工模板承接操作细则，AutoPilot 规则将 A–F 从固定六角色编制改为职责检查表；不新增运行时框架、不引入新的状态系统、不追溯改写历史交付物，并继续保留 Ready、checkpoint、stop/pause/enabled、no push、测试数据重置、并行上限和失败分类等既有硬边界。
...
- 建立统一授权门，明确哪些动作可由主线程直接执行，哪些必须或应当派工。
...
- 不取消高风险任务的独立证据要求，也不允许以“直接执行”替代必要的复核。
...
- 不包含 Git 提交、push、生产发布、生产数据库连接或运行环境变更。

## 3. 保留的安全边界

以下规则不因自适应路由而削弱：

- 业务或治理变更仍必须满足 Ready Issue 来源与状态要求；无合格 Ready Issue 不写业务代码。
- 所有关键 checkpoint 继续检查 `stop.flag`、`pause.flag`、`enabled.flag`，并保留任务开始前、选题后、改代码前、验证前、自动合并前和报告更新后的检查要求。
- `autoPush=false` 与 no push 边界保持不变；不自动发布生产，不连接生产数据库。
...
| 实时文件 | 修改口径 | 明确保留 |
| --- | --- | --- |
| `AGENTS.override.md` | 在最高优先级项目规则中定义授权门、自适应路由三选一、直接执行适用条件、强制派工条件、超时收回或重派规则 | 总负责人责任、安全硬边界、客观证据要求 |
...
| `plugins/cgc-pms-autopilot/references/owner-boundary.md` | 明确主线程可在授权门后直接承担低风险、低耦合动作；高风险和独立裁决仍优先派工 | 总负责人最终责任、证据与安全边界 |
...
2. 项目规则是否允许该动作，是否命中禁止目录、生产环境、数据重置、Ready、checkpoint 或 no push 限制。
3. 当前分支、工作区和 worktree 是否适合执行；涉及实施型、验收型或运维型派工前仍应完成规定的只读前置检查。
4. 是否存在需要用户新增授权、外部协调或显著扩大范围的情形。

授权门未通过时停止执行并说明阻塞；通过后才进入路由选择。

### 5.2 五项路由因素

主线程综合评估以下因素，不要求建立数值评分系统：
...
- 独立证据需要：结果是否需要独立测试、审查、安全复核或正式裁决依据。

### 5.3 三种执行路由

#### 直接执行

适用于授权门已通过，且任务低风险、低耦合、范围清晰、上下文集中、独立复核收益有限的动作。主线程直接执行时仍需遵守验证和收口要求，但不填写子智能体 `model`、`thinking`、`reason`，也不输出模型分配表。

#### 单子智能体派工

适用于任务需要专业角色隔离、上下文可完整交付、存在独立证据价值，或主线程需要保持裁决独立性的情形。派工必须填写完整模板，包括 `model`、`thinking`、`reason` 和验收输出；单派不要求模型分配表。

#### 多子智能体派工

适用于存在两个及以上真正独立的工作包，或实现、验收、审查之间需要责任隔离的情形。必须先输出子智能体模型分配表，再逐个使用完整派工模板。并行仍受“最多三个完全无关联任务”限制；共享文件、模块、数据库、权限、安全、租户、金额或状态机的任务不得并行。
...
- 若任务从普通实现升级为数据库、权限、安全、金额、租户或正式裁决问题，必须升档或补派适配角色，不得因赶进度直接收回。
...
每轮先判断哪些职责实际存在，再决定由主线程直接承担、单派承担或多派隔离承担。纯文档补货轮可以只覆盖 A/F；单一低风险实现可合并实现与自检，但若涉及独立裁决、高风险边界或正式上线结论，D/E 不得被“已由实现者自测”替代。动态裁剪不得省略 Ready、checkpoint、health gate、失败分类、最终复验、归档和本地收口等适用步骤。
...
**预期结果：** 两个根规则对路由条件无冲突；硬边界关键字仍可定位；`git diff --check` 无输出且退出码为 0。
...
**预期结果：** Skill 与模板对三种路由及派工字段的触发条件一致；导航可找到实时规则；`git diff --check` 通过。
...
**预期结果：** A–F 不再被描述为固定六线程；AutoPilot 硬边界完整保留；三个文件无互相矛盾表述；`git diff --check` 通过。
...
**预期结果：** 第一条冲突扫描无有效命中，若命中历史引用则人工确认其已明确标注为历史或反例；第二条硬边界扫描覆盖各实时规则；`git diff --check` 无输出；`git status --short` 仅包含本主线授权修改，且不包含历史计划、quality、iterations、memory、`.playwright-cli/` 或其他临时产物变更。
...
| A–F 动态化 | A–F 被定义为职责检查表，不是固定六子智能体 | AutoPilot Skill 与 owner boundary 条款 |
| 安全边界 | Ready、checkpoint、flags、no push、数据重置、并行上限、失败分类均保留 | 关键字扫描和人工复核 |
...
- **规则源漂移：** 根规则、Skill、模板和插件引用文档可能只改一部分，产生互相冲突。缓解方式是按九文件矩阵实施并在 M4 做静态一致性扫描。
- **A–F 过度合并：** 动态职责可能被误解为可以省略测试、审查或归档。缓解方式是明确“职责可合并、责任不可遗漏”，高风险与正式裁决继续要求独立证据。
- **超时收回误用：** 为节省等待时间直接接管高风险任务。缓解方式是超时后重新评估风险；高风险升级或重派，不允许直接降格。
- **历史文档混淆：** 历史方案与实时规则并存可能造成阅读误判。缓解方式是在 README 标注实时入口，不追溯篡改历史交付物。

### 9.2 回滚方案

本主线只修改规则和文档，不涉及数据库、服务或生产运行态。若实施后发现执行边界不清或规则冲突，按 M1–M3 的文件组回退对应文档 diff，并恢复原有“实施动作默认派工”规则；回滚前后均执行 `git diff --check` 和九文件静态扫描。不得通过删除历史计划、质量报告或迭代报告完成回滚。
...
- A–F 在 AutoPilot 中明确为职责检查表，适用职责没有被省略。
- Ready、checkpoint、stop/pause/enabled、no push、测试数据重置、最多三个完全无关联任务并行和失败分类完整保留。
...
- M4 全部静态检查通过，`git diff --check` 无输出，无未授权文件变更。
- 本主线不要求提交或 push。

### 10.2 后续派工建议

实施时按阶段重新判断路由，不预设固定子智能体数量：

1. **M1 根规则修改：** 风险较高且影响全项目执行边界，建议单派规则治理子智能体实施，并由主线程独立复核；实际派工时再填写 `model/thinking/reason`。
2. **M2 Skill 与模板：** 与 M1 口径强相关，应在 M1 验收后串行实施；可由一个文档/Skill 子智能体处理，避免并行产生措辞漂移。
3. **M3 AutoPilot：** 涉及连续执行责任边界，建议独立派工并保留 AutoPilot 安全条款逐项核对。
4. **M4 静态验收：** 应与实施责任隔离，由验收型子智能体或主线程在授权门后执行只读检查；若用于正式通过/不通过裁决，必须保留完整客观证据。
