...
**Goal:** 在不复制第二套审批、文件、权限或报表平台的前提下，为 cgc-pms 建立“业务数据快照 → 版本化模板 → 预览/生成 PDF → 文件归档 → 权限与审计”的最小闭环，首批交付付款申请单和结算单，并以金额、租户、项目、审批状态及历史文档不可变为核心验收方向。
**Architecture:** 采用“业务输出模型（Provider）+ 版本化模板 + 受限渲染适配器 + 生成记录 + 现有文件服务”的分层架构；后端在独立 `document` 责任域中编排数据、模板和渲染，复用 `UserContext`、业务对象授权、`FileService`/MinIO 与 `@AuditedOperation`，前端仅提供模板选择、预览、生成和下载入口。MVP 使用受限 HTML/CSS 模板生成 PDF，不建设通用拖拽低代码平台，不直接绑定数据库列，不执行模板脚本，不自动发送外部邮件，不把图片签章视为可信电子签名。

> 计划状态：Draft / 待产品决策门与实施授权
> 计划类型：产品能力、跨模块业务输出、权限与数据一致性
> 适用范围：付款申请、结算、合同/发票后续扩展、报表目录、文件归档、审计与下载
> 非实施声明：本计划书只定义实施边界和验收门禁，不代表已授权修改业务代码、数据库、运行环境、Git 分支、提交、推送或发布。
...
6. 当前工作区正在推进付款、结算、发票、预算和费用闭环，字段和迁移尚未稳定；实施本计划前必须先冻结首批输出契约，不得直接绑定当前未收口 diff。
...
| `FileService` | 存储生成文件、生成受控下载入口、病毒/类型与生命周期治理 | 不绕过业务对象授权 |
...
实施时 migration 编号必须基于当时分支的下一空闲版本分配，并同步提供 MySQL/H2 脚本；当前工作区已有未收口 migration，计划书不预占具体版本号。
...
| `default_version_id` | 当前默认已发布版本，可空 |
| 通用审计字段 | 创建人、更新人、时间、逻辑删除等 |

唯一约束至少覆盖 `tenant_id + template_code + deleted_flag`。删除默认使用逻辑删除；已有生成记录引用时禁止物理删除。
...
P0 不承诺“按旧业务数据重新生成完全相同文件”；正式历史证据是已归档 PDF、模板版本、sourceDigest 和生成元数据。若未来需要保存完整源快照，必须另做隐私、容量、加密和保留期评审。
...
stateDiagram-v2
    ...
    RENDERING --> SUCCEEDED
    RENDERING --> FAILED
    FAILED --> PENDING: 新请求/新幂等键
    SUCCEEDED --> [*]
```

- `SUCCEEDED` 记录不可回退或覆盖，只能新建生成记录。
- 同一幂等键只能得到一个最终结果。
- 文件上传成功但记录落库失败必须有补偿清理或可恢复记录，不能留下无归属对象。
...
至少记录：模板创建、校验、发布、停用、默认切换、预览、正式生成、下载和失败。审计中记录 ID、类型、结果和操作者，不记录完整模板变量值、银行账号、身份证号、附件内容或 PDF 字节。
...
下载复用现有文件服务受控入口；不得新建绕过 `FileService` 的匿名 PDF 下载端点。
...
5. 明确文件保留期、模板维护角色、首批租户/角色和打印样式样本。
6. 形成风险审查，特别覆盖许可证、模板注入、SSRF/本地文件读取、租户与业务授权。
...
| 层级 | 必测内容 |
| --- | --- |
| 单元测试 | 字段目录、格式化、循环、条件、模板语法、状态机、sourceDigest、幂等、失败分类 |
| Provider 测试 | 付款/结算字段、空值、长明细、金额口径、脱敏、支持状态、业务授权 |
| Renderer 测试 | 中文、分页、表格跨页、页眉页脚、图片、字体、禁止外部资源、超限失败 |
| Service 集成 | 模板发布不可变、并发默认版本、生成记录、文件补偿、历史稳定 |
| Controller | 参数、权限、租户、项目范围、错误码、响应头、下载审计 |
| 数据库 | MySQL/H2 migration 对齐、唯一约束、软删除、并发幂等、外键/引用边界 |
...
先在 dev/test/demo 开启，验证系统模板和真实角色，再按授权进入后续环境。开关关闭时前端入口隐藏、生成 API fail-close，历史文件仍按权限可下载。
...
1. 当前工作区存在大量付款、结算、发票、预算和费用改动；本计划实施前必须先确认其归属并获得干净、正确的功能分支或隔离 worktree。
...
7. 项目地图、迭代决策、Current Focus/唯一问题载体和正式质量报告完成回写。
8. 本轮所有发现项均已修复复验、正式承接或有依据关闭，无口头悬空项。
