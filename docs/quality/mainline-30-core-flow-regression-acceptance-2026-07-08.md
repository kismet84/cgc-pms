...
1. V134 改为自包含、最小幂等补丁：确保目标项目 `2071032241708793858` 存在，并在该项目下补齐演示仓库、领料单、领料明细和一条 `source_type='MAT_REQUISITION'` 的 `cost_item`。
2. 成本台账等价查询以 `projectId=2071032241708793858`、`sourceType=MAT_REQUISITION` 过滤时，可直接枚举目标项目的领料成本来源。

裁决：`MAT_REQUISITION` 已同时具备代码、测试、驾驶舱 API 和成本台账直接枚举证据，不再作为剩余风险。
...
| 编号 | 风险 | 影响 | 建议 |
| --- | --- | --- | --- |
| R1 | Mockito 动态 agent 未来兼容性警告 | 当前测试退出码为 0，不影响本轮验收；未来 JDK 策略变化可能影响测试运行方式 | 后续构建兼容性任务中按 Mockito 官方建议配置 agent |
| R2 | Spring Boot generated password 提示 | 本地测试/运行态提示，不影响 dev-login 验收和本轮主链路结论 | 后续运行态/安全配置任务中统一收敛开发环境提示 |
| R3 | M3/M4 浏览器验收使用 dev-login 超管账号 | 能证明页面和 API 主链路可达，但不能替代真实采购、生产、财务角色的权限体验复核 | 后续用真实采购、生产、财务账号做一次人工抽样复核 |
...
1. 用真实采购、生产、财务账号抽样复核关键页面和驾驶舱。
2. 在构建兼容性任务中处理 Mockito 动态 agent 未来兼容性提示。
