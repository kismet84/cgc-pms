...
| 候选动作 | 低库存建议与采购申请预填已完成；下一最小候选只配置库存项安全阈值并联动现有链路，供货周期与预测继续后置 |
...
| 采购补货 | 已有采购、收货、库存、领料及低库存到采购申请预填；`ISSUE-037-006` 已支持库存项安全阈值 | PMSmart 强调物资需用/缺口；Odoo 19 提供重订货规则、供货周期和补货报告 | 基础阈值缺口已收口，策略能力仍有差距 | 供货周期、预测和全量建议需另行决策 |
