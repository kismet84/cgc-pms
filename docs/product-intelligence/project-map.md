...
## 2026-07-17 候选：系统用户编辑详情权威加载

- 用户管理已有列表、新增、编辑、状态、删除和角色分配入口；后端详情端点按租户返回用户允许字段与角色映射，但编辑仍直接复制分页行快照。
- 最小闭环是在编辑动作中先按ID加载当前详情，成功后才开窗；失败不回退旧行，快速切换仅接受最后响应，新增动作同时取消详情语义。
- 保持现有管理员/查询权限与租户隔离，详情VO不含密码；不新增页面、数据表、权限码或用户审计域。
