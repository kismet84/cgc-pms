...
## 2026-08-10 主线86：全仓业务闭环权限、状态与事实一致性整改实施中

- 问题源：`ISSUE-086-001`；状态`IN_PROGRESS / G0_PASSED / G1_IN_PROGRESS / G2-G5_PENDING / GIT_DELIVERY_AUTHORIZED`。
- 目标链：`项目对象 → 项目数据范围`；`结算/附件/复核 → 应收/回款/现金/凭证`；`进项发票/付款条款 → 应付确认/付款分配 → 子账与总账月结`；`合同终局事实 → SETTLED → 项目关闭`；`ACTIVE基线/周计划/日报/技术 → 唯一履约事实`。
- 复核分母：15项确认缺陷、2项待裁决能力、10项业务决策；生产/目标环境、通用平台、劳务工资设备和移动/BIM扩面排除。
