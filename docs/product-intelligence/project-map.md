...
## 2026-07-25 主线53 M6：031通过，032 Ready

- M6冻结分包任务/计量/结算6条路由与付款、费用、收入回款、发票、资金运营、日记账、预测、凭证、月结10条路由，共16条目标；031后剩余13条`LEGACY_ONLY`，完成目标台账为`24/63/0`。
- 复用既有后端P0闭环，金额、来源、可付余额、日记账、预测、凭证、期间、审批和冲销均由服务端裁决；V2不新建平行资金账、前端权威计算或手工凭证生成入口。
- `ISSUE-053-031`已将分包根路由、任务和计量三条路由迁移为真实V2或受控重定向；任务/计量CRUD、清单、附件、提交及服务端范围、金额重算、CAS、失败原子性成立。
- 后端专项105项、V2全量单测334项、目标E2E连续两轮10项、静态门和当前fat JAR真实运行态三视口通过；台账达到`37/50/0`。
- `ISSUE-053-032`为当前唯一Ready，033～035保持Planned；正式入口、Legacy退役、Git交付和生产操作仍未授权。
