...
- 后端接口无前端入口治理：`ISSUE-037-019` 静态基线纳入 53 个 Controller、321 个唯一 HTTP 方法；经增量治理后，当前已验收分类为有用户入口 231、前端调用但无独立页面 58、内部/集成/运维 4、需补用户入口 17、待废弃 0、需要确认 11。`ISSUE-040-025` 已关闭角色新建叶子缺口；剩余项仍需按域验收，不是新业务域。
