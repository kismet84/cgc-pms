...
## 2026-07-17 候选：本地开发 MySQL 迁移基线重建

- 当前开发库在旧分支迁移后记录到 V203；当前 master 的 V188～V210 因合并后重新编号，Flyway 校验出现16个校验和错位，并检出 V181、V183、V204～V210 未应用。供应商 V189 三表缺失直接阻断真实运行态验收。
- 直接 `flyway repair` 会把不同业务内容的同版本号错误标成已应用，不能恢复权威基线。最小可靠方案是完整备份并独立恢复验真后，只重建允许重置的本地 `cgc_pms`，执行当前 V1→V210。
- 该候选只恢复开发验收底座，不改变产品能力、业务代码或迁移定义；原开发测试数据保留在回滚备份，不自动混入新基线。
