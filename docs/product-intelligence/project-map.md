...
## 2026-07-17 增量：库存历史净领料基线完成

- 库存页现按当前库存项展示含今日的30/90个本地自然日领料、退料和净领料，窗口起止可见；范围由服务端库存、仓库和项目关系确定，一次查询完成两个窗口。
- 只计 `MAT_REQUISITION/OUT` 与 `MATERIAL_RETURN/IN`，排除调拨、验收、供应商退货及其他流水；空结果归零，窗口内退料超过领料时保留负值。
- 页面明确该结果是历史事实而非需求预测；A-02仍保留工作日历与预测算法决策，不输出日均、缺货日或建议采购量。

## 2026-07-17 增量：同项目跨仓库存调拨过账完成

- 库存页现可从服务端候选直接发起同租户、同项目、同物料、不同启用仓库的即时调拨；V211事实与来源 `OUT`、目标 `IN` 流水在单事务内完成，不开放通用手工出入库。
- 来源和目标库存按ID稳定加锁，提交时重算安全余量；同租户幂等键唯一，锁后强制重查可让恰好耗尽可调拨量的并发重放返回同一结果而不重复过账。
- 权限取库存编辑与库存事务交集；数量和值守恒、跨租户/项目/物料/同仓拒绝、失败回滚和前端失败重试均已验收。A-02现只保留工作日历和需求预测。

## 2026-07-17 增量：现场日报当日质量安全检查只读联动完成

- 日报详情现已聚合当日到货、已审批出库领料、计划/WBS、变更历史和已提交质量安全检查；没有复制质量安全字段到日报主表。
- 独立只读端点取日报查询与质量安全查询权限交集，服务端从日报反查租户、项目和日期，并再次校验项目数据范围；任一单权限不能访问。
- 只展示检查编号、地点、结论和问题总数/高风险数/未关闭数；问题正文、责任主体、合作方、附件、金额和整改详情不进入日报。

## 2026-07-17 增量：本地开发 MySQL 迁移基线重建完成

- 原开发库旧分支编号错位已通过完整备份、独立恢复读回和本地 `cgc_pms` 重建消除；没有使用 `flyway repair` 伪造迁移状态。
- 当前权威基线由 master 执行207条迁移到 V210，失败0，Flyway validate和MySQL smoke通过；供应商三表、backend健康、开发登录及供应商只读页面均可用。
- 原开发测试数据保留在本地忽略目录的完整备份中用于回滚，一次性恢复容器已精确删除；产品能力、业务代码、迁移文件、编排和密钥未改变。
