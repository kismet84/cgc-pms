...
## 2026-08-28 主线99：每日审计并发一致性、供应链扫描与依赖安全整改已合并并完成 post-merge

- 状态：`COMPLETED / G0-G5_PASSED / ISSUE_CLOSED / GIT_DELIVERY_MERGED / REUSED_PUSH_CI / POST_MERGE_VERIFIED`；实现 PR #461 已合并为 `2e4a37118337`，Push/PR CI 与 post-merge verifier 通过。唯一载体为[`第99条计划`](../plans/第99条主线-每日审计并发一致性供应链扫描与依赖安全整改任务计划书.md)与[`质量报告`](../quality/2026-08-28-issue-099-每日审计并发一致性供应链扫描与依赖安全整改.md)。
...
- 边界：仅本地 dev/test/demo 与受保护 Git 交付；不实施 Boot 4/前端大版本升级，不建设第二扫描平台，不连接或规划生产/目标环境；版本发布未授权。
...
- 验证：后端 2735、前端 568、MySQL `1/24/1`、自有 4173 当前工作树 contract 98、9 组 live 80、技术/收尾真实浏览器、JaCoCo `79.827%/61.570%`、真实 MySQL+MinIO 恢复及引用/hash 守恒、workflow required job=16 与 Codemap 通过；push `31599845219`、PR `31600592597`、post-merge `31601538284` 全绿。
...
## 2026-08-10 主线86：全仓业务闭环权限、状态与事实一致性整改已合并

- 问题源：`ISSUE-086-001`已关闭；状态`IMPLEMENTED / G0-G5_PASSED / GIT_DELIVERY_MERGED / POST_MERGE_VERIFIED`。
- 目标链：`项目对象 → 项目数据范围`；`结算/附件/复核 → 应收/回款/现金/凭证`；`进项发票/付款条款 → 应付确认/付款分配 → 子账与总账月结`；`合同终局事实 → SETTLED → 项目关闭`；`ACTIVE基线/周计划/日报/技术 → 唯一履约事实`。
- 复核分母：15项确认缺陷、2项条件能力、10项业务决策均已收口；生产/目标环境、通用平台、劳务工资设备和移动/BIM扩面排除。
- 最小方案：复用现有授权判定、领域服务、正式供应商退货、文件中心、计划修订、会计分录、领域审计、技术状态和关闭门；不创建平行业务事实或第二财务台账。
- 证据：后端2620项、前端510项、H2/MySQL V291、备份恢复、真实V2与本地Live 80/80通过；源 HEAD `21a15ecf51cb09025c17692bfb5f7cfe9e14e644` 的 Push CI `31370852523`、PR #426 CI `31371513029`、合并 SHA `d1eea9b6897ac7df8ad67fcd8c67a7ba99c92907` 和 post-merge run `31372388330` 均通过，源分支已清理。
- 计划：[`第86条主线`](../plans/第86条主线-全仓业务闭环权限状态与事实一致性整改任务计划书.md)。

## 2026-08-10 主线85：全仓业务编号统一与任务/交付门禁契约校准已合并

- 问题源：`ISSUE-085-001`已关闭；状态`IMPLEMENTED / G0-G5_PASSED / GIT_DELIVERY_MERGED / POST_MERGE_VERIFIED`。
...
- 证据：干净实现 SHA 的9组Live共80项通过；长任务双检查复验2/2；GitHub保护回读`strict=true`、15个contexts且全部`app_id=15368`；[`质量报告`](../quality/2026-08-10-issue-085-全仓业务编号统一与任务交付门禁契约校准.md)。
- Git：源HEAD `867872ed09cbfe556b63fc990f3e258a53044d3c`，push run `31339598287`，PR #424 run `31340197968`，合并SHA `c657cf6dab8a8dafa5c82a8912e9d0207019412d`，post-merge run `31340250134`；实现源分支本地和远端已删除。
...
- 验证：后端2558项、前端478项、V286双数据库、Trivy、PWA冷离线、三视口及双标签SSE通过；新增1、关闭1、净变化0，无悬空。

## 2026-08-06 主线76：项目文件与通讯性能稳定性整改已合并

- 问题源：`ISSUE-076-001`已关闭；状态 `IMPLEMENTED / G0-G5_PASSED / GIT_DELIVERY_MERGED / POST_MERGE_VERIFIED`。PR #395合并SHA为`d046f5e7fd2907e73461460ff65727aac5c981e7`，post-merge run `31064073770`成功。
- 目标：`项目文件目录 → 数据库权限谓词 → 精确COUNT/分页`；`消息页 → 单次附件批量查询 → 稳定分组`；`过期草稿 → 单条事务 → 预期竞态隔离 → 批次续跑`。
- 计划：[`第76条主线`](../plans/第76条主线-项目文件与通讯性能稳定性整改任务计划书.md)。
- 验证：后端325份报告、2543项测试、0失败、0错误、16跳过；真实本地MySQL并发2项、关键API、文件中心与站内通讯页面及控制台通过；独立复核无P0～P3遗留。
- 边界：仅本地dev/test/demo；不新增migration、缓存、消息总线、分布式锁或非本地验收。

## 2026-08-05 主线73～75：文件中心、模板布局与站内通讯实施完成

- 第73条目标链：`项目/业务对象 → 权限交集 → sys_file单一事实 → 只读业务投影或中心版本 → 安全预览/下载 → 审计与派生物清理`；统一查看但原业务继续分域维护。
- 第74条目标链：`业务类型 → 模板 → 版本详情`；只改 V2 页面布局和本地选择状态。
...
- 问题源：`ISSUE-071-001`已关闭；状态`MERGED`，PR #390 已进入当前 `master@ac6bd6c37c99888021ca0e7037a8f2a03d465df2`。
- 目标链：`业务对象 → 服务端附件契约 → 真实格式/病毒扫描 → tenant-scoped MinIO对象 → sys_file元数据 → 审计下载/预览 → 业务不可变门 → 可重试清理 → 对账/备份/恢复`。
- 关键不变量：新对象键隔离租户；列表不下发签名URL或内部路径；投标定版、付款归档和生成文档不可由通用删除破坏；数据库与对象存储失败可补偿、可对账。
- 业务边界：重开第69条开工附件真实上传子链，回归第67条投标版本和第70条付款证据；不新建第二文件主表、对象存储或文件微服务。
- 计划：[`第71条主线`](../plans/第71条主线-附件与文件系统整改完善任务计划书.md)。
- 实现：V277～V280闭合tenant/fileId对象键、规范活动内容哈希唯一性、大小写敏感规范文件名识别、历史非规范名称兼容、耐久对象任务、最小下载审计和对账/复扫；共享附件交互接入合同、开工、分包和结算。
- 验证：迁移前碰撞预检零行，H2 active/legacy、MySQL V280空库及V180→V280升级、真实开工附件链、刷新后合同页回读及69,270字节八列元数据映射隔离恢复通过；`REL-FILE-RESCAN`仅为历史发布门，现按[`current-issues.json`](../backlog/current-issues.json)冻结为`NOT_APPLICABLE_LOCAL_ONLY`、`blocking=false`。

## 2026-08-04 主线69～70：项目准入与资金支出闭环已合并

- 问题源：`ISSUE-069-001`、`ISSUE-070-001`已关闭；状态`MERGED`，PR #389 已进入 `master@da3403809178378c8f002166af744c7b68427c55`。
- 目标链：`中标或直接立项来源 → PREPARING → 指定且已批准的权威 MAIN 主合同 → 同合同金额生成目标成本 → 唯一执行预算 + 生效 WBS + 已批准开工准入 → ACTIVE`。
- 资金链：`项目 → 合同 → 成本/预算 → 申请 → 审批 → 付款 → 现金日记 → 发票/凭证 → 驾驶舱/Trace`；合同锁后当前读，正反向证据和预算守恒均由服务端验证。
- 金额架构：`bid_cost.final_bid_price`仅表示中标价；项目指定且已批准的 MAIN 合同 `current_amount` 为正式合同额唯一事实；`pm_project.contract_amount`只保留服务端同步的兼容投影。
- 最小新增：每项目一条开工准入聚合，复用现有工作流与文件能力；不建设通用检查清单、平行合同/预算/WBS/成本台账。
- 计划：[`第69条主线`](../plans/第69条主线-项目立项主合同与开工准入闭环整改任务计划书.md)、[`第70条主线`](../plans/第70条主线-项目资金支出闭环当前HEAD复验与整改任务计划书.md)。
- 验证：V274真实MySQL、H2/MySQL迁移、MySQL并发、后端2444项、前端443项及内置浏览器通过；生产发布证据未取得且不在本轮授权范围。

## 2026-08-04 主线67～68：工程投标与施工阶段主线已合并
...
- 状态：`MERGED`；PR #387 完成原实施，PR #388 完成评审反馈整改并进入 `master@4e0a6380ccae48ba36495c9166751751fb659bc0`；`ISSUE-068-001`关闭。计划见[`第67条`](../plans/第67条主线-工程投标资料与成本日记账闭环任务计划书.md)、[`第68条`](../plans/第68条主线-施工阶段主线闭环整改任务计划书.md)。

---

## 历史快照强围栏（以下至文件末尾）

> 以下内容全部是历史时点快照，仅供追溯。下文出现的“当前”“状态”“完成”“阻塞”等字段均只描述各自历史时点，不代表 v1.6 当前事实，不得用于 Ready、验收、发布或角色放行裁决；现行事实以上方最新主线、当前代码和 `docs/backlog/current-issues.json` 为准。

## 2026-07-31 主线58：新版前端正式切换完成（历史）
...
- 当时新版根路径镜像完成 `V2_ROOT→LEGACY_ROLLBACK→V2_RESTORE`；当时的生产发布与三项既有 `RELEASE_GATE` 状态不变。
...
- 当时 Ready 为0；候选、冻结项和三项生产 `RELEASE_GATE` 当时由 `docs/backlog/current-issues.json` 承接。现行状态须直接读取该 JSON。
- 当时未创建 Tag/GitHub Release、未发布生产、未切正式入口、未退役 Legacy；现状以上方主线58为准。

## 2026-07-31 主线57：V1.5开发版本正式收口

- 第57条A～G当时全部通过：第55条非生产候选全部唯一决策，第56条D0～D4正式验收通过；当时生产发布门保持阻塞。
...
- 当时状态为`COMPLETED / V1.5_DEVELOPMENT_CLOSED / SAME_HEAD_CI_PASSED / LOCAL_RC_ACCEPTED`；代码与安全候选`c3cfcf942491f810d1c79b2cd326298524e8a1e7`的push/PR同SHA CI 13/13及Pre-PR verifier通过；新增后续项0、关闭后续项0、净变化0。
...
- C0通过；C1材料不合格退货子链已建立服务端权威退货、状态同步、业务重试幂等、冲销恢复与库存不变证据。
- C1技术子链已建立批准方案、批准图纸版本、已确认交底、`CLEAN`附件、已提交日报/WBS施工引用的真实V2闭环。
- 两项执行中发现均已做根因最小修复并复验；C1其余节点及C2～C5仍待执行，M3不通过、支线D不得启动。

## 2026-07-29 主线57 M2：第56条基础前置通过

- 目标9角色/27用户完成服务端对账及27/27真实登录；6个被活动事实引用的执行支持账号保留并排除目标计数。
- 合作方、材料复用；通过真实V2表单补齐部门、岗位、成本科目各15，服务端回读唯一启用。
- 当前15项目重新绑定为施工总承包10、专业分包3、劳务分包1、材料采购1，活动成员61；禁止按原计划假设重建。
- M2通过，进入四类业务金丝雀；完整链、金额、库存、审批、状态和权限尚未验收。

## 2026-07-29 主线57 M1：第55条非生产候选决策卡冻结

- 8项候选/观察逐项冻结用户、权威对象、最小缺口、指标、边界、回滚和第56条取证字段。
- `A-07/A-10`在V1.5保持冻结；`A-02/A-03/A-04/A-06/A-09`等待真实链，AutoPilot等待A至D完整周期。
- 0 Ready、0业务实施、0重复、0悬空；未引入预测、报表、消息、技术或集成平台。

## 2026-07-29 主线57 M0：V1.5遗留完成与收口启动

- 第57条成为第55、56条唯一编排入口；原稳定键和现行状态源继续有效，不复制全局Issue。
- 启动基线为`master@f984c162d30f49a076d9411d83e366e7481f1fb9`，活动13、重复0、悬空0。
- 第56条独立运行态与V2入口成立；目标27人存在但活动集合多6人，15个项目和多域事实为部分实施、未形成完整链验收，禁止按旧状态重复创建。
- 下一阶段为第55条8项非生产候选决策门；生产与目标环境5项前置保持排除和原状态。

## 2026-07-28 主线55：六项产品候选决策门治理收口
...
- 7项产品候选仍缺真实样本或外部前置，保持原决策门；`ISSUE-055-001`已转Done，当前Ready为0。
- Trivy新增无破坏隔离冷缓存手工触发：新job工作区跳过共享缓存恢复，默认缓存与安全阈值不变。
- 实施提交`b19d954c0d6434878025508c60c75e3970d39e09`的push CI 13/13通过；同SHA手工CI缓存恢复为`skipped`，官方漏洞库与Java DB重新下载，`HIGH/CRITICAL`扫描及13项job全绿。
- 新增0、关闭1、净变化-1、重复0、悬空0；现行问题源12项。下一完整AutoPilot周期仍由v2效率观察承接，不部署或操作生产。

## 2026-07-28 主线55：业务编码、候选观察与三报告非部署整改

- 业务编码30项完成治理：22项内部编码由服务端自动生成，8项外部、版本、关系和期间字段保留人工语义。
- v1 `cycleEfficiency`观察因正式v2不再产生新v1样本而关闭；其余7项产品候选、v2效率和Trivy观察保持，0项Ready。
- 三份外部审计报告仅保留当前仍成立的退出Cookie与角色/权限撤销根因；已用最小认证路径整改，不引入会话平台或migration。
- 活动事项14：现行问题源13项，第55条仅直接承接`DATA-001`；`BUSINESS-CODE-AUTOMATION-GOVERNANCE`已关闭，重复0、悬空0。

## 2026-07-27 主线55 M5/M6：目标环境前置未齐
...
- `ISSUE-053-043～046`全部通过；87条route name唯一处置，路由台账`0/87/0`，路由、权限、API和角色权威来源已冻结。
- Clean-room 收口扫描204+16文件、router/navigation 28项、类型/Lint/构建/包体、V2 unit 397/397、design 77/77、后端2324项和migration gate 91/91通过。
...
- `ISSUE-053-036～042`全部完成；登录/错误、账号、基础资料、成本科目、流程配置、系统管理及全局工具24条路由完成真实V2承接或受控重定向，台账`0/87/0`。
...
- 当前13节点复核结论为核心链可运行但P0未闭合；历史“P0—P3完成”和测试快照不再作为当前验收证据。
...
- `CODE-001` 无量化拆分收益，有据关闭且不改付款生产主链；`CODE-002` 以既有稳定API完成3处测试注解迁移，并删除2处无行为renderer调用。
...
- 财务定时对账完成按租户事务失败可观测和真实失败注入；失败租户全回滚，健康租户及预警继续，金额写后回读。
- Legacy/V2 建立脱敏客户端错误闭环；认证、限流、低基数指标与本地告警规则成立。普通限流与登录锁定状态已隔离。
...
- 在 `master@536584c8372b554604103d9a4f6ca8fc7c89fa4d` 复核25项唯一基线；现金预测版本并发已有项目行锁、正式M6证据和真实并发测试，状态关闭。
...
- M6分包任务/计量/结算6条路由与付款、费用、收入回款、发票、资金运营、日记账、预测、凭证、月结10条路由，共16条目标全部由真实V2或受控重定向承接；台账`24/63/0`。
- 复用既有后端P0闭环，金额、来源、可付余额、日记账、预测、凭证、期间、审批和冲销均由服务端裁决；V2不新建平行资金账、前端权威计算或手工凭证生成入口。
...
- M5 10条目标路由全部由真实Clean-room V2承接或受控重定向；台账达到`LEGACY_ONLY=40 / V2_ACCEPTED=47 / V2_SOURCE_AVAILABLE=0`。
- 供应商招采、采购申请/订单/验收、仓库/库存/来源流水/调拨及领退料的数量、价值、来源和状态保持服务端权威。
- 完成性复核关闭前端库存状态推导、可见原始主键、E2E契约漂移和后端并行夹具隔离缺口；后端全量2198项、M5顺序复验244项、V2单测321项、目标E2E 39/39、静态门、构建及demo verify通过。M5正式收口，M6未启动。

## 2026-07-24 主线53 M5：053-028领料、出库与退料V2通过

- `/inventory/material-requisition`已由真实Clean-room V2承接，M5 10条目标路由全部迁移完成。
- 领料申请/明细/提交/出库、退料确认/冲销与trace复用既有服务端闭环；数量、金额、状态、库存与来源流水均以服务端回读为准。
- 后端专项4项、V2单测288项、目标E2E 7/7及静态门通过；台账达到`40/47/0`。`ISSUE-053-029`成为唯一Ready，负责全量退出门。

## 2026-07-24 主线53 M5：053-027仓库、库存与来源流水V2通过

- `/inventory`重定向及仓库、库存、来源流水三路由已由真实Clean-room V2承接。
- 仓库范围/CRUD、库存数量/价值/KPI、阈值、在途、历史净领料、来源流水和幂等调拨复用既有服务端闭环；通用手工出入库继续失败关闭。
- 后端库存专项110项、V2单测284项、目标E2E 8/8及静态门通过；台账达到`41/46/0`。`ISSUE-053-028`成为唯一Ready。

## 2026-07-24 主线53 M5：053-026采购申请、订单与验收V2通过

- `/inventory/purchase-request`、`/purchase/order`、`/purchase/receipt`已由真实Clean-room V2页面承接，`/purchase`稳定重定向到订单页。
- 申请至订单、部分/完整验收连续流程复用既有后端；来源、数量、预算、金额、已收、剩余与状态均以服务端回读为准。
- 后端专项113项、V2单测281项、目标E2E 9/9及静态门通过；台账达到`45/42/0`。`ISSUE-053-027`成为唯一Ready。

## 2026-07-24 主线53 M5：053-025供应商招采与履约V2通过

- `/supplier-sourcing`已由真实Clean-room V2页面承接，事件至合同、订单/验收事实、履约评价、退货、黑名单和trace闭环成立。
- 七组细分权限互不兜底；动作由服务端状态控制，金额/评分保持十进制字符串，重复点击、Abort和陈旧响应隔离成立。
- 后端专项5项、V2单测277项、目标E2E 9/9及静态门通过；台账达到`49/38/0`。`ISSUE-053-026`成为唯一Ready。

## 2026-07-24 主线53 M5：053-024供应链金丝雀通过

- 供应链共享契约冻结8项查询权限、8组API入口及库存/采购/验收十进制字段；V2复用既有请求核心提供仓库、库存台账、采购订单、验收四条GET薄服务。
- 仓库查询权威权限为`inventory:warehouse:list`，旧`query`权限负样本403；通用手工出入库继续失败关闭。
- 三条真实GET的4张业务表前后快照一致；后端55项、V2 274项及静态门通过。台账保持`50/37/0`，`ISSUE-053-025`成为唯一Ready。

## 2026-07-24 主线53 M5：已进入，供应商招采Ready

- M5目标冻结为供应商招采、采购申请、订单、验收、仓库、库存、流水和领退料共10条Legacy路由；当前台账`LEGACY_ONLY=50 / V2_ACCEPTED=37 / V2_SOURCE_AVAILABLE=0`。
- 现有供应商招采P0闭环及采购、验收、库存、调拨、领料后端能力继续作为权威事实；V2不建立平行库存账，不开放通用手工出入库，不做前端权威数量计算。
- `ISSUE-053-024`已通过；`ISSUE-053-025`为当前唯一Ready，026～029保持Planned。M6～M8、正式入口、Legacy退役和生产发布边界不变。

## 2026-07-24 主线53 M4：全量退出门通过

- M4合同、变更/投标、目标成本、成本/利润、预算/产值共18条路由全部由真实Clean-room V2页面或受控重定向承接，台账固定`LEGACY_ONLY=50 / V2_ACCEPTED=37 / V2_SOURCE_AVAILABLE=0`。
- 后端全量与顺序复验、V2全量静态门、M4 Playwright 16/16及demo verify通过；Surefire内存门和demo种子验证漂移已最小根修。
- `ISSUE-053-017～023`全部收口，当前无M4 Ready；M5～M8、正式入口、Legacy退役、同SHA CI和生产发布仍未启动或授权。
...
- 权限/项目范围、金额字符串、状态机、附件恢复、对象深链、查询零写和写后权威回读由后端专项、demo v10、V2全量门及live E2E共同证明。
- M3完成；M4、正式入口切换、Legacy退役与生产发布仍未授权，不能从本结论推导上线就绪。

## 2026-07-22 主线53 M3：053-015竣工收尾闭环通过

- `/project-closeout`已迁移为真实Clean-room V2页面，发起至项目关闭及trace成立；台账达到`68/19/0`。
- 十动作由11个真实demo账号分离；10个阶段、合法关闭链、5类CLEAN附件和责任人项目成员边界完成验真。
- M3全部10个目标路由已实现；`ISSUE-053-016`随后完成全量退出门与治理收口，未切正式入口或发布生产。

## 2026-07-22 主线53 M3：053-014技术管理闭环通过

- `/technical-management`已迁移为真实Clean-room V2页面，方案、图纸版本/会审、RFI、交底、施工依据、归档及trace成立。
- 九动作由10个真实demo账号独立验证；六类附件失败可在不重复业务写的前提下恢复。
- 路由台账`LEGACY_ONLY=69 / V2_ACCEPTED=18 / V2_SOURCE_AVAILABLE=0`；仅项目收尾路由和M3退出门未完成。

## 2026-07-22 主线53 M3：053-013质量安全整改闭环通过

- `/quality-safety`已由占位替换为真实Clean-room V2页面，覆盖质量计划、检查、问题、整改、复检、后果、反向trace和阶段附件。
- 权限查询/计划/检查/整改/复检/后果已由六个真实demo账号验证；后端整改证据与复检证据授权不再混用。
- 路由台账更新为`LEGACY_ONLY=70 / V2_ACCEPTED=17 / V2_SOURCE_AVAILABLE=0`；技术管理、项目收尾和M3退出门仍未完成。

## 2026-07-21 主线53 M3：053-012计划与现场日报通过

- 两路由已达到`V2_ACCEPTED`，台账`71/16/0`；快照写权限、陈旧写保护、提交及纠偏审批并发、FormData与真实trace数组完成根修和回归。
- 本地dev/demo真实分权、受控写侧、附件不可变、三视口和axe通过；受控身份与数据已精确回滚。013未启动，须另行补为Ready。

## 2026-07-21 主线：Clean-room V2 M3项目对象工作区通过

- `ISSUE-053-011`五路由及项目/成员受控写侧已通过，台账`73/14/0`；后端专项14项与主项目37项、V2单测93项、live E2E 9项及真实成员首次新增/删除均通过。
- `demo.member-readonly`仅有项目查询/成员查看权限，真实读200、写403；服务端派生成员字段错误前置校验已修复，Service强制覆盖不变。成员删除后重新加入P1已通过原记录原子恢复闭环，唯一键与租户/项目范围不变；012随后已完成。

## 2026-07-21 主线：Clean-room V2 M3详细实施序列冻结

- M3拆为项目对象、计划日报、质量安全、技术管理、竣工收尾和退出门六个串行切片；最终目标为10个路由迁移、台账`68/19/0`。
- `ISSUE-053-011～012`已完成；013～016继续按前项通过与当前证据逐条补货，不并行、不以计划文本冒充Ready或完成。
- 权限、跨项目范围、金额字符串、状态机、附件和真实demo身份是逐阶段停止门；后端、数据库、Legacy、正式入口和生产边界保持不变。

## 2026-07-21 主线：Clean-room V2 M3项目履约启动

- M0～M2已通过；M3目标为项目管理、计划现场、质量技术和项目收尾10个路由，当前均保持`LEGACY_ONLY`。
- `ISSUE-053-010`金丝雀已冻结项目、总览、成员契约与只读请求基线；19个V2单测文件88项、契约/V2类型、Lint、Clean-room和构建通过，服务底座不计作页面迁移。
- 后端、数据库、Legacy、正式入口和生产边界不变；首个页面Ready须在核实项目写侧权限、范围、金额、状态机和真实demo身份后补货。

## 2026-07-21 主线：Clean-room V2 M2全量退出门通过

- `ISSUE-053-005～009`全部完成；驾驶舱、个人审批、驾驶舱内预警处置、有限通知摘要和权限过滤报表目录构成M2 P0完成范围。
- 九个目标路由已达到`V2_ACCEPTED`，台账为`78/9/0`；`/approval/process`及M3～M7业务页仍未迁移，六个对象详情占位仍属于后续阶段，不得把M2解释为V2全量替代Legacy。
- 八角色、四类身份、1440/1024/390、权限与项目范围、金额字符串口径、审批幂等/权限负向、权威预警范围、报表过滤、axe、控制台、包体和Dashboard SQL≤20退出门通过；桌面端通知、审批详情与预警处置统一公共弹窗视觉，下拉菜单向下展开且无裁切。
- 本阶段不切正式入口、不发布生产、不退役Legacy；M8整站切换、回滚演练和生产动作仍需独立授权与目标环境证据。
...
- 动作展示固定为服务端 `availableActions` 与显式动作权限双门禁；写请求复用 CSRF、UUID 幂等键和提交中锁，并在成功、冲突或业务拒绝后回读权威详情。
- 流程模板管理 `/approval/process` 仍保持未迁移；本切片仅补充本人可见业务类型查询、已办业务编号和八角色审批测试矩阵，不新增工作流状态或数据库结构。审批个人工作台更新为 `Complete(P0)`，流程模板管理与通知/预警联动仍为 `Partial`。
- 本地真实演示库验收通过：八角色各覆盖5种实例状态和6项审批动作权限；四类列表、受控实例深链、未知实例、权限负例及1440/1024/390三视口审批 live E2E 10项通过，未执行审批业务写入。

## 2026-07-20 主线：Clean-room V2 M2 八角色驾驶舱真实数据闭环

- `cgc_pms_demo_v2` 的在建项目已补齐八角色驾驶舱所需真实业务事实；成本经理额外复用现有 `5401` 权威科目树，形成1个父级、4个子级的两级只读下钻，目标、实际、动态、偏差父子金额均守恒。
- 财务经理已按现有合同、付款申请和付款记录增加合同资金两级分解；合同汇总与付款记录下钻共享原财务端点，全部项目结果继续受项目数据范围约束。
- 成本、财务保留项目/报告期/卡片周期联动趋势；其他角色在没有权威时间序列时展示 API 返回的“经营动态”，不伪造折线数据，也不再显示与下方真实指标冲突的空图表。
- `ISSUE-053-006` 本地数据门禁解除并通过；演示包18阶段、V2单测69项与live E2E 7项通过。驾驶舱 V2 页面状态更新为 `Complete(P0)`；报表目录仍为 `Partial`，不得据此扩大 M2 其他工作区完成范围。
...
- V2 只呈现 `/projects` 在当前租户与后端数据范围内返回的项目；无项目不回退租户全量，无权角色和非法月份不发送畸形驾驶舱请求。
- M2 当前状态为 `共享契约与请求基线完成 / 八角色真实驾驶舱待实施`。驾驶舱与报表总体仍为 `Partial`，不得把请求底座计作页面迁移完成。

## 2026-07-19 主线：Clean-room V2 M1 完整退出门通过

- `ISSUE-053-004` 已补齐 1440/1024/390 响应式、减少动效、键盘焦点、语义地标、可访问名称、403/404、全局错误边界、加载/空/错误状态和非业务通知占位。
- ADMIN、普通有权、无权、未登录四类身份与三视口 Edge 样本通过；axe 无 serious/critical，控制台新增 warning/error 为 0，壳层业务 API 请求为 0。Legacy 路由台账仍为 87 个命名路由、73 个视图引用、65 个独立视图。
- M1 当前状态为 `设计系统、安全会话、八域应用壳、响应式、无障碍与错误状态全部完成`。M2 真实驾驶舱尚未启动，须经产品决策与 Ready 补货，不得把当前壳层占位视为业务页面迁移。

## 2026-07-19 主线：Clean-room V2 M1 应用壳切片完成

- `ISSUE-053-003` 已实现八域、30 个工作区、57 个 Tab 路径及项目/合同/结算对象上下文；菜单、Tab 与深链只消费权限码或通配权限，不以角色名硬编码业务可达性。
- 响应式壳覆盖 1440 桌面、1024 紧凑栏和 390 移动抽屉；全局项目和报告期当前只有确定的空槽位，不读取业务 API、不注入演示数字，未知深链 query、刷新和退出不会形成陈旧上下文。
- M1 当前状态为 `设计系统、安全会话、八域应用壳完成 / 完整错误页与全局失败状态待实施`。下一串行切片为 `ISSUE-053-004`；业务迁移台账仍保持 86 个 `LEGACY_ONLY` 和驾驶舱 `V2_SOURCE_AVAILABLE`，真实驾驶舱仍属于 M2。

## 2026-07-19 主线：Clean-room V2 M1 安全会话切片完成

- `ISSUE-053-002` 已建立仅依赖共享 DTO、权限码和 API 相对路径的 V2 同源请求核心；登录、用户恢复、CSRF、401 单飞恢复、错误去重、退出清理和路由守卫完成，前端不持有或持久化 token。
- ADMIN、普通项目经理、未知身份和未登录浏览器样本与 Legacy 后端契约一致；1440 与 390 视口无横向溢出，认证页继续使用既有 V2 设计令牌和最小组件。
- M1 当前状态为 `设计系统、安全会话完成 / 应用壳、域级权限守卫与完整错误页待实施`。下一串行切片为 `ISSUE-053-003`；真实驾驶舱仍属于 M2。

## 2026-07-19 主线：Clean-room V2 M1 设计系统切片完成

- `ISSUE-053-001` 已冻结可追溯到选定驾驶舱视觉源的 V2 设计令牌，并建立 11 类最小无业务组件；令牌与组件只存在于 `frontend-admin-v2`，未复用 Legacy Vue、Pinia、DOM、CSS 或消息层。
- 1440、1024、390 三视口组件预览通过，无横向溢出；移动控件为 44px。Dialog 键盘关闭和焦点恢复、禁用/加载/错误状态、减少动效基线均有自动化或浏览器证据。
- M1 当前状态为 `设计系统完成 / 安全会话、应用壳、完整响应式与错误状态待实施`。下一串行切片为 `ISSUE-053-002`；真实驾驶舱仍属于 M2，不在本切片迁移。

## 2026-07-19 增量：会计凭证页面说明与自动生成边界对齐

- 会计凭证页已移除“来源单据到借贷科目的生成规则待会计确认”陈旧说明，明确付款、回款凭证由权威业务写侧自动生成，手工生成入口未开放。
- 页面继续只提供查询、详情、复核、过账、重提和冲销等既有受权操作；未新增生成按钮、API、`accounting:add` 权限、科目策略、迁移或业务数据。
- 专项页面测试、Legacy 类型检查和真实只读浏览器复验通过；`A-01-ACCOUNTING-GENERATION-COPY` 状态为 `Complete(P1)`。

## 2026-07-18 主线：全量 UI Clean-room V2 M0 隔离底座完成
...
- 新环境迁移已冻结为 MySQL/H2 `B215`，历史 V1—V215 原文件按 SHA-256 清单完整移入 legacy 兼容目录；新空库直接选择最终基线，既有数据库继续验证和执行历史增量链。
...
- 第48条主线最小闭环限定为受限HTML/CSS、付款申请单和结算单两种系统模板、同步预览/生成、内部归档、当前业务权限下载与审计追溯；不包含自由设计器、Office、脚本、签章、邮件、批量或异步。
- 状态：`M0、M1、M2、M3、M4通过 / M5候选门未启动`。V212已建立通用文档域，模板版本、生成状态、内部归档、对账、权限和默认关闭开关具备可复验证据。
- M1现状：OpenHTMLtoPDF 1.1.40、PDFBox 3.0.8和随包Noto Sans SC 6.1.0完成中文、安全资源与发票回归；生产发布仍受LGPL/OFL材料复核、代表性容量样本和dev/test/demo金丝雀阻塞。付款/结算生产Provider和前端入口仍为M2/M3范围。
...
- 详情失败不使用分页行兜底；请求序号使快速切换只接受最后目标，点击新增同时使在途编辑详情失效。
- 后端模型、权限码和租户语义未改；管理员/`system:user:query` 正样本、无关权限403、跨租户不可见与无密码响应已验收。
...
### 2026-07-20 增量：M2 八角色驾驶舱统一风险等级（历史）

- Clean-room V2 八角色驾驶舱均已接入真实后端 API；成本、财务保留权威趋势，其他角色复用各自业务列表展示经营动态。
- 风险展示统一为“高/中/低/其他”，“全部预警”仅为筛选汇总；映射来源分别为告警严重度、合同到期窗口、逾期天数、业务状态及项目待办语义，不改原始业务枚举。
- `cgc_pms_demo_v2` 17阶段验真通过，八角色每个等级均有至少一条可筛选数据；真实浏览器 E2E 覆盖32个角色—等级组合。
- 原“五类驾驶舱角色的真实账号与数据可见范围”待复验项关闭；正式入口切换仍不在 M2 范围。
...
### 2026-07-17 增量：财务核算与月结 P0 闭环（历史）
...
### 2026-07-18 增量：V2 导航与工作区（历史）

- 信息架构入口已收敛为工作台、项目履约、商务合约、供应链与物资、分包与结算、资金财务、基础资料、系统管理八个业务域。
- 30 个二级工作区和 54 个路由化 Tab 复用既有业务页面、URL 与权限码；采购、库存、分包、成本和资金等连续流程已有统一工作区入口。
- 项目、合同、结算对象详情使用上下文导航，不升格为侧栏；个人资料、偏好设置、帮助、通知和待办属于全局工具区。
- 当前状态：`Implemented`。本地全量测试、构建、包体和真实浏览器验收通过；V2 候选路径尚未启用，不代表已经完成 URL 改名。

### 2026-07-19 增量：核心字典与质量处罚合同追溯（历史）

- DQ-005：质量处罚、返工成本与合作方评价强制关联合同；前端候选合同、DTO 与 Service 共同校验租户、项目和责任合作方关系，状态为 `Complete(P0)`。
- DQ-009：V216 九类核心字典和正式工作流状态机统一由权威定义驱动；普通业务角色可读，核心业务写入受后端校验，核心标签防删除/停用并纳入审计，状态为 `Complete(P0)`。
- 三工作树资产已收敛到 `codex/mainline-53-ui-v2-m0`；源工作树保留作历史对照。字段覆盖与数据库完整性门已按 V217 更新为 166 张目标表、374 条外键。
