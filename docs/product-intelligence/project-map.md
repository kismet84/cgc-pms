...
## 2026-07-12 增量：WBS 单前置 FS 状态门禁候选

- 现有分包任务已支持单前置 FS、同项目/环/日期校验和延期风险展示。
- 当前后端仍允许前置任务未完成时把后续任务保存为 `IN_PROGRESS` 或 `COMPLETED`，展示型风险尚未形成执行门禁。
- 下一最小候选只复用现有单前置字段，在 Service 拒绝该状态推进，并让前端禁用对应选项；不新增关系表、状态、自动排程或多前置。
