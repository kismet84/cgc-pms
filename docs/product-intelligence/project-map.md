...
## 2026-07-17 候选：库存历史净领料基线

- 库存流水已区分领料出库、材料退料入库、验收、调拨和供应商退货，具备按当前库存项形成历史领料基线的权威事实。
- 最小闭环只汇总含今日的30/90个本地自然日：分别展示领料、退料和净领料，范围由服务端库存与仓库关系确定；一次查询完成两个窗口。
- 本切片明确不是需求预测，不输出日均、缺货日或建议采购量；完成后 A-02 仍保留工作日历与预测算法决策。
