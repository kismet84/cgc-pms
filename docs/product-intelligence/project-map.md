...
## 2026-07-17 候选：同项目跨仓库存调拨过账

- 库存页已经能从当前库存项反查同项目、同物料、其他启用仓库的可调拨余量，并扣除来源仓安全库存；当前结果只是查询快照，不会预占或过账。
- 最小闭环新增一张调拨事实表和成对库存流水，在同一事务中按来源仓历史单位成本扣减并等值增加目标仓；不开放通用手工入库/出库。
- 写权限取库存编辑与库存事务权限交集，服务端再次校验租户、项目、仓库、物料、安全库存、幂等键和并发版本；不同项目、同仓、不同物料或超可调拨量全部拒绝。
