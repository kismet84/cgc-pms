...
- `.agent-runtime/`: local agent runtime artifacts — excluded from repo (`.gitignore`)
...
python scripts/rebuild.py backend       # 仅后端 (mvnw -> docker restart)
python scripts/rebuild.py frontend      # 仅前端 (docker restart)
...
### ESLint / Prettier 非零退出码不是失败

ESLint 退出码 1 = 检测到 lint error（正常工作），不是崩溃。Prettier/ShellCheck 类似。Bash 工具将任何非零退出码当作 Error，但 linter 的任务就是发现问题。

### .gitignore 只阻止未跟踪文件

已提交到 Git 历史中的文件不受 `.gitignore` 影响。要清理：`git rm --cached <path>` 然后提交 + push。之后 `.gitignore` 规则才会对其生效。

### 源代码测试中使用正则需预期跨行

`eslint --fix` / prettier 会将模板格式化为多行，使 `<p>{{ fn() }}</p>` 变为 `<p>\n{{ fn() }}\n</p>`。做 `toMatch` 正则测试时用 `[\s\S]*` 替代直接匹配。

## Frontend Design System (lg-* CSS)

- `lg-page` → 页面容器（min-height, padding）
- `lg-search-bar` → 搜索栏（flex row with gap, border-bottom）
- `lg-grid` → 双栏布局（left + analysis rail, 1280px 折叠）
- `lg-kpi-strip` / `lg-kpi-card` → KPI 横条卡片
- `lg-table-wrap` → 表格容器（白底, 边框, 阴影, 表头 #ebede9）
- `lg-toolbar` → 工具栏（左/右分组）
- `lg-panel` → 右侧分析面板（`--surface-subtle` 背景）
- `lg-type-list` / `lg-type-row` → 分布色条列表（5列 grid）

See `frontend-admin/src/assets/styles/global.css` for full definitions.
...
1. **Think Before Coding** — State assumptions explicitly. If unclear, ask.
