...
1. 读项目 `ready` / `current-issues` / `blocked` / `focus` 事实源，只选择合格 Ready Issue 实施；Ready 为空时，先从 `docs/backlog/current-issues.json` 拆安全可执行的存量问题，再处理当前 focus 可解除阻塞，之后才使用决策证据完整的 Ad-hoc Candidate，最后才刷新产品情报。
...
- 存量问题是 Ready 补货的第一事实源，但只允许自动拆分 `OPEN` / `OBSERVATION`、`blocking=false`、证据与验收标准完整且不是聚合父项的问题；`RELEASE_GATE`、`FROZEN`、`NEEDS_CONFIRMATION` 和生产前置不得自动实施。每个存量问题 Ready 必须保留 `[stock:<issueKey>]` 唯一标记，收口时同步更新或移除原问题，防止重复拆单。
