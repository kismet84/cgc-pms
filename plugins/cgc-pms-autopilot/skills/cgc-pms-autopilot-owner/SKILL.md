...
- 运行态或浏览器验收前检查后端 health、前端入口和 dev-login；任一不通先归为环境前置类，刷新运行态，并按项目配置的 `runtimeRefresh.waitSeconds` 稳定等待后复验。
- 先做失败分类，再决定复跑、修复或阻塞；不得把一次命令失败直接定性为业务代码失败。
- 并行上限和安全模式只从项目配置的 `maxParallel`、`maxParallelIssues` 与 `parallelSafetyMode` 读取，实际并行数不得超过其中适用的最小上限；不能证明无关联或无法读取配置时串行，数据库、权限、安全、租户、金额、审批状态机等任务不得并行。
...
- 本地自动合并只允许配置的 baseBranch；不确定已提交 worktree 不猜测合并。存在有效 phase checkpoint 时保留现场并按阶段恢复；缺少或冲突时进入 quarantine，禁止直接重跑。
